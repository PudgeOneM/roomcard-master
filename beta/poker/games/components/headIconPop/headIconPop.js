...
var headIconPop = 
{
    ...
    getPreLoadRes:function()
    {
        ...
        return [
            resp + 'headIconPop.plist', 
            resp + 'headIconPop.png',
            resp + 'circle00.png',
            resp + 'circle01.png',
        ]
    },
    ...
    {   
        ...
        var node  = cc.BuilderReader.load('components/headIconPop/res/headIconPop.ccbi', headIconPop)
        headIconPop.nameLabel.setString(user.szNickName)  
        headIconPop.idLabel.setString('id:' + (200000 + user.dwUserID*88 + 66666) )       
     
        var headIcon = new cc.Sprite('#headIcon.png')
        var hnode = getCircleNodeWithSpr(headIcon)
        hnode.setScale(0.8)
        headIconPop.headNode.addChild(hnode)
        ...
        headIconPop.nameLabel.y = 410
        headIconPop.vipLabel.visible = false
        if(dwUserID == selfdwUserID)
        {
            headIconPop.diamondSpr.setVisible(true)
            headIconPop.diamondTTF.setVisible(true)
            headIconPop.diamondTTF.setString(self.lDiamond)
            headIconPop.diamondSpr.setPositionX( headIconPop.diamondTTF.getPositionX() -  headIconPop.diamondTTF.getContentSize().width - 18)
       
            var t = user.dwMemberOverTime*1000 - new Date().getTime()
            if(t>0)
            {
                headIconPop.nameLabel.y = 420
                headIconPop.vipLabel.visible = true
                ...
                if(day>0)
                {
                    ...
                    headIconPop.vipLabel.setString(originStr)
                }
                else
                {
                    ...
                    clock.tickLabel(headIconPop.vipLabel, originStr, endStr, getStrFun) 
                }
        ...
        {
            headIconPop.diamondSpr.setVisible(false)
            headIconPop.diamondTTF.setVisible(false)
        }
        ...
        listView.setContentSize(headIconPop.itemListNode.getContentSize().width-10,headIconPop.itemListNode.getContentSize().height+10)
        listView.x = 5
        listView.y = -10
        headIconPop.itemListNode.addChild(listView)
        ...
        //
        headIconPop.timeLabel.setString("牌局数")
        if(user.dwDrawCount)
        {
            headIconPop.winRateLabel.setString("胜/平/逃") 
            headIconPop.winRateNumLabel.setString(user.dwWinCount + '/' + user.dwDrawCount + '/' + user.dwFleeCount )
        }
        else
        {
            headIconPop.winRateLabel.setString("胜/逃") 
            headIconPop.winRateNumLabel.setString(user.dwWinCount + '/' + user.dwFleeCount )
        }

        var winRateNumLabelW = headIconPop.winRateNumLabel.getContentSize().width
        var scale = winRateNumLabelW < 150?1:150/winRateNumLabelW

        headIconPop.winRateNumLabel.setScaleX(scale)

        var count = user.dwWinCount + user.dwLostCount + user.dwDrawCount + user.dwFleeCount
        headIconPop.timeNumLabel.setString(count)
        ...
        headIconPop.circleNode.addChild(right)
        headIconPop.circleNode.addChild(left)
        ...
        cc.eventManager.addListener(listener, headIconPop.voiceSpr)
        ...
        cc.eventManager.addListener(listener, headIconPop.closeSpr)
        ...
        cc.eventManager.addListener(listener, headIconPop.kickSpr)
        ...
        cc.eventManager.addListener(listener, headIconPop.bankdownSpr)
        ...
        headIconPop.bankdownSpr.setVisible(isShowBankDown)
        headIconPop.kickSpr.setVisible(isShowKick)
