...
{   
    ...
    enterListener:function(msg)
    {
        ...
        //         case SUB_S_GAME_SET: 
        //         {   
        //             var body = buffer2StructObj(msg.slice(8), 'CMD_S_GAME_SET') 
        //             cmdBaseWorker.onCMD_GameSet(body)

        //             var event = new cc.EventCustom("cmdEvent")
        //             event.setUserData(['onCMD_GameSet'])
        //             cc.eventManager.dispatchEvent(event)
        //             break
        //         } 
        //         }
        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                ...
                // case SUB_S_HEAP_INIT: 
                // {   
                //     var body = buffer2StructObj(msg.slice(8), 'CMD_S_HEAP_INIT') 
                //     cmdBaseWorker.onCMD_HeapInit(body)

                //     var event = new cc.EventCustom("cmdEvent")
                //     event.setUserData(['onCMD_HeapInit'])
                //     cc.eventManager.dispatchEvent(event)
                //     break
                // } 
                case SUB_S_OUT_CARD: 
                ...
                case SUB_S_MOSHI_RESULT:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_MOSHI_RESULT') 
                    cmdBaseWorker.onCMD_MoshiResult(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_MoshiResult'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                }
