...
{   
    ...
    onCMD_StatusFree:function() 
    {
        ...
        {
            topUINode.gameSelectNode.visible = true
            
            if (tableData.tableID >= 0){
                // tableNode.showPiaoHuaLabel.setPositionX(playNode.scoreTTF.getPositionX() + 65)
                tableNode.showPiaoHuaLabel.setString('飘花')
            }
        }
        else
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        //下面这条语句，add at 0901-pm
        if(cmdBaseWorker.wShowLightUser != INVALID_WORD)
            playNode.timer.switchTimer([tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wShowLightUser)])        

        var self = tableData.getUserWithUserId(selfdwUserID)
        //--begin erase at 0630,嘉兴不需要跑分
        // for(var i=0;i<GAME_PLAYER;i++)
        // {
        ...
        ////////////////////以下是嘉兴规则////////////////////
        cmdBaseWorker.isCHongLian = true
         //不能吃什么打什么
        playNode.setCardEnableColor(self.wChairID, playNode.handMajiangs4D[0], 0)
        playNode.setMagicCardEnabledColor(self.wChairID, playNode.handMajiangs4D[0], 0)
        if(self.wChairID == cmdBaseWorker.wCurrentUser && !cmdBaseWorker.bthePiaoHuaUser[self.wChairID])
            playNode.setCardEnableColorAfterPiaoHua(self.wChairID, playNode.handMajiangs4D[0], 0)
    ...
    onCMD_OutCard:function() 
    {
        ...
        playNode.setMagicCardEnabledColor(self, playNode.handMajiangs4D[0], 0)
        ////////////////嘉兴规则，结束//////////////////
    ...
    onCMD_SendCard:function() 
    {
        ...
        /////嘉兴规则，开始/////////
        var self = tableData.getUserWithUserId(selfdwUserID).wChairID
        if(self == cmdBaseWorker.wCurrentUser)
            playNode.setCardEnableColorAfterPiaoHua(self, playNode.handMajiangs4D[0], 0)
    ...
    onCMD_OperateResult:function() 
    {
        ...
        playNode.timer.switchTimer(cmdBaseWorker.wCurrentUser==INVALID_WORD?[]:[tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wCurrentUser)])
        //下面这句，add at 0901-pm
        playNode.timer.switchTimer([tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wOperateUser)])
        ...
        if(cmdBaseWorker.cbOperateCode != WIK_NULL)
        {
            ...
            ///////////////////嘉兴规则，开始///////////////////
            //不能吃什么打什么
            if(cmdBaseWorker.cbOperateCode == WIK_LEFT || cmdBaseWorker.cbOperateCode == WIK_CENTER || cmdBaseWorker.cbOperateCode == WIK_RIGHT)
            {
                var m_self = tableData.getUserWithUserId(selfdwUserID).wChairID
                var m_cbCardData = majiangs4W4D.handMajiangs4D[0]
                cmdBaseWorker.mHandCardData = m_cbCardData
                if(cmdBaseWorker.wCurrentUser == m_self)                
                    playNode.setCardEnableColor(m_self, m_cbCardData, 0)
            }
        }
        if(cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID){
            playNode.setCardEnableColorAfterPiaoHua(cmdBaseWorker.wCurrentUser, playNode.handMajiangs4D[0], 0)
            playNode.setMagicCardEnabledColor(cmdBaseWorker.wCurrentUser, playNode.handMajiangs4D[0], 0)
        }
        ...
        if(sortedActions.length>0)
        {
            ...
            playNode.setCardEnableColor(tableData.getUserWithUserId(selfdwUserID).wChairID, cmdBaseWorker.mHandCardData, 0)
            playNode.showActionBtns(sortedActions)  //吃碰后杠 idx不确定 需要searchGangIdxs
        }

    },
    ////////////////////////// 自己添加的函数（开始） ///////////////////////
    isTwoCaiShen:function()
    {
        if(playNode.handMajiangs4D[0][0].length == 1 && playNode.handMajiangs4D[0][1])
        {
            if(playNode.handMajiangs4D[0][0][0].cardData == cmdBaseWorker.cbMagicCardData && playNode.handMajiangs4D[0][1].cardData == cmdBaseWorker.cbMagicCardData)
                return true
        }
        return false
    },
    ...
    {
        // tableNode.showPiaoHuaLabel.setPositionX(playNode.scoreTTF.getPositionX() + 65)
        tableNode.showPiaoHuaLabel.setString(cmdBaseWorker.cbSelectResult<100?'飘花':'无飘花')
    },

    //需求更改(8月16日)，为了不影响其他功能，另外加一个设置灰色的函数，只用来飘花之后的抓什么打什么
    setCardEnableColorAfterPiaoHua:function(self, m_cbCardData, m_type)
    {
        if(self == INVALID_WORD){
            cmdBaseWorker.isCHongLian = false
            return;
        }

        var hasPiaoHua = false
        for(var i=0; i<GAME_PLAYER; i++)
        {
            if(cmdBaseWorker.bthePiaoHuaUser[i] == true){
                hasPiaoHua = true
                break;
            }
        }

        if(m_cbCardData != null && hasPiaoHua)
        {
            for(var i=0; i<m_cbCardData[0].length; i++)
            {
                if(m_cbCardData[0][i].cardData != cmdBaseWorker.sendCardArray[0].cbCardData)
                    m_cbCardData[0][i].color = cc.color(125, 125, 125)

                // if(m_cbCardData[1].cardData == cmdBaseWorker.cbMagicCardData[0])
                //     m_cbCardData[1].color = cc.color(255, 255, 255) 
            }
            if(cmdBaseWorker.isCHongLian == true && cmdBaseWorker.bSendCardData != 0 && cmdBaseWorker.bSendCardData != m_cbCardData[1].cardData )
                m_cbCardData[1].color = cc.color(125, 125, 125)
        }

        //当打出牌之后，手牌除了财神都去掉深色
        if(m_cbCardData != null && m_type == 1){
            for(var i=0; i<m_cbCardData[0].length; i++){
                if(m_cbCardData[0][i].cardData != cmdBaseWorker.cbMagicCardData[0])
                    m_cbCardData[0][i].color = cc.color(255, 255, 255)
            }
        }
        cmdBaseWorker.isCHongLian = false
    },
    //需求更改(8月16日)，为了不影响其他功能，重新写一个函数，只用来飘花之后的抓什么打什么
    allowToOutCurrentCardAfterPiaoHua:function(majiang)
    {
        if(cmdBaseWorker.sendCardArray[0].cbCardData == majiang.cardData){
            return true
        }else{
            return false
        }
    ...
    //需求更改(8月22日)，为了不影响其他功能，重新写一个函数，只用来给财神设置颜色
    setMagicCardEnabledColor:function(self, m_cbCardData, mtype)
    {
        if(self == INVALID_WORD)
            return;

        var data1 = cmdBaseWorker.cbMagicCardData[0]
        var canOutCard = playNode.allowToOutMagicCard(self)

        if(m_cbCardData != null)
        {    
            if(m_cbCardData[1]!=null && m_cbCardData[1].cardData == data1){
                m_cbCardData[1].color = cc.color(125, 125, 125)
                if(canOutCard)
                    m_cbCardData[1].color = cc.color(255, 255, 255)
            }
            ...
            for(var i=0; i<m_cbCardData[0].length; i++)
            {
                if(m_cbCardData[0][i].cardData == data1){
                    m_cbCardData[0][i].color = cc.color(125, 125, 125)
                    if(canOutCard)
                        m_cbCardData[0][i].color = cc.color(255, 255, 255)
                }
    ...
    //需求更改(8月22日),如果允许打出财神，则不设置深色(包括飘花之后的抓什么打什么)
    allowToOutMagicCard:function(self)
    {
        if(self == INVALID_WORD)
            return false;

        if(cmdBaseWorker.wCurrentUser == self)
        {
            if(cmdBaseWorker.wCanPiaoHuaUser == self)
                return true;

            var hasPiaoHua = false
            for(var m=0; m<GAME_PLAYER; m++)
            {
                if(cmdBaseWorker.bthePiaoHuaUser[m] == true){
                    hasPiaoHua = true
                    break;
                }
            }

            if(cmdBaseWorker.sendCardArray[0].cbCardData == cmdBaseWorker.cbMagicCardData[0] && hasPiaoHua)
                return true;
        }
        return false;
    ...
    //不能吃什么打什么，设置灰色
    setCardEnableColor:function(self, m_cbCardData, mtype)
    {
        if(self == INVALID_WORD)
            return;
        ...
        //剩余两张的时候，不设置颜色
        if(m_cbCardData != null){
            if(m_cbCardData[1]!=null && m_cbCardData[0].length==1)
                return
        }      

        var canOutCard = playNode.allowToOutCurrentCard(self)
        var data1 = cmdBaseWorker.cbForbidOutData[self][0]
        var data2 = cmdBaseWorker.cbForbidOutData[self][1]

        if(m_cbCardData != null)
        {    
            if(m_cbCardData[1]!=null && (m_cbCardData[1].cardData == data1 || m_cbCardData[1].cardData == data2)){
                m_cbCardData[1].color = cc.color(125, 125, 125)
                if(mtype == 1 || canOutCard)
                    m_cbCardData[1].color = cc.color(255, 255, 255)
            }
            
            for(var i=0; i<m_cbCardData[0].length; i++)
            {
                if(m_cbCardData[0][i].cardData == data1 || m_cbCardData[0][i].cardData == data2){
                    m_cbCardData[0][i].color = cc.color(125, 125, 125)
                    if(mtype == 1 || canOutCard)
                        m_cbCardData[0][i].color = cc.color(255, 255, 255)
                }
    ...
    //如果剩余的牌全部是不能打出去的，则允许打出去
    allowToOutCurrentCard:function(self)
    {
        if(self == INVALID_WORD)
            return;

        if(playNode.handMajiangs4D[0]  != null)
        {
        
            if(playNode.handMajiangs4D[0][1]!=null && playNode.handMajiangs4D[0][0].length==1)
            {
                //只剩余两张牌的时候，飘花玩法允许打出所有牌
                if(cmdBaseWorker.cbSelectResult < 100 )
                    return true

                // //只剩余两张牌的时候，无飘花玩法允许打出所有非财神牌
                // if(playNode.handMajiangs4D[0][1].cardData != cmdBaseWorker.cbMagicCardData[0] &&  playNode.handMajiangs4D[0][0][0].cardData != cmdBaseWorker.cbMagicCardData[0])
                //     return true
            }
            ...
            if(playNode.handMajiangs4D[0][1]!=null && (playNode.handMajiangs4D[0][1].cardData == data1 || playNode.handMajiangs4D[0][1].cardData == data2))
                a ++;

            for(var i=0; i<playNode.handMajiangs4D[0][0].length; i++)
            {
                var data3 = playNode.handMajiangs4D[0][0][i].cardData
                if(data3 == data1 || data3 == data2)
                    a ++;
            }

            if(a == (playNode.handMajiangs4D[0][0].length+1))
                return true
        }
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            //剩余最后两张，并且都是财神的时候，允许打出财神           
            if(playNode.isTwoCaiShen() && cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID)
                isAllowOut = true

            //玩家满足飘花的时候，可以打出财神(最初版本)
            if(self.wChairID == cmdBaseWorker.wCanPiaoHuaUser && cmdBaseWorker.wCurrentUser == self.wChairID)
                isAllowOut = true
            
            // //飘花玩法时，财神可以随时打出(新版本)
            // if(cmdBaseWorker.cbSelectResult < 100 && cmdBaseWorker.wCurrentUser == self.wChairID) 
            //     isAllowOut = true

            //不能吃什么打什么
            if(cmdBaseWorker.cbForbidOutData && 
              (majiang.cardData == cmdBaseWorker.cbForbidOutData[self.wChairID][0] || majiang.cardData == cmdBaseWorker.cbForbidOutData[self.wChairID][1]) && 
               cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID)
            {
                isAllowOut = false
                //如果剩余的牌全部是不能打出去的，则允许打出去
                if(playNode.allowToOutCurrentCard(self.wChairID))
                    isAllowOut = true

                //无飘花玩法，剩余最后两张，财神不允许打出
                if(cmdBaseWorker.cbSelectResult>=100 && playNode.handMajiangs4D[0][1]!=null && playNode.handMajiangs4D[0][0].length==1)
                    isAllowOut = !(!MAGIC_CARD_ALLOWOUT && cmdBaseWorker.isMagicCard(majiang.cardData, cmdBaseWorker.cbMagicCardData) )

            }
            //飘花玩法，当有玩家飘花的时候，其他玩家只能抓什么打什么
            if(cmdBaseWorker.cbSelectResult < 100 && cmdBaseWorker.wCurrentUser == self.wChairID)
            {
                var hasPiaoHua = false
                for(var m=0; m<GAME_PLAYER; m++)
                ...
                if(hasPiaoHua && !cmdBaseWorker.bthePiaoHuaUser[self.wChairID])
                    isAllowOut = playNode.allowToOutCurrentCardAfterPiaoHua(majiang)
            ...
            if(isAllowOut)
            {
                //当牌打出去之后，把设置的灰色去掉
                if(cmdBaseWorker.cbForbidOutData){
                    playNode.setCardEnableColor(self.wChairID, playNode.handMajiangs4D[0], 1)
                    playNode.setCardEnableColorAfterPiaoHua(self.wChairID, playNode.handMajiangs4D[0], 1)
                }
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var m_wChairID=0; m_wChairID<GAME_PLAYER; m_wChairID++)
        {
            var weaveItems = cmdBaseWorker.WeaveItemArray[m_wChairID]
            for(var groupIdx=0; groupIdx<weaveItems.length; groupIdx++)
            {
                var weaveItem = weaveItems[groupIdx]
                if(weaveItem.cbWeaveKind == WIK_NULL)
        ...
        //////////////////嘉兴规则，开始////////////////////
        var m_piaoCOuntLabel = new cc.LabelTTF("", "Arial", 18) 
        var m_gangCountLabel = new cc.LabelTTF("", "Arial", 18) 

        var noPiaoHuaType = true 
        var noPiaoOrGangType = true;
        //////////////////嘉兴规则，结束////////////////////
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            for (var i = 0; i < map_mask2Name.length; i++) 
            {
                ...
                if (cmdBaseWorker.dwChiHuRight[wChairID] & chr_type)
                {
                    ...
                    ///////////////嘉兴规则，开始////////////////
                    if(map_mask2Name[i][1] == '飘花' && cmdBaseWorker.cbPiaoHuaCount == 2)
                    {
                        var add_str = '两个'
                        map_mask2Name[i][1] = add_str + map_mask2Name[i][1]
                        noPiaoHuaType = false
                    }

                    if((map_mask2Name[i][1] == '杠飘' || map_mask2Name[i][1] == '飘杠') && cmdBaseWorker.cbPiaoHuaCount == 2)
                        noPiaoOrGangType = false;

                    ///////////////嘉兴规则，结束////////////////

                    chrStr += map_mask2Name[i][1] + ' ';

                    if(map_mask2Name[i][1] == '两个飘花') //此句是嘉兴规则
                        map_mask2Name[i][1] = '飘花'
                } 
            }
            ///////////////嘉兴规则，开始////////////////
            if(noPiaoHuaType && !noPiaoOrGangType && cmdBaseWorker.cbPiaoHuaCount == 2 && wChairID == cmdBaseWorker.wWinner) 
                chrStr += '一个飘花 '

            if (cmdBaseWorker.clMoreScore[wChairID] == 4 && wChairID != cmdBaseWorker.wWinner)
                chrStr += '抢杠胡 ';

            if (cmdBaseWorker.clMoreScore[wChairID] == 10 && wChairID != cmdBaseWorker.wWinner)
                chrStr += '放炮 ';

            if(cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU && cmdBaseWorker.cbLiujuCount != 0)                
                chrStr += '横翻 '
            ///////////////嘉兴规则，结束////////////////
            ...
            majiangsNode.x = 100
            majiangsNode.y = gendBar.height - 10 - 0.5*(majiangFactory.down_handHeight*majiangFactory.scale_upDown*majiangsNode.scale)
            ...
            if(wChairID==cmdBaseWorker.wExitUser)
                ...
                //////////////////// 嘉兴规则，开始 ////////////////////
                var m_gangCountString = cmdBaseWorker.cbGangCount == 0? "":('连续开杠' + cmdBaseWorker.cbGangCount + '次')
                m_gangCountLabel.setString(m_gangCountString)               
                m_gangCountLabel.setPosition(cc.p(gendBar.getContentSize().width*0.80, gendBar.getContentSize().height*0.23))
                m_gangCountLabel.setVisible(true)

                // if(cmdBaseWorker.cbSelectResult<100){
                //     var m_piaoCOuntString = cmdBaseWorker.cbPiaoHuaCount == 0? "":('飘花' + cmdBaseWorker.cbPiaoHuaCount + '个')
                //     m_piaoCOuntLabel.setString(m_piaoCOuntString)
                //     m_piaoCOuntLabel.setPosition(cc.p(gendBar.getContentSize().width*0.7, gendBar.getContentSize().height*0.23))
                //     m_piaoCOuntLabel.setVisible(true)

                //     if(m_gangCountString == "")
                //         m_piaoCOuntLabel.setPosition(cc.p(gendBar.getContentSize().width*0.83, gendBar.getContentSize().height*0.23))

                // }

                //gendBar.addChild(m_piaoCOuntLabel)
                gendBar.addChild(m_gangCountLabel)
                //////////////////// 嘉兴规则，结束 ////////////////////
    ...
    //         // 一炮多响，胡牌玩家都要显示“胡”
    //         if(cmdBaseWorker.clMoreScore[i] != 0 && i!=cmdBaseWorker.wWinner) 
    //         {
    //             control['winflag'+i].setVisible(true)
    //             control['winflag'+i].setSpriteFrame('winFlag_' + 7 + '.png')                
    //         } 
    ...
    //         }
    //     }
    //     // control.piaoHuaCountLabel.setVisible(cmdBaseWorker.cbPiaoHuaCount != 0)
    //     // control.piaoHuaCountLabel.setString('飘花' + cmdBaseWorker.cbPiaoHuaCount + '个')
    //     // control.gangCountLabel.setVisible(cmdBaseWorker.cbGangCount != 0)
    //     // control.gangCountLabel.setString('连续开杠' + cmdBaseWorker.cbGangCount + '次')
    ...
    showLaizi:function()
    {
        ...
        //////////////////嘉兴规则///////////////////
        if(cmdBaseWorker.cbSiceCount != null)
        {
            var siceSpr1 = new cc.Sprite('#dian_' + cmdBaseWorker.cbSiceCount[2] + '.png')
            var siceSpr2 = new cc.Sprite('#dian_' + cmdBaseWorker.cbSiceCount[3] + '.png')
            siceSpr2.setPositionX(siceSpr1.getPositionX() + siceSpr1.getContentSize().width*0.83)
            playNode.DiceNode.addChild(siceSpr1)
            playNode.DiceNode.addChild(siceSpr2)
        }
    ...
    hideLaizi:function()
    {
        ...
        //////////////////嘉兴规则///////////////////
        playNode.DiceNode.removeAllChildren()
    },
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        else if(cmdBaseWorker.endType == 3)
        {
            ...
            for(var i=0; i<GAME_PLAYER; i++)
            {
                if(cmdBaseWorker.clMoreScore[i] != 0 && i != winner)
                    playNode.playAnimationWithDirection('hu', tableData.getShowChairIdWithServerChairId(i))
            }
        }
    },
    /////other ui end////////
