...
{   
    ...
    {   
      ...
      // socket.sendMessage(MDM_GR_LOGON, SUB_GR_UPDATE_LOCATION, UpdateLocation)
        ...
        var mj = majiangFactory.getOne(0, 0, 0)
        playNode.haiDiCardNode.addChild(mj, 0, 1)
    ...
    {   
        ...
        playNode.kaiGangOutCall = function()
        {
            ...
            // for(var i=0;i<2;i++)
            // {
            //     var mj = playNode.kaiGangCardNode.getChildByTag(i)
            //     if(mj)
            //       cbOutCardDatas[cbOutCardDatas.length] = mj.cardData
            // }

            // var OutCard = getObjWithStructName('CMD_C_OutCard_KaiGang')
            // OutCard.cbOutCardData = cbOutCardDatas
            socket.sendMessage(MDM_GF_GAME,SUB_C_OUT_CARD_KAIGANG)


            playNode.kaiGangNode.setVisible(false)
            playNode.kaiGangCardNode.removeAllChildren()
            playNode.kaiGangHuBtn.setVisible(false)
            playNode.kaiGangOutBtn.setVisible(false)
        }


        playNode.kaiGangHuCall = function()
        {
        ...
        ///////
        playNode.haiDiYaoCall = function()
        {
            var HAIDI_NOTIFY = getObjWithStructName('CMD_C_HAIDI_NOTIFY')
            HAIDI_NOTIFY.bIsYao = true
            socket.sendMessage(MDM_GF_GAME,SUB_C_HAIDI_NOTIFY,HAIDI_NOTIFY)

            playNode.haiDiNode.setVisible(false)
        }

        playNode.haiDiBuYaoCall = function()
        {
            var HAIDI_NOTIFY = getObjWithStructName('CMD_C_HAIDI_NOTIFY')
            HAIDI_NOTIFY.bIsYao = false
            socket.sendMessage(MDM_GF_GAME,SUB_C_HAIDI_NOTIFY,HAIDI_NOTIFY)
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        if(cmdBaseWorker.wOutCardUser!=INVALID_WORD)
        {
            ...
            if(cmdBaseWorker.cbKaiGangCardDatas[0]!=INVALID_CARD_DATA)
                playNode.setCurrentDiscardMj(cmdBaseWorker.cbKaiGangCardDatas[0], d)
            else
        ...
        {
            if(cmdBaseWorker.cbKaiGangCardDatas[0]!=INVALID_CARD_DATA&&self.wChairID==cmdBaseWorker.wCurrentUser)
            {
                playNode.kaiGangNode.setVisible(true)

                var cardDatas = []
                for(var i=0;i<2;i++)
                {
                    if(cmdBaseWorker.cbKaiGangCardDatas[i] == HAS_DISPATCH_CARD_DATA)//丢弃牌
                        continue
                    cardDatas[cardDatas.length] = cmdBaseWorker.cbKaiGangCardDatas[i]
                }

                playNode.kaiGangOutBtn.setVisible(true)
                playNode.kaiGangCardNode.removeAllChildren()
                if(cardDatas.length == 1)
                {
                    var mj = majiangFactory.getOne(cardDatas[0], 0, 0)
                    playNode.kaiGangCardNode.addChild(mj, 0, 1)
                }
                else
                {
                    var mj = majiangFactory.getOne(cardDatas[0], 0, 0)
                    ...
                    var mj = majiangFactory.getOne(cardDatas[1], 0, 0)
                    mj.x = 50
                    playNode.kaiGangCardNode.addChild(mj, 0, 2)
                }

                playNode.kaiGangHuBtn.setVisible(cmdBaseWorker.cbActionMask == WIK_CHI_HU)
            }
        ...
        if(cmdBaseWorker.wCurrentHaiDiUser!=INVALID_CHAIR && self.wChairID == cmdBaseWorker.wCurrentHaiDiUser)
            playNode.haiDiNode.setVisible(true)
    },
    onCMD_QISHOUHU_RESULT:function() 
    {       
        var sortedActions = playNode.getSortedActionsWithMask(cmdBaseWorker.cbActionMask)
        if(sortedActions.length>0)
            playNode.showActionBtns(sortedActions)

        if(cmdBaseWorker.cbQishouhuSiceCount[0]==0)//起手胡过
            return;
        ...
        control.gendTitle.setSpriteFrame('gendTitle4.png')

        for(var i=0;i<2;i++)
        {
            var niaoCardNode = control['niaoCardNode'+i]
            var saizi = new cc.Sprite('#gendDice'+cmdBaseWorker.cbQishouhuSiceCount[i]+'.png')
            saizi.y = 26
            niaoCardNode.addChild(saizi)
        }
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            for (var i = 0; i < map_mask2Name.length; i++) 
            {
                ...
                if (cmdBaseWorker.dwChiHuRight_qishouhu[wChairID] & chr_type)
                {
                    // if(chrStr == '')
                    //     chrStr += '胡型：'
            ...
            //手牌
            var handCardDatas = cmdBaseWorker.cbHandCardData_qishouhu[wChairID]
            for(var j=0;j<handCardDatas.length;j++)
            {
            ...
            //扎鸟 
            var zhaNiaoTTF = cc.LabelTTF.create('', "Helvetica", 14)
            zhaNiaoTTF.setFontFillColor( cc.color(244, 230, 55) )
            zhaNiaoTTF.anchorX = 0
            zhaNiaoTTF.x = 102
            zhaNiaoTTF.y = 35
            gendBar.addChild(zhaNiaoTTF, 2)

            var map = [0,0,1,2,3,0,1]
            var num = 0
            if( map[ cmdBaseWorker.cbQishouhuSiceCount[0] ] == dir)
                num++
            if( map[ cmdBaseWorker.cbQishouhuSiceCount[1] ] == dir)
                num++          
            if(num>0)
                zhaNiaoTTF.setString('扎'+num+'鸟')

            ///////////
            if(wChairID == cmdBaseWorker.wOperateUser)
            ...
            control['fenTTF'+wChairID].setString((cmdBaseWorker.qishouhuGameScore[wChairID]>0?'+':'') + cmdBaseWorker.qishouhuGameScore[wChairID]) 

        }
       
        mainScene.top.addChild(node) 
    },
    onCMD_GANGDATA:function() 
    {        
    },
    ...
    onCMD_OutCard_KaiGang:function() 
    {
        ...
        var cbKaiGangCardDatas = cmdBaseWorker.cbKaiGangCardDatas
        for(var i=0;i<cbKaiGangCardDatas.length;i++)
        {
            var outCardData = cbKaiGangCardDatas[i]
            if(outCardData!=HAS_DISPATCH_CARD_DATA)
        ...
        playNode.setCurrentDiscardMj(cbKaiGangCardDatas[0], outDir)
    ...
    {
        if(cmdBaseWorker.bIsKaiGang)
        {
            var userZhiSaizi = cmdBaseWorker.bIsSuccessKaiGang?cmdBaseWorker.wTakeCardUser:((cmdBaseWorker.wTakeCardUser-1+GAME_PLAYER)%GAME_PLAYER) 

            managerTouch.closeTouch()  
            var data = clone(cmdBaseWorker)  
            playNode.playDiceOneDirection(
                function()
                {
                    managerTouch.openTouch() 
                    if( cmdBaseWorker.bIsSuccessKaiGang )
                        playNode._onCMD_SendCardKaiGang(data)
                    else
                        playNode._onCMD_SendCard(data)
                }, cmdBaseWorker.cbKaiGangSiceCount[0], 
                cmdBaseWorker.cbKaiGangSiceCount[1], 
                tableData.getShowChairIdWithServerChairId(userZhiSaizi)
                )
        }
        else
            playNode._onCMD_SendCard(cmdBaseWorker)
    },
    _onCMD_SendCardKaiGang:function(cmdBaseWorker) //开杠会发两张牌
    {
        ...
        for(var i=0;i<2;i++)
        {
            ...
            if(item.cbCardData == HAS_DISPATCH_CARD_DATA)//丢弃牌
                continue
            cardDatas[cardDatas.length] = item.cbCardData
        }

        if(tableData.getUserWithChairId(cmdBaseWorker.wTakeCardUser).dwUserID == selfdwUserID)
        {
            playNode.kaiGangNode.setVisible(true)
    ...
    _onCMD_SendCard:function(cmdBaseWorker) 
    {
        if(cmdBaseWorker.cbOutCardCount != 0)
            managerAudio.playEffect('gameRes/sound/sendcard.mp3')
    ...
    onCMD_OperateResult:function() 
    {
        ...
        setTimeout(function()
        {
            if( cmdBaseWorker.cbOperateCode&(WIK_MINGANG|WIK_ANGANG|WIK_PENGGANG) )
            {
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var i=0;i<2;i++)
        {
            if(cmdBaseWorker.cbNiaoCardData[i] == INVALID_CARD_DATA) 
                continue
            var niaoCardNode = control['niaoCardNode'+i]
            var majiang = majiangFactory.getOne(cmdBaseWorker.cbNiaoCardData[i], 0, 0)
            majiang.x = niaoCardNode.width*0.5
            majiang.y = niaoCardNode.height*0.5
            majiang.setScaleX(niaoCardNode.width/majiang.width)
            majiang.setScaleY(niaoCardNode.height/majiang.height)
            niaoCardNode.addChild(majiang)

            var s = new cc.Sprite("#zhaniao.png")
        ...
        if(cmdBaseWorker.cbHaiDiCardData != INVALID_CARD_DATA)
        {
            var majiang = majiangFactory.getOne(cmdBaseWorker.cbHaiDiCardData, 0, 0)
            majiang.x = control.haidiCardNode.width*0.5
            majiang.y = control.haidiCardNode.height*0.5
            majiang.setScaleX(control.haidiCardNode.width/majiang.width)
            majiang.setScaleY(control.haidiCardNode.height/majiang.height)
            control.haidiCardNode.addChild(majiang)

            var s = new cc.Sprite("#haidi.png")
            var zi = majiang.getChildByTag(101)
            if(zi)
            {
                s.x = 0.5*zi.width
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            for(var j=0;j<handCardDatas.length;j++)
            {
                ...
                if(j==handCardDatas.length-1)
                {
                    if( cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU
                        && cmdBaseWorker.cbHuProvideCardData[0] != INVALID_CARD_DATA )
                    {   
                        for(var jj=0;jj<2;jj++)
                        {
                            if(cmdBaseWorker.cbHuProvideCardData[jj] == INVALID_CARD_DATA)
                                break
                            var majiang = majiangFactory.getOne(cmdBaseWorker.cbHuProvideCardData[jj], 2, 0)
                            majiang.idxInHandMajiangs = j+1+jj
                            var pos = playNode.getDisplayHandMajiangPosAndTag(handCardDatas.length+2, majiang.idxInHandMajiangs, 0, false)
                            majiang.x = 20 + startPos + pos.x
                            majiang.y = 0//pos.y
                            majiang.setScale(pos.scale)
                            // majiang.setLocalZOrder(pos.zOrder)
                            ...
                            if(jj==0 
                            && cmdBaseWorker.cbHuProvideCardData[jj] == cmdBaseWorker.cbHaiDiCardData)
                            {
                                var s = new cc.Sprite("#haidi.png")
                                var zi = majiang.getChildByTag(101)
                                if(zi)
                            ...
                            if(cmdBaseWorker.cbHuProvideCardData[jj] == cmdBaseWorker.cbKaiGangCardDatas[jj])
                            {
                                var s = new cc.Sprite("#kaigang.png")
                                var zi = majiang.getChildByTag(101)
                                if(zi)
                                {
                                    s.x = 0.5*zi.width
            ...
            var map = [0,0,1,2,3,0,1,2,3,0]
            var num = 0
            if( map[ cmdBaseWorker.cbNiaoCardData[0]%16 ] == dir)
                num++
            if( map[ cmdBaseWorker.cbNiaoCardData[1]%16 ] == dir)
                num++          
            if(num>0)
                zhaNiaoTTF.setString('扎'+num+'鸟')
            ...
            {
                if(cmdBaseWorker.cbHaiDiCardData!= INVALID_CARD_DATA)
                    control['winflag'+wChairID].setSpriteFrame('gendFlag3.png') 
                else
    ...
    getSortedActionsWithMask:function(acitonMask)
    {
        ...
        if((acitonMask&WIK_MINBU)!=0)
            actions[actions.length] = WIK_MINBU
        
        if((acitonMask&WIK_ANBU)!=0)
            actions[actions.length] = WIK_ANBU
        
        if((acitonMask&WIK_PENGBU)!=0)
            actions[actions.length] = WIK_PENGBU
        ...
        if((acitonMask&WIK_QISHOUHU)!=0)
            actions[actions.length] = WIK_QISHOUHU
    ...
    hideActionBtns:function()
    {
        ...
        playNode.btn_minbu.setVisible(false)
        playNode.btn_anbu.setVisible(false)
        playNode.btn_pengbu.setVisible(false)
        playNode.btn_ting.setVisible(false)
        playNode.btn_hu.setVisible(false)
        playNode.btn_qishouhu.setVisible(false)
    ...
    showActionBtns:function(sortedActions)
    {
        ...
        {   
            ...
            if(action == WIK_CHI_HU)
                btn = playNode.btn_hu
            else if(action == WIK_QISHOUHU)
                btn = playNode.btn_qishouhu
            ...
            else if(action == WIK_MINBU)
            {
                btn = playNode.btn_minbu
            }
            else if(action == WIK_ANBU)
            {
                btn = playNode.btn_anbu
            }
            else if(action == WIK_PENGBU)
            {
                btn = playNode.btn_pengbu
            }
    ...
    bindActionListener:function()
    {
        ...
        var actionCall_chi = function()
        {
            ...
            if(cmdBaseWorker.cbKaiGangCardDatas[0]!=INVALID_CARD_DATA)
            {
                for(var i=0;i<2;i++)
                {
                    if(i==1 && cmdBaseWorker.cbKaiGangCardDatas[1]==cmdBaseWorker.cbKaiGangCardDatas[0])
                        break
                    var cardData = cmdBaseWorker.cbKaiGangCardDatas[i]
                    if(cardData == HAS_DISPATCH_CARD_DATA)
                        continue
                    weaveItems = weaveItems.concat(gameLogic.analyseCard_Chi(handCardDatas, cardData))
                }
            }
            else
                weaveItems = gameLogic.analyseCard_Chi(handCardDatas, cmdBaseWorker.cbOutCardData) 
        ...
        var actionCall_peng = function()
        {
            ...
            if(cmdBaseWorker.cbKaiGangCardDatas[0]!=INVALID_CARD_DATA)
            {
                for(var i=0;i<2;i++)
                {
                    ...
                    if(cardData == HAS_DISPATCH_CARD_DATA)
                        continue
                    weaveItems = weaveItems.concat(gameLogic.analyseCard_Peng(handCardDatas, cardData))
                }
            }
            else
                weaveItems = gameLogic.analyseCard_Peng(handCardDatas, cmdBaseWorker.cbOutCardData) 
        ...
        var actionCall_mingang = function()
        {
            ...
            if(cmdBaseWorker.cbKaiGangCardDatas[0]!=INVALID_CARD_DATA)
            {
                for(var i=0;i<2;i++)
                {
                    ...
                    if(cardData == HAS_DISPATCH_CARD_DATA)
                        continue
                    weaveItems = weaveItems.concat(gameLogic.analyseCard_MinGang(handCardDatas, cardData))
                }
            }
            else
                weaveItems = gameLogic.analyseCard_MinGang(handCardDatas, cmdBaseWorker.cbOutCardData) 

            for(var i=weaveItems.length-1;i>=0;i--)
            {
                var isTingAfterGang = false
                for(var j=0;j<cmdBaseWorker.cbGangData.length;j++)
                {
                    if(weaveItems[i].cbValidCardDatas[0] == cmdBaseWorker.cbGangData[j])
                    {
                        isTingAfterGang = true
                        break
                    }
                }

                if(!isTingAfterGang)
                    weaveItems.splice(i,1)
            }
       ...
       var actionCall_minbu = function()
        {
            var sendAction = function(operateCardDatas)
            {
                cmdBaseWorker.sendMessage_operateCard(operateCardDatas, WIK_MINBU)
                playNode.hideActionBtns()
            }
            ...
            if(cmdBaseWorker.cbKaiGangCardDatas[0]!=INVALID_CARD_DATA)
            {
                for(var i=0;i<2;i++)
                {
                    ...
                    if(cardData == HAS_DISPATCH_CARD_DATA)
                        continue
                    weaveItems = weaveItems.concat(gameLogic.analyseCard_MinBu(handCardDatas, cardData))
                }
            }
            else
                weaveItems = gameLogic.analyseCard_MinBu(handCardDatas, cmdBaseWorker.cbOutCardData) 
        ...
        bindListener(actionCall_minbu, playNode.btn_minbu)

        var actionCall_anbu = function()
        {
            var sendAction = function(sortedOperateCardDatas)
            {
                cmdBaseWorker.sendMessage_operateCard(sortedOperateCardDatas, WIK_ANBU)
                playNode.hideActionBtns()
            }
            ...
            if(cmdBaseWorker.bPlayerStatus[tableData.getUserWithUserId(selfdwUserID).wChairID][1])//听牌状态 暗杠只会判断摸上来的牌能不能暗杠 
            {
                ...
                var weaveItem = clone(tagWeaveItem)
                weaveItem.cbValidCardDatas = [cardData, cardData, cardData, cardData]
                weaveItem.cbWeaveKind = WIK_ANBU
                weaveItems[weaveItems.length] = weaveItem
            }
            else
            {
                ...
                var direction = tableData.getShowChairIdWithServerChairId(selfChairId)
                var handCardDatas = playNode.getHandCardDatas(playNode.handMajiangs4D[direction])
                weaveItems = gameLogic.analyseCard_AnBu(handCardDatas)
            }
        ...
        bindListener(actionCall_anbu, playNode.btn_anbu)

        var actionCall_pengbu = function()
        {
            var sendAction = function(sortedOperateCardDatas)
            {
                cmdBaseWorker.sendMessage_operateCard(sortedOperateCardDatas, WIK_PENGBU)
                playNode.hideActionBtns()
            }
            ...
            if(cmdBaseWorker.bPlayerStatus[tableData.getUserWithUserId(selfdwUserID).wChairID][1])//听牌状态 暗杠只会判断摸上来的牌能不能暗杠 
            {
                var weaveItem = clone(tagWeaveItem)
                weaveItem.cbValidCardDatas = [cmdBaseWorker.cbSendCardData]
                weaveItem.cbWeaveKind = WIK_PENGBU
                weaveItems[weaveItems.length] = weaveItem
            }
            else
            {
                ...
                weaveItems = gameLogic.analyseCard_PengBu(cmdBaseWorker.WeaveItemArray[selfChairId], 
                    handCardDatas)
            }
        ...
        bindListener(actionCall_pengbu, playNode.btn_pengbu)
        ...
        var actionCall_qishouhu = function()
        {
            var btn = playNode.btn_qishouhu
            cmdBaseWorker.sendMessage_operateCard([], WIK_QISHOUHU)
            playNode.hideActionBtns()
        }
        bindListener(actionCall_qishouhu, playNode.btn_qishouhu)
    ...
    {
        if(action==WIK_MINBU)
            playNode.onActionMinBu(operateWeaveIdx, operateWeaveItem, operateUserChairId, provideUserChairId, provideCardData)
        else if(action==WIK_ANBU)
            playNode.onActionAnBu(operateWeaveIdx, operateWeaveItem, operateUserChairId, provideUserChairId, provideCardData)
        else if(action==WIK_PENGBU)
            playNode.onActionPengBu(operateWeaveIdx, operateWeaveItem, operateUserChairId, provideUserChairId, provideCardData)
        else if(action==WIK_MINGANG)
    ...
    onActionMinBu:function(operateWeaveIdx, operateWeaveItem, operateUserChairId, provideUserChairId, provideCardData)
    {
    ...
    onActionAnBu:function(operateWeaveIdx, operateWeaveItem, operateUserChairId, provideUserChairId, provideCardData)
    {
        var operateDir = tableData.getShowChairIdWithServerChairId(operateUserChairId)
        var operateUser = tableData.getUserWithChairId(operateUserChairId)
    ...
    onActionPengBu:function(operateWeaveIdx, operateWeaveItem, operateUserChairId, provideUserChairId, provideCardData)
    {
        var operateDir = tableData.getShowChairIdWithServerChairId(operateUserChairId)
        var operateUser = tableData.getUserWithChairId(operateUserChairId)
    ...
    weaveItem2Majiangs:function(operateDirection, weaveIdx, weaveItem, isPublicAnGang)
    {
        ...
        if(provideDirection!=operateDirection)
        {
            if(weaveItem.cbWeaveKind == WIK_LEFT)
                ...
                arrowIdx = 1    
            else if(weaveItem.cbWeaveKind&(WIK_MINGANG|WIK_ANGANG|WIK_PENGGANG|WIK_MINBU|WIK_ANBU|WIK_PENGBU))
                arrowIdx = 3   
        }
        ...
        for(var i=0;i<cbValidCardDatas.length;i++)
        {
            ...
            if( weaveItem.cbWeaveKind&(WIK_ANGANG|WIK_ANBU) )
            {
                if(i<3)
                    cardData = 0
    ...
    getHeapCardDatasArray:function(cbHeapCardInfo, TurnoverCard, kaiGangHeapCardInfo)
    {
        ...
        // {
        //     var wTurnoverCardHeapDir = tableData.getShowChairIdWithServerChairId(TurnoverCard[i].heapIdx.wHeapDir)
        ...
        //         heapCardDatasArray[wTurnoverCardHeapDir][wTurnoverCardHeapPos] = HAS_DISPATCH_CARD_DATA//cbTurnoverCardData
        // }


        for(var i=0;i<cmdBaseWorker.kaiGangCard.length;i++)
        {
            var kaiGangCardItem = cmdBaseWorker.kaiGangCard[i]
            for(j=0;j<2;j++)
            {
                var heap = kaiGangCardItem[j]
                var cardHeapDir = tableData.getShowChairIdWithServerChairId(heap.heapIdx.wHeapDir)
                var cardHeapPos = heap.heapIdx.wHeapPos
                var cardData = heap.cbCardData

                if(heapCardDatasArray[cardHeapDir][cardHeapPos] == 0)
                    heapCardDatasArray[cardHeapDir][cardHeapPos] = HAS_DISPATCH_CARD_DATA//cardData
            }
        }
