        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                switch(head.CommandInfo.wSubCmdID)
                {
                case SUB_S_QISHOUHU_RESULT: 
                {   
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_QishouhuResult') 
                    cmdBaseWorker.onCMD_QISHOUHU_RESULT(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_QISHOUHU_RESULT'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }     
                case SUB_S_GANGDATA: 
                {   
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_GANGDATA') 
                    cmdBaseWorker.onCMD_GANGDATA(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_GANGDATA'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }       
                case SUB_S_HAIDI_NOTIFY: 
                {   
                    cmdBaseWorker.onCMD_HAIDI_NOTIFY()

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_HAIDI_NOTIFY'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }  
                case SUB_S_TINGDATA: 
                {   
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_TINGDATA') 
                    cmdBaseWorker.onCMD_TINGDATA(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_TINGDATA'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }             
                case SUB_S_PLAYER_STATUS_UPDATA: 
                {   
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_PlayerStatusUpdata') 
                    cmdBaseWorker.onCMD_PlayerStatusUpdata(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_PlayerStatusUpdata'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }  
                ...
                case SUB_S_OUT_CARD_KAIGANG: 
                {   
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_OutCard_KaiGang') 
                    cmdBaseWorker.onCMD_OutCard_KaiGang(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_OutCard_KaiGang'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }   
                ...
                case SUB_S_REPLACE_RESULT: 
                {  
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_ReplaceResult') 
                    cmdBaseWorker.onCMD_ReplaceResult(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_ReplaceResult'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_LISTEN_RESULT: 
                {  
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_ListenResult') 
                    cmdBaseWorker.onCMD_ListenResult(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_ListenResult'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_GAME_END: 
