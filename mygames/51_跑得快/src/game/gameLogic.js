...
gameLogic.getTipsArrayForFirstOut = function(handCardIdxs)
{
    ...
    for(var i=1;i<5;i++)
    {
        if(handCardIdxs[i] == 51)
            beginIdxs.splice(0, 0, 51)
...
var cardType =  
{
    ...
    parser:function(c_sortedIdx54sExcludeLazi)
    {
        ...
        for(var i=3;i<15;i++)
        {
            var num = i==14?1:i
            var someCardNums_sorted = [num, num, num]
            var isHas = cardLogic.isHas(c_sortedIdx54sExcludeLazi, [], someCardNums_sorted)
            var idxs = isHas[0]
            if(idxs)
            {
        ...
        {   
            ...
            else
            {
                ...
                {
                    currentContinueLen = currentContinueLen + 1
                    maxNum = numsHas3[i]
                }
                else
                    currentContinueLen = 1
            ...
            maxContinueLen = Math.max(currentContinueLen, maxContinueLen)
        }
        ...
        if(c_sortedIdx54sExcludeLazi.length<=maxContinueLen*5)
        {
            ...
            return {id:this.id, idxs:san.concat(yi), typeIdx:this.typeIdx, typeLevel:this.typeLevel, typeScores:0,typeKind:this.typeKind} 
        }
...
[ 
    ...
    [
        ...
        {id:2, minLen:2},
        {id:3, minLen:1, maxLen:1, minCardNum:1, maxCardNum:13, type:3},
        {id:3, type:3},
    ], 
    [
        {id:4},
