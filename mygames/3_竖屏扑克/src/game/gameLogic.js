...
gameLogic.isSpecialType = function(cardDatas)
{
    ...
    if(tonghuashunArray.length == 1 && tonghuashunArray[0].length == 13 )
        specialType = special_tonghuashisanshui
    else if(shunziArray.length == 1 && shunziArray[0].length == 13 )
        specialType = special_shisanshui
    else if( tonghuashunArray.length == 2 || (tonghuashunArray.length==1&&tonghuashunArray[0].length>=10) )
    {
        specialType = special_2tonghuashun

        if(tonghuashunArray.length == 2)
        {
            centerCardDatas = tonghuashunArray[0]
            tailCardDatas = tonghuashunArray[1]
        }
        else
        {
    ...
    else if(sizhangArray.length == 1 && tonghuashunArray.length == 1)
    {
    ...
    else if( santiaoArray.length == 4 || (santiaoArray.length == 3&&sizhangArray.length == 1) )
        specialType = special_4santiao
    else if(sizhangArray.length*2 + duiziArray.length == 6)
        specialType = special_liuduiban
    else if( (colorLen0==0 || colorLen0==3 || colorLen0==5 || colorLen0==8 || colorLen0==10)
    ...
    else if(shunziArray.length == 2 || (shunziArray.length==1&&shunziArray[0].length>=10))
    {
        ...
        for(var i=0;i<combination_centerAndTail.length;i++)
        {
            ...
            &&  cardLogic.getNum(differ[1]) - cardLogic.getNum(differ[2]) == -1 )
            {
...
gameLogic.getScore = function(type, sortedCardDatas, param1, param2)
{
    ...
    if(type==level_duizi)
    {
        ...
        var colorDuiz = cardLogic.getColor(param1[1])
        score = scoreDuiz*Math.pow(13,6)*4 + score_0*Math.pow(13,5)*4 + score_1*Math.pow(13,4)*4 
        + score_2*Math.pow(13,3)*4 + score_3*Math.pow(13,2)*4 
        + score_4*Math.pow(13,1)*4 + colorDuiz
    }

    if(type==level_liangdui)
    {
        //分别比较大对子score 小对子score score_0,score_1,score_2,score_3,score_4, 大对子color 
        var scoreDuiz1 = gameLogic.num2Scores[ cardLogic.getNum(param1[1]) ]
        var scoreDuiz2 = gameLogic.num2Scores[ cardLogic.getNum(param2[1]) ]
        var colorDuiz2 = cardLogic.getColor(param2[1])
        score = scoreDuiz2*Math.pow(13,7)*4 + scoreDuiz1*Math.pow(13,6)*4 + score_0*Math.pow(13,5)*4 + score_1*Math.pow(13,4)*4 
        + score_2*Math.pow(13,3)*4 + score_3*Math.pow(13,2)*4 
        + score_4*Math.pow(13,1)*4 + colorDuiz2
    }
...
gameLogic.analyseCardDatas = function(sortedCardDatas)
{
    ...
    for(var i=1;i<sortedCardDatas.length;i++)
    {
        ...
        ///////
        var hasInsert = false
        for(var ii=0;ii<plus1NumArray.length;ii++)
        {
