...
var MAGIC_COLOR = cc.color(255, 255, 255);
           	...
           	onTouchBegan: function (touch, event) {
				//outMahJong.hideOutCard();
       	        ...
       	        for (var i = 0; i < MAX_COUNT; i++) {
            		...
            		if(handMahJong.cbCantEatCard[0] == handMahJong.control[i].cbCardData && handMahJong.cbCardCount>3)continue;
            		if(handMahJong.cbCantEatCard[1] == handMahJong.control[i].cbCardData && handMahJong.cbCardCount>3)continue;
...
var weaveControl = 
{
	create:function(wChaird, cbCardData)
	{
		...
		else
		{
			...
			item.baopi = new cc.Sprite("#laizi.png");
			item.addChild(item.baopi);
			item.baopi.x = 35;
			item.baopi.y = 38;
			item.baopi.visible = false;
		}
		return item;
	},
	init:function(uiPlay)
	...
	setCardData:function(wChaird, cbWeaveIndex)
	{
		...
		else if(cbWeaveKind & WIK_CENTER ||cbWeaveKind &WIK_LEFT||cbWeaveKind &WIK_RIGHT) 
		{
			var nIndex = 1;
			cbCardData.sort(function(a, b){return a-b;});
			if (cbCardData[0] == 0)
