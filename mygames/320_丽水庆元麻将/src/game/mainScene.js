...
var mainScene = {
  ...
  sortHandCard:function(cbCardData)
  {
      var sortBP = function(cbCardData)
      {
        ...
        else if(cbCardData == 0x37 && !handMahJong.isMagicCard(0x37))
        {
            //白板显示在 癞子牌之 的位置
            return handMahJong.cbMagicCard[0];
        }
        return cbCardData;
      };
  ...
  onEventScenePlay:function(data)
  {
      ...
      handMahJong.cbMagicCard    = data.cbMagicCard;

      tableNode.spriteLaizi1.visible = true
      tableNode.laizipiNode1.addChild(weaveControl.create(0, handMahJong.cbMagicCard[0]));
      ...
      for (var i = 0; i < GAME_PLAYER; i++) {
          ...
          weaveControl.cbWeaveArray[wViewChairID] = data.WeaveItemArray[i];
           gameLog.log("组合数量"+data.cbWeaveCount[i]);
  ...
  //游戏开始
	onSubGameStart:function(data)
	{
      ...
      mainScene._cbMagicIndex     = data.cbMagicIndex[0];

      handMahJong.cbMagicCard[0] = gameLogic.switchToCardData(mainScene._cbMagicIndex);

      mainScene._szNickName       = ["","","",""];
      mainScene._bMustBuHua       = data.bMustBuHua;
      mainScene._cbSice1          = data.cbSice1[0];
      mainScene._cbSice2          = data.cbSice2[0];
      mainScene._cbSice3          = data.cbSice1[1];
      mainScene._cbSice4          = data.cbSice2[1];
      ...
      mainScene.playSaiZi(mainScene._cbSice1,mainScene._cbSice2);
      mainScene.scene.scheduleOnce(mainScene.playSecondSaiZi,1.5);
      mainScene.scene.scheduleOnce(mainScene.setUserMahJong, 3.5);
      return true;
	},
  //补花 多张
  ...
  onSubOutCard:function(data)
  {
      ...
      if (outUser.cbGender)
        managerAudio.playEffect('gameRes/sound/man/cardData'+mainScene._cbOutCardData+'.wav');
      else
  ...
  onSubSendCardEx:function()
  {
      ...
      if(mainScene._cbLeftCardCount<=34+4)
      {
          //播放动画
           operateAmt.setAmtInfo(mainScene._wMeChaird, WIK_BAO_TIP);
      }
  ...
  //飘财提示
  onSubPiaoCai:function(data)
  {
      if(data.bPiaoCai)
      {
         operateAmt.setAmtInfo(data.wChairID,WIK_PIAO_CAI);
      }
  },

  playSaiZi:function(value1,value2)
  {
      //播放骰子音效
      managerAudio.playEffect('gameRes/sound/dice.wav');
      //播放骰子动画
      var spr = actionFactory.getSprWithAnimate('shaizi_anmi', true, 0.07, function()
      {   
         mainScene.sice1 = new cc.Sprite("#SiceOne_" + value1 + '.png');
         mainScene.sice2 = new cc.Sprite("#SiceTwo_" + value2 + '.png');
