var gameEnd = 
{
	...
	setChiHuResult:function(wWinner)
	{
		...
		for (var i = 0; i < 19; i++) 
		{
			...
			{
				chrStr += chr_str[i];
				if(chr_type[i] != CHR_ZIMO && chr_type[i] != CHR_DAN_DIAO)
				{
					...
					{
						if (user.cbGender)
		          		managerAudio.playEffect('gameRes/sound/man/'+arrayBoFan[iReadyBoFanNum]+'.wav');
		        		else
		          		managerAudio.playEffect('gameRes/sound/woman/'+arrayBoFan[iReadyBoFanNum]+'.wav');
		          		iReadyBoFanNum++;
					}
