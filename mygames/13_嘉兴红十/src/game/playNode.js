...
{   
    ...
    onCMD_CallNotify:function()
    {
        ...
        {
            if( gameLogic.getRed10Count(cmdBaseWorker.cbHandCardData[ self.wChairID ]) > 0 )
                playNode.popCallNode(true)   
        }
...
// ]
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var wChairID=0;wChairID<GAME_PLAYER;wChairID++)
        {
            ...
            if(cmdBaseWorker.cbCallRecord[wChairID] > 0)
            {
                var resultTTF = cc.LabelTTF.create('', "Helvetica", 16)
                resultTTF.setFontFillColor( cc.color(244, 230, 159) )
                resultTTF.setString('亮10 x ' + cmdBaseWorker.cbCallRecord[wChairID])
                resultTTF.anchorX = 0
                resultTTF.x = 102
                resultTTF.y = 25
                gendBar.addChild(resultTTF)
    ...
    _initCallCall:function()
    {
        ...
        playNode.callCall4 = function(){ callCard(gameLogic.getRed10Count(cmdBaseWorker.cbHandCardData[ tableData.getUserWithUserId(selfdwUserID).wChairID ])) }
    },
    ...
    getSoundName:function(cardsType)
    {
        var name = cardsType.name
                        console.log(44444, name, cardsType)
