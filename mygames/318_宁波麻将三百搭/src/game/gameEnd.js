var gameEnd = 
{
	setGameEndInfo:function(data)
	{
		...
		for (var i = 0; i < GAME_PLAYER; i++) {
			...
			else if (data.lGameScore[i] < 0)
			{
				...
				if (data.dwChiHuRight & CHR_SONG_GANG && data.wSongGangID == i)
				{
	...
	setGameResult:function(wWinner)
	{
		...
		else
		{
			...
			for (var i = 0; i < CHR_MAX_COUNT; i++) {
				//暂时CCB只做了11个牌型LABEL
				if (gameEnd.result.dwChiHuRight & chr_type[i] && nType < 11)
				{

					if (i == CHR_MAX_COUNT-3)
					{
						//中发白
						gameEnd['type'+nType].setString(chr_str[i]+gameEnd.result.cbZhongFaBai[wWinner]+'台');
					}
					else if (i == CHR_MAX_COUNT-2)
					{
						//正花
						gameEnd['type'+nType].setString(chr_str[i]+gameEnd.result.cbZhengHua[wWinner]*2+'台');
					}
					else if (i == CHR_MAX_COUNT-1)
					{
						//野花
	...
	setCardData:function(wWinner)
	{
		...
		mainScene.sortHandCard(gameEnd.result.cbCardData[wWinner]);
