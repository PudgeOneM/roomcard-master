...
var mainScene = {
  ...
  initUiPlayCCB:function()
  {
      ...
      var l = cc.EventListener.create({
          ...
          {   
              ...
              listenMahJong.init(mainScene.uiPlay);   //初始化听口界面
  ...
  gameSetInfo:function(data)
  {
      ...
      gameLog.log("gameSetInfo:"+data.cbRule);
      gameLog.log("lCellScore:"+data.lCellScore);
      mainScene._cbRule = data.cbRule;
      gameLog.log("房间规则1:"+mainScene._cbRule);
      if(mainScene._cbRule&RULE_FENG_HAO_ZI)
        tableNode.labelRule.setString("抓耗子玩法");
      else  if(mainScene._cbRule&RULE_WU_FENG)
        tableNode.labelRule.setString("不抓玩法");
  ...
  {
      topUI.callFengHao = function()
      {
          //发送抓耗子消息
          var operateMsg = getObjWithStructName('CMD_C_RuleSet');
          operateMsg.cbRule = RULE_FENG_HAO_ZI;
          socket.sendMessage(MDM_GF_GAME, SUB_C_RULE_SET, operateMsg); 
          topUI.ruleNode.visible = false
      }
      topUI.callWuFeng = function()
      {
          //发送不抓消息
          var operateMsg = getObjWithStructName('CMD_C_RuleSet');
          operateMsg.cbRule = RULE_WU_FENG;
          socket.sendMessage(MDM_GF_GAME, SUB_C_RULE_SET, operateMsg); 
          topUI.ruleNode.visible = false
      }
  },
  testCode:function()
  {
      ...
      for (var i = 0; i < GAME_PLAYER; i++) {
          mainScene._szNickName[i] = '山西抠点麻将'+i;
         // for (var j = 0; j < 17; j++) {
  ...
  onEventScenePlay:function(data)
  {
      ...
      mainScene.scene.unschedule(mainScene.updateTableUser);
      //设置变量 
      mainScene._cbGangCardData  = data.cbGangCardData; //指定可杠的牌
      mainScene._bIsListened     = (data.cbListenStatus[gameLogic.getMeChaird()] & WIK_LISTEN);
      mainScene._cbTokenCard     = data.cbTokenCard;
      ...
      handMahJong.cbEnjoinGang  =  data.cbEnjoinGang;
      ...
      if(mainScene._cbRule & RULE_FENG_HAO_ZI)
      {
          tableNode.laiziNode.visible = true;
      ...
      if(mainScene._bIsListened)
      {
          ...
          {
              handMahJong.setCardEnableFalse();
          }
          
          //显示听口
          listenMahJong.showListenMouth(data.cbListenMouth);
      }


      if(mainScene._wBankerUser != mainScene._wMeChaird)//还在游戏中，隐藏开始等按钮
      {
          if(tableNode.startNode)
  ...
  //游戏开始
	onSubGameStart:function(data)
	{
      ...
      tableMahJong.clearCardData();
  		//设置变量
      mainScene._cbGangCardData  = 0; //服务端指定可杠的牌
  ...
  onSubOutCard:function(data)
  {
      ...
      if(data.bShowCardBack)
      {
         mainScene._cbOutCardData = 0xff;
         return true;
      }
  ...
  onSubSendCardEx:function()
  {
      ...
      if (mainScene.onSendData.wCurrentUser == mainScene._wMeChaird || (mainScene.onSendData.wCurrentUser == 0 && mainScene._wMeChaird == INVALID_CHAIR)) 
      {
          ...
          mainScene._cbGangCardData = mainScene.onSendData.cbGangCardData;
          //当前出牌用户自检是否有花 有则补花
          if (mainScene.checkBuHua()) 
          {
             handMahJong.setCardEnable(false);
  ...
  onSubOperateResult:function(pOperateResult)
  {
      ...
      else if((pOperateResult.cbOperateCode & WIK_LISTEN)!=0)
      {
          //显示玩家的听牌标志
          if(mainScene._wCurrentUser == mainScene._wMeChaird)
          {
                mainScene._bIsListened = true;
                //手牌蒙灰处理
                handMahJong.setCardEnable(false);
          }
      }
      else if (pOperateResult.cbOperateCode!=WIK_NULL)
      {
        ...
        if ((pOperateResult.cbActionMask & WIK_GANG ||pOperateResult.cbActionMask & WIK_LISTEN)&& wOperateUser == mainScene._wMeChaird)
        {
  ...
  //显示可听的牌，不可听的牌置灰且不可出牌
  onSubListenCards:function(data)
  {
      ...
      handMahJong.setCardEnableFalse();
      handMahJong.setCardColorFalse();

      for (var i = 0; i < MAX_COUNT; i++) 
      {
          if (data.cbListenCards[i] > 0)
          {
               handMahJong.setCardEnableByValue(true,data.cbListenCards[i]);
          }
      };

      listenMahJong.bIsGetListenMonth = true;
      listenMahJong.cbListenCards = data.cbListenCards
      listenMahJong.cbListenMouth = data.cbListenMouth
  ...
  {
      mainScene._cbRule = data.cbRule;
      gameLog.log("房间规则:"+mainScene._cbRule);
      if(data.cbRule&RULE_FENG_HAO_ZI)
      {
        tableNode.labelRule.visible = true;
        tableNode.labelRule.setString("抓耗子玩法");
      }
      else  if(data.cbRule&RULE_WU_FENG)
      {
        tableNode.labelRule.visible = true;
        tableNode.labelRule.setString("不抓玩法");
      }
	...
	setUserMahJong:function()
	{
      ...
      if(mainScene._cbMagicIndex!=MAX_INDEX)
      {
        tableNode.laiziNode.visible = true;
        var cbLaiZiPi = gameLogic.switchToCardData(mainScene._cbMagicIndex);
        tableNode.laizipiNode.addChild(weaveControl.create(0, cbLaiZiPi));
