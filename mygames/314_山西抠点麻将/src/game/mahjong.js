...
var handMahJong = {
    ...
    {   
        var listener = cc.EventListener.create({
           ...
           onTouchBegan: function (touch, event) {
           		...
           		var bAllIsMagic = true
           		for(var i=0;i<handMahJong.cbCardCount;i++)
           		{	
           			if(!handMahJong.isMagicCard(handMahJong.cbCardData[i]))
           			{
           				bAllIsMagic = false;
           			}
           		}	
       	        for (var i = 0; i < MAX_COUNT; i++) {
            		...
            		if(handMahJong.isMagicCard(handMahJong.control[i].cbCardData)&&!bAllIsMagic)continue
	                ...
	                if (cc.rectContainsPoint(rect, locationInNode)) {
	                    ...
	                    if(! listenMahJong.showListenMouthByCard(handMahJong.control[i].cbCardData))
                    	{
            ...
            onTouchEnded: function (touch, event) {
        	    ...
        	    for (var i = 0; i < MAX_COUNT; i++) {
	                ...
	                {
	                	//出牌
	                	if(handMahJong.control[i].y >= handMahJong.uy &&handMahJong.bCanOut)
	                	{
	                		handMahJong.control[i].bShoot = false;
	...
	{
		if(mainScene._cbMagicIndex == gameLogic.switchToCardIndex(card))
			return true;
		return false;
	},
...
var listenMahJong = 
{
	init:function(uiPlay)
	{
		listenMahJong.cbListenCards = []
		listenMahJong.cbListenMouth = [];
		listenMahJong.bIsGetListenMonth = false;
		for (var i = 0; i < MAX_COUNT; i++) 
		{
			listenMahJong.cbListenMouth[i] = []; 
		}

		listenMahJong.outCardBg = [];
		listenMahJong.outCard = [];
		for(var i=0;i< MAX_COUNT;i++)
		{
			listenMahJong.outCardBg[i] = new cc.Sprite("#outcardBg.png");
			listenMahJong.outCardBg[i].visible = false;
			listenMahJong.outCard[i] = weaveControl.create(0, 1)
			listenMahJong.outCard[i].visible = false;
			uiPlay.addChild(listenMahJong.outCardBg[i], ZORDER_MAX);
			uiPlay.addChild(listenMahJong.outCard[i], ZORDER_MAX);
		}
	...
	showListenMouthByCard:function(cbCard)
	{
		...
		//显示听口
        if(!mainScene._bIsListened && listenMahJong.bIsGetListenMonth)
		{
			var cbIndex = -1;
			for(var j =0;j<MAX_COUNT;j++)
			{
				if(listenMahJong.cbListenCards[j] == cbCard)
				{
					cbIndex = j;
					break;
				}
			...
			if(cbIndex!=-1)
			{
				listenMahJong.showListenMouth(listenMahJong.cbListenMouth[cbIndex]);
				return true;
			}
			else
			{
				listenMahJong.hideListenMouth();
				listenMahJong.setListenMonthState(false);
				return false;
			}
		}
		return false;
	...
	showListenMouth:function(cbListenMouth)
	{
		...
		var max = 8;//MAX_COUNT
		var step = mainScene.ctPosX *2 / 12;
		...
		gameLog.log("cblistenNum"+cblistenNum);
		for(var i=0;i<cblistenNum&&i<max ;i++)
		{
			gameLog.log("i"+i);
			gameLog.log("听的牌"+cbListenMouth[i]);
			listenMahJong.outCard[i].spValue.setSpriteFrame('value_'+cbListenMouth[i]+'.png');
			listenMahJong.outCardBg[i].visible = true;
			listenMahJong.outCard[i].visible = true;
			var pos ;
			...
			listenMahJong.outCardBg[i].setPosition(pos);
			listenMahJong.outCard[i].setPosition(pos);
	...
	{
		listenMahJong.bIsGetListenMonth = bEnable;
	},
};
...
var buHua = 
{
	...
	updateBuHua:function(wChaird, nNum)
	{
		...
		topUI['zhenghua'+wViewChaird].visible = false;
	},
	hideBuHua:function()
	{
		for (var i = 0; i < GAME_PLAYER; i++) {
