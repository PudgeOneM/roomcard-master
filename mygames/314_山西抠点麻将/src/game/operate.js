...
var operateBtn = 
{
	init:function(uiPlay)
	{
        ...
        //听
        operateBtn._btnTing = new ccui.Button(resp.btnTing);
        operateBtn._btnTing.addTouchEventListener(this.onTouchEvent, this);
        operateBtn._btnTing.visible = false;
        operateBtn._btnTing.y = 180;
        operateBtn._btnTing.setScale(1);
        uiPlay.addChild(operateBtn._btnTing,ZORDER_OPERATE_BTN);
	},
	setOperateInfo:function(cbAcitonMask)
	{
		...
		if ((cbAcitonMask&WIK_LISTEN)!=0)
		{
			operateBtn._btnTing.setScale(1);
			operateBtn._btnTing.visible = true;
			operateBtn._btnTing.x = posX;
			posX -= 130;
			bOperate = true;
		}
    ...
    onTouchEvent: function (render, type) {
    	...
    	else if (render == operateBtn._btnTing) 
    	{	
    		cbOperateCode = WIK_LISTEN;
    	}
		...
		else
		{
            handMahJong.bCanOut = true
            operateBtn.hideOperate();
			//if(!handMahJong.bCanOut)
			//{
				//handMahJong.bCanOut = true;
				//operateBtn.hideOperate();
				//return ;
			//}
			//else if(handMahJong.bCanOut)
			//{
				//handMahJong.bCanOut = false;
				//operateBtn._btnTing.setScale(1.5);
			//}
		}
		...
		else if (cbOperateCode &WIK_LISTEN)	
		{
			mainScene._cbActionMask = WIK_NULL;
			mainScene._cbActionCard = 0;
		...
		else
		{
			...
			if(mainScene._bIsListened && mainScene._cbGangCardData && cbInfoCount)
			{
				cbOperateCode = WIK_GANG;
				cbOperateCard[0] = mainScene._cbGangCardData;
				cbOperateCard[1] = mainScene._cbGangCardData;
				cbOperateCard[2] = mainScene._cbGangCardData;
		...
		if( bDone )
		{
			...
			mainScene._cbActionMask = WIK_NULL;
			mainScene._cbActionCard = 0;
			mainScene._cbGangCardData = 0x00;
			var operateMsg = getObjWithStructName('CMD_C_OperateCard');
			operateMsg.cbOperateCode = cbOperateCode;
			operateMsg.cbOperateCard = cbOperateCard;
			socket.sendMessage(MDM_GF_GAME, SUB_C_OPERATE_CARD, operateMsg); 
