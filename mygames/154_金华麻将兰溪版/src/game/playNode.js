...
{   
    ...
    {   
        ...
        playNode.richtext = new cc.LabelAtlas("0123456789",resp.nums4,18,25,'0')
        playNode.scoreTTF.addChild(playNode.richtext)
    ...
    onReStart:function()
    {
        ...
        playNode.LeftZi.setVisible(false)
    ...
    updateOnFree:function()
    {
        ...
        if(!tableNode.remainOpenTimeTTF2.parent)
        {
            tableNode.ControlShow.setVisible(false)
        }
    ...
    {
        console.log(tableData.createrUserID,selfdwUserID,cmdBaseWorker.bSureMoShi)
        if (tableData.createrUserID == selfdwUserID && !cmdBaseWorker.bSureMoShi) 
        {

            topUINode.button.setVisible(true)
        }
       else
            topUINode.button.setVisible(false)

        if(cmdBaseWorker.wtype != 0)
            playNode.GameMoShi.setString('闲4庄8')
    ...
    onCMD_MoShiResult:function()
    {
        if(cmdBaseWorker.wtype != 0)
            ...
            playNode.GameMoShi.setString('闲3庄6')
    ...
    {
        playNode.LeftZi.setVisible(true)
        tableNode.ControlShow.setVisible(false)
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
        playNode.richtext.string  = cmdBaseWorker.cbLeftCardCount
        playNode.showLaizi()
        ...
        if(cmdBaseWorker.ischi[cmdBaseWorker.wCurrentUser] && cmdBaseWorker.wCurrentUser != INVALID_CHAIR)
        {
           ...
           if(cmdBaseWorker.notallowout_2 != INVALID_BYTE)
           {
                ...
                playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] =  INVALID_BYTE
        }
        if(cmdBaseWorker.wtype != 0)
            playNode.GameMoShi.setString('闲4庄8')
        else
        ...
        { 
            var direction = tableData.getShowChairIdWithServerChairId(wChairID)
            if(cmdBaseWorker.cbWeaveCount[wChairID]>0 && cmdBaseWorker.bHasTing[wChairID])
            {
                ...
                {   
                    ...
                    if (group.cbWeaveKind == WIK_GANG)
                    {
                        
                        var cardData = cmdBaseWorker.WeaveItemArray[wChairID][groupIdx].cbCenterCardData;
                        playNode.changeWeaveShowValue(cardData,group,direction)
                    }
                }
            }
        }
    ...
    switchViewChairID:function(wChairID)
     {
      ...
      if(playNode.getMeChaird() == INVALID_CHAIR)
       wViewChairID = (wChairID - 0 + GAME_PLAYER) % GAME_PLAYER;
      else
       wViewChairID = (wChairID - playNode.getMeChaird() + GAME_PLAYER) % GAME_PLAYER;
      return wViewChairID;
     },
     changeWeaveShowValue:function(cardData,group,direction)
     {
        ...
        if(cardData!=0)
        {
            ...
            zi.scale = scale 
            majiangFactory.down_weaveWidth
            switch(direction)
            {
                case 0:
                {
    ...
    onCMD_OutCard:function() 
    {
        ...
        playNode.iscontrol_1[cmdBaseWorker.wOutCardUser] = INVALID_BYTE
        playNode.iscontrol_2[cmdBaseWorker.wOutCardUser] = INVALID_BYTE
        ...
        console.log(cmdBaseWorker.bHasTing[cmdBaseWorker.wOutCardUser])
        if(cmdBaseWorker.cbWeaveCount[cmdBaseWorker.wOutCardUser]>0 && cmdBaseWorker.bHasTing[cmdBaseWorker.wOutCardUser])
        {
            var direction = playNode.switchViewChairID(cmdBaseWorker.wOutCardUser);
            var weaveMajiangs = playNode.weaveMajiangs4D[direction]
            for(var groupIdx=0;groupIdx<weaveMajiangs.length;groupIdx++)
            {
                var group = weaveMajiangs[groupIdx]
                if (group.cbWeaveKind == WIK_GANG)
                {
                    ...
                    var cardData = cmdBaseWorker.WeaveItemArray[cmdBaseWorker.wOutCardUser][groupIdx].cbCenterCardData
                    console.log(group[3])
                    playNode.changeWeaveShowValue(cardData,group,direction)
                }
            }
    ...
    onCMD_OperateResult:function() 
    {
        ...
        playNode.iscontrol_1[cmdBaseWorker.wOperateUser] = INVALID_BYTE
        playNode.iscontrol_2[cmdBaseWorker.wOperateUser] = INVALID_BYTE
        playNode.timer.switchTimer(cmdBaseWorker.wCurrentUser==INVALID_WORD?[]:[tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wCurrentUser)])
        console.log(cmdBaseWorker.wChiTwo[cmdBaseWorker.wOperateUser][cmdBaseWorker.wProvideUser])
        playNode.ischi = false
        var tmp = false//用来判断有没有吃
        if(cmdBaseWorker.wChiTwo[cmdBaseWorker.wOperateUser][cmdBaseWorker.wProvideUser] == 2)
        {
            
            console.log(cmdBaseWorker.cbProvideCardData)
            if(cmdBaseWorker.cbOperateCode == WIK_RIGHT && (cmdBaseWorker.cbProvideCardData < 0x31 || cmdBaseWorker.cbProvideCardData == 0x37))
            {
                tmp = true
                playNode.iscontrol_2[cmdBaseWorker.wOperateUser] = cmdBaseWorker.cbProvideCardData - 3
            ...
            else if(cmdBaseWorker.cbOperateCode == WIK_LEFT && (cmdBaseWorker.cbProvideCardData < 0x31 || cmdBaseWorker.cbProvideCardData == 0x37))
            {
                tmp = true
                playNode.iscontrol_2[cmdBaseWorker.wOperateUser] = cmdBaseWorker.cbProvideCardData + 3
                playNode.ischi = true
            ...
            if(tmp)
                playNode.iscontrol_1[cmdBaseWorker.wOperateUser] = cmdBaseWorker.cbProvideCardData 
        }
        ...
        //console.log(operateUser.userNodeInsetChair.currentRoundNode.weaveMajiangsNode.children[0])
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            if(wChairID==cmdBaseWorker.wExitUser)
            {
                ...
                for(var i =0;i<4;i++)
                {
                    control['laozhuang'+i].setVisible(false)
                    if(i==wChairID)
                        continue
            ...
            else if(cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU)
            {
                ...
                if(cmdBaseWorker.laozhuan[wChairID] == 1 || cmdBaseWorker.laozhuan[wChairID] == 0 )
                    control['laozhuang'+wChairID].setString("老庄")
                else
                    control['laozhuang'+wChairID].setString(tmp[cmdBaseWorker.laozhuan[wChairID]]+"老庄")
                control['laozhuang'+wChairID].setVisible(true)
            }
            else if(wChairID==cmdBaseWorker.wProvideUser)
            {
                
                control['laozhuang'+wChairID].setVisible(false)
    ...
    showLaizi:function()
    {
        for(var i=0;i<TURNOVER_COUNT_MAGIC;i++)
        {
            ...
            mj.x = 50*i + 25
            mj.y = - 30
            mj.setScaleX((bg.width/mj.width*0.8)*1.5)
            mj.setScaleY((bg.height/mj.height*0.8)*1.5)
