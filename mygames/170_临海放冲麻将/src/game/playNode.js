...
{   
    ...
    onCMD_OutCard:function() 
    {
        ...
        if(isRecordScene)
        {
            ...
            var selfChairId = tableData.getServerChairIdWithShowChairId(selfDir)
            var sortedActions = majiangLogic.getSortedActionsWithMask(cmdBaseWorker.cbReActionMask[selfChairId])
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            else if(cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU)
            {
                ...
                //{
                    ...
                    for(var j=0;j<cbGangScorecard.length;j++)
                    {
                        ...
                        majiang.x =  pos.x//startPos + pos.x
                        majiang.y = 0//pos.y
                        majiang.setScale(pos.scale)
            ...
            else if(wChairID==cmdBaseWorker.wProvideUser && cmdBaseWorker.endType ==3)
            {
                ...
                TTF.anchorX = 0
                TTF.x = 102
                TTF.y = 25//gendBar.height - 74 - 0.5*(majiangFactory.down_handHeight*majiangFactory.scale_upDown*gangcardNode.scale)
                gendBar.addChild(TTF)
    ...
    weaveItem2Majiangs:function(groupIdx, direction, weaveItem, isSelf, isPublicAnGang)
    {
        ...
        for(var idxInGroup=0;idxInGroup<cardDatas.length;idxInGroup++)
        {
            ...
            if(  weaveItem.cbPublicCard != false && !hasAddDirectionSpr && cardData == weaveItem.cbCenterCardData)//weaveItem.provideDirection!=direction && 
            {
                ...
                var dict = weaveItem.provideDirection//tableData.getServerChairIdWithShowChairId(weaveItem.provideDirection)
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        else if(cmdBaseWorker.endType == 2)
        {
            ...
            playNode.playGenderEffect(cmdBaseWorker.whukind[cmdBaseWorker.wFristWinner], tableData.getUserWithChairId(cmdBaseWorker.wProvideUser).cbGender)
        }
        else if(cmdBaseWorker.endType == 3)
        {
            ...
            for(var i=0;i<GAME_PLAYER;i++)
            {
                if(cmdBaseWorker.dwChiHuKind[i] == WIK_CHI_HU)
                {
                    ...
                    playNode.playActionEffect(cmdBaseWorker.whukind[i], tableData.getUserWithChairId(i).cbGender)
                }
            }
        }
