...
var special_wuduisantiao = 4 //五对三条
var special_shisanshui = 5 //十三水： 数字为一至十三的牌型！赢每家52水。
var special_tonghuashisanshui = 6 //同花十三水：同花一至十三的牌型！赢每家104水（可做可不做，只存在理论）
...
gameLogic.isSpecialType = function(cardDatas)
{
    ...
    else if((santiaoArray.length == 1) && (sizhangArray.length*2 + duiziArray.length == 5))
    ...
    else if(sizhangArray.length == 1 && tonghuashunArray.length == 1)//1炸弹1同花顺
    {
        ...
        for(var i =0;i<5;i++)
        {
            tailCardDatas[i] = tonghuashunArray[0][i]
        }
        var differ = getDiffer(cardDatas, centerCardDatas.concat(tailCardDatas))
...
gameLogic.getScore = function(type, sortedCardDatas, param1, param2)
{
    ...
    if(type==level_duizi)
    {
        ...
        score = scoreDuiz*1000000+score_0*1000+score_1*100+score_2*10
    }
