...
var mainScene = {
  ...
  onEventScenePlay:function(data)
  {
      ...
      mainScene._wPlayerPC       = data.wPlayerPC;
      mainScene._bFirstGame      = false;
      mainScene.setHuaCardData();
      for (var i = 0; i < GAME_PLAYER; i++) {
          mainScene._szNickName[i] = tableData.getUserWithChairId(i).szNickName;
      };
      //设置癞子皮
      var cbLaiZiPi = mainScene._cbHuaCard;
  ...
  //游戏开始
	onSubGameStart:function(data)
	{
      ...
      mainScene._szNickName      = ["","","",""];
      mainScene._bOutMagicCard   = data.bOutMagicCard;
      mainScene._wPlayerPC       = INVALID_CHAIR;
      mainScene._cbBankerData    = data.cbBankerData;
      mainScene.setHuaCardData();
      //设置庄家
      for (var i = 0; i < GAME_PLAYER; i++) {
          mainScene._szNickName[i] = tableData.getUserWithChairId(i).szNickName;
      ...
      //显示4张随机麻将牌背
      if (mainScene._bFirstGame)
      {
          tableNode.mahJongBanker0.addChild(weaveControl.create(0, 0));
          tableNode.mahJongBanker1.addChild(weaveControl.create(0, 0));
          tableNode.mahJongBanker2.addChild(weaveControl.create(0, 0));
          tableNode.mahJongBanker3.addChild(weaveControl.create(0, 0));
          tableNode.mahJongBanker0.visible = true;
          tableNode.mahJongBanker1.visible = true;
          tableNode.mahJongBanker2.visible = true;
          tableNode.mahJongBanker3.visible = true;
      }
  		//播放骰子动画
      ...
      {  
         ...
         mainScene.sice2 = new cc.Sprite("#SiceTwo_" + data.cbSice2 + '.png');
        ...
        if (mainScene._bFirstGame)
        {
            tableNode.mahJongBanker0.removeAllChildren();
            tableNode.mahJongBanker1.removeAllChildren();
            tableNode.mahJongBanker2.removeAllChildren();
            tableNode.mahJongBanker3.removeAllChildren();
            for (var i = 0; i < GAME_PLAYER; i++) {
                var wChairdID = gameLogic.switchLogicChairdID(i);
                var cbBankerData = mainScene._cbBankerData[wChairdID];
                tableNode['mahJongBanker'+i].addChild(weaveControl.create(0, cbBankerData));
            };
            mainScene._bFirstGame = false;
            mainScene.scene.scheduleOnce(mainScene.setUserMahJong, 2);
        }
        else
        {
  ...
  {
      //播放补花音效
      var User = tableData.getUserWithChairId(data.wReplaceUser);
      if (User.cbGender)
        managerAudio.playEffect("gameRes/sound/man/hua.wav");
      else
        managerAudio.playEffect("gameRes/sound/woman/hua.wav");
      //显示补花麻将
      var nBuHua = 0;
      ...
      if (data.wReplaceUser == mainScene._wMeChaird) 
      {
          ...
          {
              if (mainScene._wPlayerPC == INVALID_CHAIR || mainScene._wPlayerPC == mainScene._wMeChaird)
              {
                  handMahJong.setCardEnable(true);
                  //设置吃碰杠操作
                  operateBtn.setOperateInfo(mainScene._cbActionMask);
  ...
  onSubOutCard:function(data)
  {
      ...
      {
          if (outUser.cbGender)
            managerAudio.playEffect("gameRes/sound/man/piaocai.wav");
          else
            managerAudio.playEffect("gameRes/sound/woman/piaocai.wav");
      }
      else
      {
          if (outUser.cbGender)
  ...
  onSubGameGM:function(data)
  {
	...
	setUserMahJong:function()
	{
      ...
      tableNode.mahJongBanker0.visible = false;
      tableNode.mahJongBanker1.visible = false;
      tableNode.mahJongBanker2.visible = false;
      tableNode.mahJongBanker3.visible = false;
      mainScene.setBankerIcon(mainScene._wBankerUser,true);
      //设置癞子皮
      var cbLaiZiPi = mainScene._cbHuaCard;
      //var cbLaiZiPi = gameLogic.switchToCardData(mainScene._cbMagicIndex);
  ...
  checkBuHua:function()
  {
      ...
      for (var i = 0; i < handMahJong.cbCardCount; i++) {
         ...
         if (handMahJong.cbCardData[i] == 0x37) return true;
         if (handMahJong.cbCardData[i] == mainScene._cbHuaCard) return true;
      }
      return false;
  },
  ...
  setHuaCardData:function()
  {
      if (mainScene._cbMagicIndex == 0)
      {
          mainScene._cbHuaCard = gameLogic.switchToCardData(8);
      }
      else if (mainScene._cbMagicIndex == 9)
      {
          mainScene._cbHuaCard = gameLogic.switchToCardData(17);
      }
      else if (mainScene._cbMagicIndex == 18)
      {
          mainScene._cbHuaCard = gameLogic.switchToCardData(26);
      }
      else if (mainScene._cbMagicIndex == 27)
      {
          mainScene._cbHuaCard = gameLogic.switchToCardData(30);
      }
      else if (mainScene._cbMagicIndex == 31)
      {
          mainScene._cbHuaCard = gameLogic.switchToCardData(33);
      }
      else 
      {
          mainScene._cbHuaCard = gameLogic.switchToCardData(mainScene._cbMagicIndex-1);
      }
  },
