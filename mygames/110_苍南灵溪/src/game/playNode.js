...
{   
    ...
    deleteHeapMajiangs:function(heapMajiangs, wSendHeapPosArray)
    {
        ...
        {
            if( wSendHeapPosArray[i]  &&  heapMajiangs[wSendHeapPosArray[i]] )
    ...
    onCMD_SendCard:function() 
    {
        ...
        for(var i=0;i<cmdBaseWorker.cbSendCardCount;i++)
        {
            ...
            //if( heapMajiangsDir &&  playNode.heapMajiangs4D[heapMajiangsDir] && item.wHeapPos < 18 )
            playNode.deleteHeapMajiangs(playNode.heapMajiangs4D[heapMajiangsDir], [item.wHeapPos])
            //手牌
        ...
        /////摸到麻将时有可能出现杠听胡补花
        var sortedActions = majiangLogic.getSortedActionsWithMask(cmdBaseWorker.cbActionMask)
        var isPengHouGang = false;
        var self = tableData.getUserWithUserId(selfdwUserID)
        var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID
        var selfDir = tableData.getShowChairIdWithServerChairId(self.wChairID)
        var isChiPeng = cmdBaseWorker.cbBuGangStatus==1;//cmdBaseWorker.wOutCardUser != cmdBaseWorker.wProvideUser && cmdBaseWorker.wOutCardUser!=INVALID_WORD 

        var outTempUser = cmdBaseWorker.wOutCardUser
        cmdBaseWorker.wOutCardUser = INVALID_WORD
        var idxs = cmdBaseWorker.searchGangIdxs(playNode.handMajiangs4D[selfDir], playNode.weaveMajiangs4D[selfDir])
        cmdBaseWorker.wOutCardUser = outTempUser
        if( idxs && idxs.length == 1 )
        {
            var handMajiangs = playNode.handMajiangs4D[selfDir]
            var handIdxs = cmdBaseWorker.cbHandCardData[selfChairId]
            var gangCard =  idxs[0]
            var cardNum = 0
            for( var i=0;i<handIdxs.length;i++ )
            {
                if (handIdxs[i] == gangCard) 
                {
                    cardNum++;
                };
            }

            if ( cardNum<4 && isChiPeng && sortedActions.length == 1 && sortedActions[0] == WIK_GANG ) 
            {
                isPengHouGang = true;
            };
        }

        if(sortedActions.length>0 && !isPengHouGang)
    ...
    onCMD_OperateResult:function() 
    {
        ...
        cmdBaseWorker.cbBuGangStatus = 1
        var sortedActions = majiangLogic.getSortedActionsWithMask(cmdBaseWorker.cbActionMask)
        if(sortedActions.length>0)
        {
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            var isAllMagicCard = false;
            var isAllowOut2 =  cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID
            if(isAllowOut2 && cmdBaseWorker.cbHandCardCount && cmdBaseWorker.cbHandCardCount[cmdBaseWorker.wCurrentUser] ){
                isAllMagicCard = true;
                for( var i = 0;i< cmdBaseWorker.cbHandCardCount[cmdBaseWorker.wCurrentUser];i++ )  //handMajiangs[cmdBaseWorker.wCurrentUser][0].length
                {
                    var curCard = cmdBaseWorker.cbHandCardData[cmdBaseWorker.wCurrentUser][i]//handMajiangs[cmdBaseWorker.wCurrentUser][0][i].cardData
                    if( !cmdBaseWorker.isMagicCard(curCard, cmdBaseWorker.cbMagicCardData) && curCard > 0 ) 
                    {
                        isAllMagicCard = false;
                        break;
                    }
                }
                if(handMajiangs[cmdBaseWorker.wCurrentUser][1])
                {
                    var newCard = handMajiangs[cmdBaseWorker.wCurrentUser][1].cardData
                    if ( !cmdBaseWorker.isMagicCard(newCard, cmdBaseWorker.cbMagicCardData) )
                    {
                        isAllMagicCard = false
                    };
                }
            ...
            if( (isAllowOut || cmdBaseWorker.cbCanOutMagic == 1 ) && cmdBaseWorker.cbLeftCardCount >= cmdBaseWorker.cbEndLeftCount + 4)
            {
            ...
            if ( cmdBaseWorker.cbLeftCardCount < cmdBaseWorker.cbEndLeftCount + 4 && isAllowOut2 ) 
            {
                //系统发牌给下一个玩家下一个玩家
                var OutCard = getObjWithStructName('CMD_C_NEXT_CARD')
                OutCard.wCurrentUser = cmdBaseWorker.wCurrentUser;
                socket.sendMessage(MDM_GF_GAME,SUB_C_NEXT_CARD,OutCard)
                playNode.hideActionBtns()
                cmdBaseWorker.wCurrentUser = INVALID_WORD
            };
        }
    ...
    popGameEnd2:function(continueCall, szNickName_gameEnd)
    {
        ...
        for(var i = 0;i<MAX_FLOWER_COUNT;i++)
        {
          ...
          //  var oldIndex = cardIndex
            ...
            for(var i = 0;i<MAX_WEAVE;i++)
            {
                ...
                for(var j = 0;j<4;j++)
                {
                    ...
                    if (cardIdx>0) 
                    {
                        ...
                        if ((weaveItems[i].cbWeaveKind == WIK_GANG || cardIdx>0) && j == 3 ) 
                        {
                            cardIndex -=2;
                            aCard.x = midPos//150+( cardIndex+1 )*(aCard.getContentSize().width*aCard.getScale() - 2), aCard.getContentSize().height /2 *aCard.getScale()  
                            aCard.y = 45 +10
                            cardIndex++
                        }
                        else
            ...
            if (cardIndex<1 || weaveItems[0].cbCardData[0] <=0 ) {cardIndex=-1};
            cardIndex += 0.3
