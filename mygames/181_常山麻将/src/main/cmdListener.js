        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                switch(head.CommandInfo.wSubCmdID)
                { 
                case SUB_S_MORDEN_RESULT: 
                {   
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_MordenResult') 
                    cmdBaseWorker.onCMD_MordenResult(body)
                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_MordenResult'])
                    cc.eventManager.dispatchEvent(event)
                    break
                } 
                case SUB_S_TINGDATA: 
