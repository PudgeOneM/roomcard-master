...
{   
    ...
    {   
        ...
        // var Styletype = cc.sys.localStorage.getItem("Style");
        // if(Styletype)
        //     playNode.outCardStyle = Styletype
    ...
    removeHandMajiangsCard:function()
    {
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            flowerMajiangsNode.removeAllChildren()
            heapMajiangsNode.removeAllChildren()
            handMajiangsNode.removeAllChildren()
            discardMajiangsNode.removeAllChildren()
            weaveMajiangsNode.removeAllChildren()
        }
    },
    addHandMajiangsCard:function()
    {
    ...
    {   
        // playNode.outStyleChoose = function()
        // {
        //     if(playNode.outCardStyle == 0 && !playNode.isTouched)
        //     {
        //         playNode.outCardStyle = 1
        //         //cc.sys.localStorage.setItem("Style", playNode.outCardStyle);
        //         cc.eventManager.removeListener(playNode.listenerControl)
        //         playNode.removeHandMajiangsCard()
        //         playNode._getHandMajiangsGroupNode()
                
        //         playNode.addHandMajiangsCard()
        //         playNode.outStyle._titleLabel.setString("滑动出牌")
        //     }
        //     else if(!playNode.isTouched)
        //     {
        //         playNode.outCardStyle = 0
        //         //cc.sys.localStorage.setItem("Style", playNode.outCardStyle);
        //         cc.eventManager.removeListener(playNode.listenerControl)
        //         playNode.removeHandMajiangsCard()
        //         playNode._getHandMajiangsGroupNode()
        ...
        //         playNode.addHandMajiangsCard()
        //         playNode.outStyle._titleLabel.setString("单击出牌")
        //     }
        // }
        //林州规则：
    ...
    {
        if(cmdBaseWorker.wBaoPiGuType)
            tableNode.baoPiGu.setString('有')
        else
            tableNode.baoPiGu.setString('没有')
        if(cmdBaseWorker.wQiangGangType)
            tableNode.qiangGang.setString('有')
        else
            tableNode.qiangGang.setString('没有')
        if (tableData.createrUserID == selfdwUserID && cmdBaseWorker.bSureMoShi == 0) 
        {
            topUINode.chooseNode.setVisible(true)
        }
       else
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        // if(Styletype)
        //     playNode.outCardStyle = Styletype
        // if(playNode.outCardStyle == 0)
        //     playNode.outStyle._titleLabel.setString("单击出牌")
        // else
        //     playNode.outStyle._titleLabel.setString("滑动出牌")
        // playNode.outMorden.setVisible(true)
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
        ...
        else if(cmdBaseWorker.bIsAutoOutCard && self.wChairID != INVALID_WORD)
        {
            playNode.node.scheduleOnce(this.autoOutcard, 1); 
    ...
    onCMD_OperateResult:function() 
    {
        ...
        {
            var self = tableData.getUserWithUserId(selfdwUserID)
            if(cmdBaseWorker.bIsAutoOutCard && cmdBaseWorker.wOperateUser == self.wChairID)
            {
                cmdBaseWorker.wCurrentUser = INVALID_WORD
                playNode.node.scheduleOnce(this.autoOutcard, 1)
            }
        ...
        function onPlayAnimationOfGameEnd()
        {
            playNode.gameEndAction = cc.sequence( 
                ...
                {   
                    ...
                    {
                        playNode.outMorden.setVisible(false)
                        playNode.isGameEnd = false
                        playNode.isLookingResult = false   
    ...
    _gethandGroupNodeListener:function(touchEndCall)
    {
        ...
        var listener = cc.EventListener.create({
            ...
            onTouchMoved: function (touch, event) {
                ...
                var locationInNode = target.convertToNodeSpace(touch.getLocation())
                var s = target.getContentSize()
                playNode.isTouched = true
                playNode.outStyle.setVisible(false)
                var rect = cc.rect(0, 0, s.width, s.height)
                if (cc.rectContainsPoint(rect, locationInNode)) {
                    onTouch(locationInNode.x<0?0:locationInNode.x)
                    currentMajiang.ismove = true
                }
            },
            onTouchEnded: function (touch, event) 
            {
                ...
                // }
                if(currentMajiang && currentMajiang.ismove == false)
                {
                ...
                touchedMjNum = 0
                isTouchFromPop = false
                playNode.isTouched = false
                playNode.outStyle.setVisible(true)
    ...
    _gethandGroupNodeMovedListener:function(touchEndCall)
    {
        var majiangs = playNode.handMajiangs4D[0]
        var handGroupNode = playNode.handGroupNode4D[0]
        ...
        var currentPos = null
        var canOut = false
        var touchPosX2TouchedMj = function(posX,ismoved)
        {      
            if(majiangs[1])
                canOut = true
            else
            ...
            if(posX>mjWidth*majiangs[0].length)
            {
                if(!ismoved && majiangs[1])
                {
                    currentPos = majiangs[1].getPosition()
            ...
            else
            {
                var idx = Math.floor( posX/mjWidth )
                if(!ismoved)
                currentPos = majiangs[0][idx].getPosition()
                return majiangs[0][idx]
            }
        }
        ...
        {   
            ...
            else if(currentMajiang && currentMajiang!=touchedMj)//摸到新的麻将 
            {
                ...
                currentMajiang = touchedMj
                currentMajiang.y = mjHeight*0.5 + 20
                currentPos = currentMajiang.getPosition()
                var event = new cc.EventCustom("handMajiangUp")
                event.setUserData(currentMajiang.cardData)
                cc.eventManager.dispatchEvent(event) 
        ...
        var listener = cc.EventListener.create({
            ...
            onTouchBegan: function (touch, event) {
                ...
                if (cc.rectContainsPoint(rect, locationInNode)) 
                {
                    ...
                    playNode.outStyle.setVisible(false)
                    //currentMajiang = touchedMj
                    return  onTouch(locationX,false)
                }
                else
                {
                    if(currentPopMajiang)
            ...
            onTouchMoved: function (touch, event) {
                ...
                if(currentMajiang)
                {
                    ...
                    var deltaX = delta.x
                    var deltaY = delta.y
                    //console.log(deltaX,"wwwww",deltaY)
                    if(locationInNode.y + deltaY <= 150 && currentMajiang.ismove == false)
                    {
                        isMoved = false
                        var locationInNode = target.convertToNodeSpace(touch.getLocation())
                        var s = target.getContentSize()
                        var rect = cc.rect(0, 0, s.width, s.height)
                        if (cc.rectContainsPoint(rect, locationInNode)) {
                            onTouch(locationInNode.x<0?0:locationInNode.x,true)
                        }
                    }
                    //console.log(Math.abs(deltaY))
                    else if(locationInNode.y + deltaY > 150)
                    {
                        console.log( deltaY)
                        if(canOut)
                        {
                    ...
                    else if(locationInNode.y + deltaY <= 150 && currentMajiang.ismove == true)
                    {
                        currentMajiang.ismove = false
                        isMoved = false
                        currentMajiang.setPosition(locationInNode.x + deltaX,locationInNode.y + deltaY)
            ...
            onTouchEnded: function (touch, event) 
            {
                ...
                //     currentPopMajiang = currentMajiang
                // }
                if(canOut&&currentMajiang&&isMoved&&currentMajiang.touchEnable && currentMajiang.ismove)
                {
                ...
                {
                    currentMajiang.setPosition(currentPos)
                    if(currentMajiang.y == mjHeight*0.5 + 20)
                        currentMajiang.y = mjHeight*0.5
                }
                playNode.isTouched = false
                currentPos = null
        ...
        if(tableData.getUserWithUserId(selfdwUserID).wChairID == tableData.getServerChairIdWithShowChairId(0))
        {
            var touchEndCall = function(majiang)
            {
                ...
                else
                {
                    ...
                    if(isCurrentUser && majiang.touchEnable && !cmdBaseWorker.bIsAutoOutCard)
                    {
                        cmdBaseWorker.wCurrentUser = INVALID_WORD
                        var OutCard = getObjWithStructName('CMD_C_OutCard')
           ...
           //  if(playNode.outCardStyle == 0)
           //     playNode.listenerControl  = playNode._gethandGroupNodeListener(touchEndCall)
           // else
               var listener  = playNode._gethandGroupNodeMovedListener(touchEndCall)
            //console.log(playNode.listenerControl,playNode.handGroupNode4D[0])
    ...
    //             if (isRecordScene)
    //                 cardData = oldHandCardDatas[j]
    //             var majiang = playNode.getOne(cardData, 0, direction)
    ...
    {   
        ...
        console.log(handCardDatasArray)
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            for(var groupIdx=0;groupIdx<weaveItems.length;groupIdx++)
            {
                ...
                var majiangsOneGroup = playNode.weaveItem2Majiangss(0, groupIdx, weaveItem, true)
                console.log(majiangsOneGroup,"22222222222",weaveItem)
            ...
            control['taiTTF'+wChairID].setString(cmdBaseWorker.lGameTaiCount[wChairID]) 
    ...
    bindActionListener:function()
    {
        ...
        var actionCall_chi = function()
        {
            ...
            var handCardDatas = playNode.getHandCardDatas(playNode.handMajiangs4D[direction])
             console.log(cmdBaseWorker.cbMagicCardData[0])
            if(cmdBaseWorker.cbOutCardData == 55)
                cmdBaseWorker.cbOutCardData = cmdBaseWorker.cbMagicCardData[0]
            for(var i=0;i<handCardDatas.length;i++)
            {
                if(handCardDatas[i] == 55)
                    handCardDatas[i] = cmdBaseWorker.cbMagicCardData[0]
            }
    ...
    onActionMinGang:function(operateWeaveIdx, operateWeaveItem, operateUserChairId, provideUserChairId, provideCardData)
    {
        ...
        var addHandCardData = (isRecordScene||operateUser.dwUserID == selfdwUserID)?provideCardData:0
        ...
        for(var i=0;i<4;i++)
        {
            var deleteCardData = (isRecordScene||operateUser.dwUserID == selfdwUserID)?operateWeaveItem.cbValidCardDatas[i]:0
    ...
    weaveItem2Majiangss:function(operateDirection, weaveIdx, weaveItem, isPublicAnGang)
    {
        ...
        var arrowIdx = -1
        if(provideDirection!=operateDirection || playNode.isGameEnd)
        {
        ...
        for(var i=0;i<cbValidCardDatas.length;i++)
        {
            ...
            if( weaveItem.cbWeaveKind&(WIK_ANGANG) && !isRecordScene )
            {
                if(i<3)
                    cardData = 0
            ...
            if(i == arrowIdx && weaveItem.cbWeaveKind != WIK_ANGANG)
            {
                var use = wProvideUser
                if(weaveItem.cbWeaveKind&(WIK_PENGGANG))
                    use = weaveItem.wProvideUserOld
                var dire = null
                if(use == cmdBaseWorker.wBankerUser)
                    dire = 0
                else if(use == (cmdBaseWorker.wBankerUser + 1)%4)
                    dire = 1
                else if(use == (cmdBaseWorker.wBankerUser + 2)%4)
                    dire = 2
                else if(use == (cmdBaseWorker.wBankerUser + 3)%4)
                    dire = 3
                var zi = majiang.getChildByTag(101)
                var directionSpr = new cc.Sprite('#d'+dire+'.png')
                console.log(wProvideUser)
