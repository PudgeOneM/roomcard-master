...
{   
    ...
    onCMD_OutCard:function() 
    {
        ...
        playNode.showActionBtns(sortedActions)
            ...
            //var isSelf = tableData.getUserWithChairId(cmdBaseWorker.wTakeCardUser).dwUserID == selfdwUserID
            //gameLog.log(111111, tableData.getUserWithChairId(cmdBaseWorker.wTakeCardUser).dwUserID)
            // var sortedActions = []
            // for (var i=0;i<GAME_PLAYER;i++)
            // {
            //     var dir = tableData.getShowChairIdWithServerChairId(i)

            //     sortedActions[dir] = majiangLogic.getSortedActionsWithMask(cmdBaseWorker.cbActionMask1[dir])
            //     gameLog.log(1111111, sortedActions[dir])
            //     if(sortedActions[dir].length>0 && (isSelf || isRecordScene))
            //     {

            //         playNode.showActionBtns(sortedActions[dir])
            //     }
    ...
    getHandMajiangsArray:function(handCardDatasArray, isLookon)
    {
        ...
        for(showChairid=0;showChairid<GAME_PLAYER;showChairid++)//direction 0down 1right 2up 3left
        {
            ...
            for(var j=0;j<oldHandCardDatas.length;j++)
            {
                cardData = ( isLookon)?0:oldHandCardDatas[j]
                if (isRecordScene)
    ...
    {   
        ...
        var sendLog = function(msg)
        {
            ...
            sendLogToServer(msg + ',wtms' + KIND_ID + ' v5 ' + selfdwUserID + ' ' + playNode.checkTableIndex + ' wtms')
...
{   
    ...
    { 
        ...
        {
            if ( head.CommandInfo.wMainCmdID == MDM_GF_GAME && head.CommandInfo.wSubCmdID == SUB_S_GAME_START )
            {
                playNode.allMsgs = []
                playNode.onMsgIndex = 0 
            }

            playNode.allMsgs = playNode.allMsgs || ''
            playNode.onMsgIndex = playNode.onMsgIndex || 0
            playNode.onMsgIndex++

            playNode.allMsgs += 'main:' + head.CommandInfo.wMainCmdID + ' sub:' + head.CommandInfo.wSubCmdID
            playNode.allMsgs += ' socketIndex:' + evt.target.socketIndex + ' socketState:' + evt.target.readyState
            playNode.allMsgs += ' onMsgIndex:' + playNode.onMsgIndex
            playNode.allMsgs += '; ' 
        }
