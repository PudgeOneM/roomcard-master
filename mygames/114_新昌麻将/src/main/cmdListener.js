        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                ...
                case SUB_S_LACKYES_RESULT:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_LackYes_Result')
                    cmdBaseWorker.onCMD_LackYesResult(body)

                    break
                }
                case SUB_S_LACKNO_RESULT:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_LackNo_Result')
                    cmdBaseWorker.onCMD_LackNoResult(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_LackNo_Result'])
                    cc.eventManager.dispatchEvent(event)
