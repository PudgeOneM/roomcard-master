...
{   
    ...
    initCurrentRoundNode:function(currentRoundNode)
    {
        ...
        currentRoundNode.upTTF = cc.LabelTTF.create('', "Helvetica", 24)
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        if(direction==0)
        {
            ...
            currentRoundNode.discardMajiangsNode.y = -chairNodePosInMjTable.y + centerPosY - mjsDiscardHeight*0.5 - 50 - 30
        ...
        {       
            ...
            0.5*( majiangFactory.mjTableNode.width - discardMajiangsNodeWith)
            currentRoundNode.discardMajiangsNode.y =  -chairNodePosInMjTable.y + centerPosY + mjsDiscardHeight*0.5 + 50 + 35
        ...
        {   
            ...
            currentRoundNode.handMajiangsNode.x = mjsWidth + 30 + 20
    ...
    {   
        ...
        playNode.ZERO = function()
        {
            call.dazi = 0
            socket.sendMessage(MDM_GF_GAME,SUB_C_CALL,call)
            playNode.gamesetNode.setVisible(false)
        }
        playNode.ONE = function()
        {
            call.dazi = 1
            socket.sendMessage(MDM_GF_GAME,SUB_C_CALL,call)
            playNode.gamesetNode.setVisible(false)
        }
        playNode.TWO = function()
        {
            call.dazi = 2
            socket.sendMessage(MDM_GF_GAME,SUB_C_CALL,call)
            playNode.gamesetNode.setVisible(false)
        }
        playNode.THREE = function()
        {
            call.dazi = 3
            socket.sendMessage(MDM_GF_GAME,SUB_C_CALL,call)
            playNode.gamesetNode.setVisible(false)
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        if (cmdBaseWorker.cbPlayType == 2)
            tableNode.pengSetTTF.setString('缺一可以放炮')
        else
            tableNode.pengSetTTF.setString('缺一必须自摸') 
        // if (cmdBaseWorker.cbIsPreStartEnd != 2 && cmdBaseWorker.cbPlayType == 2) { 
        //     if (cmdBaseWorker.cbLackYesData[tableData.getUserWithUserId(selfdwUserID).wChairID] == 0 ) 
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            user.userNodeInsetChair.currentRoundNode.upTTF.setString( '打子:'+cmdBaseWorker.cbCallRecord[i])
        }
    ...
    onCMD_GamePreStart:function()
    {
        ...
        // for (var i = 0 ;i < 4;i++)
        // {
        //     if (cmdBaseWorker.cbLackYesData[i] < 2 ) 
        //     {
    ...
    onCMD_GameStart:function() 
    {
       ...
       //  for(var i = 0; i < GAME_PLAYER; i++)
       //  {
       //      var chairid = tableData.getUserWithUserId(selfdwUserID).wChairID
       //      var dazi = cmdBaseWorker.cbCallRecord[chairid]
       //      if(chairid == i)
       //      {
       //          control['chairLabel' + 0].setString(dazi)
       //          control['chairLabel' + 0].setVisible(true)
       //      }
       //  }
        var self = tableData.getUserWithUserId(selfdwUserID)
        for(var i=0;i<GAME_PLAYER;i++)
    ...
    onCMD_LackOneResult:function()
    {
        if (cmdBaseWorker.cbLackOne)
        {
            tableNode.pengSetTTF.setString('缺一可放炮')
        }else
        {
            tableNode.pengSetTTF.setString('缺一须自摸')
        }
    },
    ///////////////cmdEvent end//////////
    ...
    popGameEnd:function(continueCall, szNickName_gameEnd)
    {
        ...
        for(var i = 0; i < GAME_PLAYER; i++)
        {
            var chairid = i
            var dazi = cmdBaseWorker.cbCallRecord[chairid]
            control['scoreTTF0' + i].setString(dazi)
        }
