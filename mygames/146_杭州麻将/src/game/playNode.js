...
{   
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            //杭州麻将     判断是否有人打财神
            var Apply = true      //是否允许出牌

            //跟服务器一样的判断，当有人打财神的时候，若一个玩家，则该玩家允许出任何牌，若两个及以上则必须打摸来的牌
            // Apply = ( (cmdBaseWorker.wCurrentUser==cmdBaseWorker.bIsForbidActionUser[0] || !cmdBaseWorker.bIsOutMagic[cmdBaseWorker.bIsForbidActionUser[0]]) && !(cmdBaseWorker.bIsForbidActionUser[0]==cmdBaseWorker.bIsForbidActionUser[1]?false:cmdBaseWorker.bIsOutMagic[cmdBaseWorker.bIsForbidActionUser[1]]) )
            // if (majiang.cardData == cmdBaseWorker.cbProvideCardData)
            //     Apply = true
            for (var i=0;i<2;i++)
            {//杭州麻将    这里是不能吃什么打什么，里面放着不能打的牌
                if (cmdBaseWorker.cbForbidEatAndOutCardData[i]!=INVALID_CARD_DATA && cmdBaseWorker.cbForbidEatAndOutCardData[i]==majiang.cardData)
                {
                    Apply = false
                    break
                }
