...
{   
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            var direction = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(i))
    ...
    onCMD_GameStart:function() 
    {
        ...
        {
            playNode.showLaizi()
            playNode.timer.initFenwei( playNode.reGetShowChairId(bankerShowChairid ) )
            playNode.timerNode.setVisible(true)
            ...
            case 1:
            {
                ...
                var listener = cc.EventListener.create({
                    ...
                    onTouchBegan: function (touch, event) {
                        ...
                        if (!isopen&&locationInNode.x<(majiangs[0].length*62*majiangFactory.scale_upDown) && isSelf)
                            return false
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        else if(cmdBaseWorker.endType == 2)
        {
            ...
            {
                playNode.playAnimationWithDirection('zim', playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(winner)), call)
                playNode.playGenderEffect('zimo0', playNode.reGetShowChairId(tableData.getUserWithChairId(winner)).cbGender)
            }
        }
...
//         var bError = false
//         errorStr += '本次：des=' + des + ';socketIndex=' + this.socketIndex + ';curSocketIndex=' + socket.socketIndex + ';onMsgIndex=' + this.onMsgIndex + ','
//         errorStr += '上次：des=' + this.tableCardInfo.des + ';socketIndex=' + this.tableCardInfo.socketIndex + ';curSocketIndex=' + this.tableCardInfo.curSocketIndex + ';onMsgIndex=' + this.tableCardInfo.onMsgIndex + ','
...
//                     errorStr += '上次记录：count=' + this.tableCardInfo.cardInfo[i].count + '; src=' + this.tableCardInfo.cardInfo[i].src + ','
...
//         for ( var i = 0; i < GAME_PLAYER; i++ )
//         {
//             var handTotalCount = this.weaveMajiangs4D[i].length * 3 + this.handMajiangs4D[i][0].length
//             if ( this.handMajiangs4D[i][1] )
//                 handTotalCount++

//             if ( handTotalCount < MAX_COUNT - 1 || handTotalCount > MAX_COUNT )
//             {
//                 errorStr += '大小相公：wChairID = ' + i + ';handTotalCount = ' + handTotalCount
//                 errorStr += ';weaveCount=' + this.weaveMajiangs4D[i].length + ';oldHandCount=' + this.handMajiangs4D[i][0].length
//                 if ( this.handMajiangs4D[i][1] )
//                     errorStr += ';newHandCount=' + this.handMajiangs4D[i][1].length
//                 errorStr += ','
...
//         //连续处理同一个消息
//         if ( des == this.tableCardInfo.des && des != 'onCMD_OperateResult' && des != 'onCMD_StatusPlay' )
//         {
//             errorStr += '连续处理同一个消息: des=' + des + ','
...
//         {
//             msg += 'msgs:' + self.tableCardInfo.msgs + ','
...
//             sendLogToServer(msg + ',wtms' + KIND_ID + ' v4 ' + selfdwUserID + ' ' + playNode.checkTableIndex + ' wtms')
...
//         this.tableCardInfo.socketIndex = this.socketIndex
//         this.tableCardInfo.curSocketIndex = socket.socketIndex
//         this.tableCardInfo.msgs += des + ':' + playNode.onMsgIndex + ';'
//         this.tableCardInfo.onMsgIndex = playNode.onMsgIndex
//     },
// }
...
//     socket.socketIndex = socket.socketIndex || 0
//     socket.socketIndex++
//     instance.socketIndex = socket.socketIndex
...
//         {
//             playNode.socketIndex = evt.target.socketIndex
//             socket._onSocketMessage(data)
//         }
//         else if ( evt.target.socketIndex == socket.instance.socketIndex )
//         {
//             var head = buffer2StructObj(data.slice(0, 8), 'TCP_Head') 
//             if ( head.CommandInfo.wMainCmdID == MDM_GF_GAME || head.CommandInfo.wMainCmdID == MDM_GF_FRAME )
//             {
//                 var logMsg = 'wMainCmdID = ' + head.CommandInfo.wMainCmdID + ';'
//                 logMsg += 'wSubCmdID = ' + head.CommandInfo.wSubCmdID + ';'
//                 logMsg += 'evt.target.readyState = ' + evt.target.readyState + ';'
//                 logMsg += 'socket.instance.readyState = ' + socket.instance.readyState + ';'

//                 gameLog.log(logMsg)
//                 playNode.checkTableIndex = playNode.checkTableIndex || 0
//                 sendLogToServer(logMsg + ',wtms' + KIND_ID + ' v4_2 ' + selfdwUserID + ' ' + playNode.checkTableIndex + ' wtms')
//                 playNode.checkTableIndex++
//             }
//         }
//     } 
...
//     if ( head.CommandInfo.wMainCmdID == MDM_GF_GAME || head.CommandInfo.wMainCmdID == MDM_GF_FRAME )
//     {
//         playNode.onMsgIndex = playNode.onMsgIndex || 0
//         playNode.onMsgIndex++
//     }
