	...
	setCardData:function(cbCardData, cbCardCount, bSend)
	{
		...
		if (bSend)
		{
			for (var i = 0; i < handMahJong.cbCardCount; i++) 
			{
				...
				{
					handMahJong.control[i].baopi.setSpriteFrame('laizi.png');
					handMahJong.control[i].baopi.visible = true;
				}
				else if (cbCardIndex == mainScene._cbPiziIndex1 || cbCardIndex == mainScene._cbPiziIndex2)
				{
					handMahJong.control[i].baopi.setSpriteFrame('pi.png');
					handMahJong.control[i].baopi.visible = true;
				}
			}
		}
...
var disCardItem = 
{
	create:function(wChaird, cbCardData)
	{
		...
		if (cbCardIndex == mainScene._cbMagicIndex || cbCardIndex == mainScene._cbPiziIndex1 || cbCardIndex == mainScene._cbPiziIndex2)
		{
			var fPos = 
			[
				cc.p(25, 22),
			...
			var baopiRes = (cbCardIndex == mainScene._cbMagicIndex)?"#laizi.png":"#pi.png";
			item.baopiSign = new cc.Sprite(baopiRes);
			item.addChild(item.baopiSign);
			item.baopiSign.scale = disCardControl.baopiSignScale*SCALE_H_NO_MOBILE;
			item.baopiSign.setRotation(angle[wChaird]);
			item.baopiSign.setPosition(fPos[wChaird]);
		}
		return item;
	},
};
...
var disCardControl = 
{
	...
	setControlPos:function(wChaird)
	{
		...
		{
			case 0:
			fx = handMahJong.control[9].x;
			fy = handMahJong.control[0].y + handMahJong.control[0].height/2 + 10 + disCardControl.control[wChaird][i].height/2;
...
var baopi = 
{
	init:function()
	{
		...
		for (var i = 0; i < GAME_PLAYER; i++) {
			baopi.baoNum[i] = 0;
			baopi.piNum[i] = 0;
			topUI['baoNum'+i].setString(baopi.baoNum[i]);
      		topUI['piNum'+i].setString(baopi.piNum[i]);
		};
	},
	updateData:function(wChaird, cbCardData)
	{
		var wViewCharid = gameLogic.switchViewChairID(wChaird);
		var cbCardIndex = gameLogic.switchToCardIndex(cbCardData);
		if (cbCardIndex == mainScene._cbMagicIndex)
		{
			baopi.baoNum[wViewCharid]++;
			topUI['baoNum'+wViewCharid].setString(baopi.baoNum[wViewCharid]);
		}
		else if(cbCardIndex == mainScene._cbPiziIndex1 || cbCardIndex == mainScene._cbPiziIndex2)
		{
			baopi.piNum[wViewCharid]++;
			topUI['piNum'+wViewCharid].setString(baopi.piNum[wViewCharid]);
		}
	},
	clearData:function()
	{
