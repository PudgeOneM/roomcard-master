...
{   
    ...
    cbSiceCount:null,
    TurnoverCard:null,
    cbMagicCount:null,  //吉安
    cbNextGangCount:0,
    wSelfActionUser:INVALID_WORD,
    wBaWangUser:INVALID_WORD,
    cbChongGuanFlag:null,
    lEveryScore:null,
    lGangScore:null,
    bGameSetResult:null,
    bGameHasSelect:false,
    bIsChaoZhuang:false,
    init:function()
    {   
    },
    onReStart:function()
    {
        ...
        cmdBaseWorker.cbMagicCount        = null   //吉安
        cmdBaseWorker.cbNextGangCount     = 0
        cmdBaseWorker.wSelfActionUser     = INVALID_WORD
        cmdBaseWorker.wBaWangUser         = INVALID_WORD
        cmdBaseWorker.cbChongGuanFlag     = null
        cmdBaseWorker.lEveryScore         = null
        cmdBaseWorker.lGangScore          = null
        cmdBaseWorker.bGameSetResult      = null
        cmdBaseWorker.bGameHasSelect      = false
        cmdBaseWorker.bIsChaoZhuang       = false
    },
    onCMD_StatusFree:function(body) 
    {
        cmdBaseWorker.lCellScore        = body.lCellScore
        cmdBaseWorker.bGameHasSelect    = body.bGameHasSelect
        cmdBaseWorker.bGameSetResult    = body.bGameSetResult
    },
    ...
    onCMD_SelectResult:function(body)
    {
        cmdBaseWorker.bGameSetResult      = body.bGameSetResult
        cmdBaseWorker.bGameHasSelect      = body.bGameHasSelect
    },
    ...
    {	
        ...
        cmdBaseWorker.wSelfActionUser       = body.wSelfActionUser //吉安
        cmdBaseWorker.bGameSetResult        = body.bGameSetResult
        cmdBaseWorker.bGameHasSelect        = body.bGameHasSelect
    ...
    onCMD_OperateResult:function(body) 
    {
        ...
        cmdBaseWorker.wSelfActionUser      = body.wSelfActionUser
    },
    onCMD_GameEnd:function(body) 
    {
        cmdBaseWorker.cbMagicCount        = body.cbMagicCount //吉安
        cmdBaseWorker.cbNextGangCount     = body.cbNextGangCount
        cmdBaseWorker.wBaWangUser         = body.wBaWangUser
        cmdBaseWorker.cbChongGuanFlag     = body.cbChongGuanFlag
        cmdBaseWorker.lEveryScore         = body.lEveryScore
        cmdBaseWorker.lGangScore          = body.lGangScore
        cmdBaseWorker.bIsChaoZhuang       = body.bIsChaoZhuang
    ...
    getSortedActionsWithMask:function(acitonMask)
    {
        ...
        if((acitonMask&WIK_CHI_A)!=0)
            actions[actions.length] = WIK_CHI_A

        if((acitonMask&WIK_CHI_B)!=0)
            actions[actions.length] = WIK_CHI_B
    ...
    onActionResult:function(action, cardDatas, operateUser, provideUser, majiangs4W4D, handGroupNode4D)
    {
        if(action==WIK_GANG)
            ...
            majiangFactory.onActionChi(action, cardDatas, operateUser, provideUser, majiangs4W4D, handGroupNode4D)
        else if(action == WIK_CHI_A)
            majiangFactory.onActionChi(action, cardDatas, operateUser, provideUser, majiangs4W4D, handGroupNode4D)
        else if(action == WIK_CHI_B)
            majiangFactory.onActionChi(action, cardDatas, operateUser, provideUser, majiangs4W4D, handGroupNode4D)
    },
    ...
    getSortedOperateIdxs:function(provideIdx, action)
    {
        switch(action)
        {
            ...
            {
                if(provideIdx == 49)
                    return [provideIdx, cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)+1), cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)+3)]
                else if(provideIdx == 52)
                    return [provideIdx, cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)-3), cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)-1)]
                else if(provideIdx == 50)
                    return [provideIdx, cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)-1), cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)+2)]
                else if(provideIdx == 51)
                    return [provideIdx, cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)-2), cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)+1)]
            }
            case WIK_CHI_B:
            {
                if(provideIdx == 49)
                    return [provideIdx, cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)+2), cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)+3)]
                if(provideIdx == 52)
                    return [provideIdx, cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)-3), cmdBaseWorker.cai2Bao(cmdBaseWorker.bao2Cai(provideIdx)-2)]
    ...
    {
        if(kind == WIK_RIGHT || kind==WIK_CENTER || kind==WIK_LEFT || kind==WIK_CHI_A || kind==WIK_CHI_B)
        {
    ...
    {        
        ...
        for(var i=0;i<showLen;i++)
        {
            ...
            if((action==WIK_CHI_A || action==WIK_CHI_B) && mj.cardData==cmdBaseWorker.cbProvideCardData)
                mj.color = cc.color(122, 122, 122)
