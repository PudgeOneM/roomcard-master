...
{   
    ...
    {   
        ...
        playNode.actionCall_chi = function()
        {
            ...
            if(actions.length>1)
            {
                ...
                cmdBaseWorker.showChoosePopOfAction(idxsArray, actions, sendChi)
    ...
    {
        if(playNode.showCaiShenNumber == 0)
            return
        ...
        if( cmdBaseWorker.isMagicCard(mj.cardData, cmdBaseWorker.cbMagicCardData) ) //|| mj.cardData == REPLACE_CARD_DATA)
        {
            ...
            // mj.color = cc.color(255,255,130) //130
            // return;    
           

            //var s = new cc.Sprite("#caiShen.png")
            var s = new cc.Sprite(resp.jingSpr)
    ...
    onCMD_StatusFree:function() 
    {
        ...
        //pop
        if(!cmdBaseWorker.bGameHasSelect && tableData.createrUserID==selfdwUserID){
            topUINode.gameSetPopNode.visible = true
            myGameSetPop.createGameSetPop(topUINode.gameSetPopBg)
        }
    ...
    {
        playNode.showCaiShenNumber = 1 //吉安
    ...
    {
        topUINode.gameSelectSetNode.visible = false
        ...
        //cmdBaseWorker.sortHandIdxs(handIdxsArray[0][0])  //首次加载
        //get heapIdxsArray 
        var HcbTurnoverCard = cmdBaseWorker.TurnoverCard[0].cbCardData
        cmdBaseWorker.TurnoverCard[0].cbCardData = 0  // 第一次加载牌墙不显示翻出的牌
        var heapIdxsArray = majiangFactory.getHeapCardDatasArray(cmdBaseWorker.cbHeapCardInfo, cmdBaseWorker.TurnoverCard)   
        cmdBaseWorker.TurnoverCard[0].cbCardData =  HcbTurnoverCard  
        ...
        function showItemAfterDice()
        {
            playNode.showCaiShenNumber = 1
            for(var i=0; i<playNode.handMajiangs4D[0][0].length; i++)
            {
                var HcbCardData = playNode.handMajiangs4D[0][0][i].cardData
                if( HcbCardData == cmdBaseWorker.cbMagicCardData[0] || HcbCardData == cmdBaseWorker.cbMagicCardData[1])
                    playNode.decorateMj(playNode.handMajiangs4D[0][0][i])
            }
            if(playNode.handMajiangs4D[0][1])
            {
                var data11 = playNode.handMajiangs4D[0][1].cardData
                if(data11 == cmdBaseWorker.cbMagicCardData[0] || data11 == cmdBaseWorker.cbMagicCardData[1])
                    playNode.decorateMj(playNode.handMajiangs4D[0][1])
            }


            playNode.deleteOrAddTurnoverCard()
            playNode.showLaizi()
            playNode.deleteOrAddMagicCard()  //00000000000
        ...
        {
            tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
            playNode.playDiceOneDirection(showItemAfterDice, cmdBaseWorker.cbSiceCount[2], 
    ...
    onCMD_SendCard:function() 
    {
        ...
        playNode.timer.switchTimer([tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wTakeCardUser)]) //吉安
    ...
    {
        tableNode.selectLabel.setString("")
    },

    deleteOrAddMagicCard:function()
    {
        //首次加载手牌时，不特殊排序财神，显示癞子之后，才排序财神        
        var takeMajiang = playNode.handMajiangs4D[0][1]
        if(takeMajiang && playNode.isMagicCardData(takeMajiang))
            majiangFactory.deleteHandMajiangs(playNode.handMajiangs4D[0], 0, takeMajiang.cardData)
        ...
        for(var i=0; i<playNode.handMajiangs4D[0][0].length; i++)
        {
            ...
            if( HcbCardData == cmdBaseWorker.cbMagicCardData[0] || HcbCardData == cmdBaseWorker.cbMagicCardData[1])
            { 
                magicArray[count] = playNode.handMajiangs4D[0][0][i]
                majiangFactory.deleteHandMajiangs(playNode.handMajiangs4D[0], 0, HcbCardData)
                i --
                count ++
            }
        }
        ...
        {
            var HcbCardData = magicArray[i].cardData
            majiangFactory.insertHandMajiangsOld(playNode.handMajiangs4D[0], 0, HcbCardData, playNode.handGroupNode4D[0])
        }

        if(takeMajiang && playNode.isMagicCardData(takeMajiang))
        {
             var takeDir =  tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wTakeCardUser)
             majiangFactory.addHandMajiang(playNode.handMajiangs4D[takeDir], takeDir, takeMajiang.cardData, playNode.handGroupNode4D[takeDir], playNode.weaveMajiangs4D[takeDir].length)
        }
        
        
    },
    isMagicCardData:function(majiang)
    {
        if(majiang == null)
            return false

        return cmdBaseWorker.isMagicCard(majiang.cardData, cmdBaseWorker.cbMagicCardData)
    },


    deleteOrAddTurnoverCard:function()
    {
        //翻出的牌刚开始没显示，现在要把原来位置的牌删掉，重新加上要显示的牌
        var item = cmdBaseWorker.TurnoverCard[0]
        var heapMajiangsDir = tableData.getShowChairIdWithServerChairId(item.wHeapDir)  
        playNode.heapMajiangs4D[heapMajiangsDir][item.wHeapPos].cardData = item.cbCardData
        playNode.changeWeaveShowValue(item.cbCardData,playNode.heapMajiangs4D[heapMajiangsDir][item.wHeapPos],heapMajiangsDir)

    },
    changeWeaveShowValue:function(cardData,group,direction)
     {
        var data = group
        ...
        var prefix = t[direction] + 'heap'
        var frameName = 'style' + 0 + '_' + prefix + 'Bg' + '.png'
        //var spr = new cc.Sprite("#" + frameName)
        //spr.setSpriteFrame(frameName)
        //var frameName = prefix + 'Bg.png'
        data.setSpriteFrame(frameName)
        if(cardData!=0)
        {
    ...
    addMagicLabelEveryOne:function(gendBar, wChairID)
    {
        if(cmdBaseWorker.wExitUser != INVALID_WORD)
            return;

        var MagicCountLable = new cc.LabelTTF("", "Arial", 18)
        var magicCount = cmdBaseWorker.cbMagicCount[wChairID]
        //var magicLabelString = ('正精'+ magicCount[0] + '个,已出'+ magicCount[2]+'个  副精'+magicCount[1]+'个,已出'+magicCount[3]+'个')
        var magicLabelString = ('已出'+magicCount[2]+'正精,'+magicCount[3]+'副精')
        MagicCountLable.setString(magicLabelString)
        MagicCountLable.setPosition(cc.p(gendBar.getContentSize().width*0.55, gendBar.getContentSize().height*0.23))
        MagicCountLable.setVisible(true)
        gendBar.addChild(MagicCountLable)


        var charStr = ''
        if(magicCount && magicCount[4] == 1)
            charStr += ' 杠精'
        if(cmdBaseWorker.wBaWangUser != INVALID_WORD && wChairID == cmdBaseWorker.wBaWangUser)
            charStr += ' 霸王精'
        if(cmdBaseWorker.cbChongGuanFlag && cmdBaseWorker.cbChongGuanFlag[wChairID] == 1)
            charStr += ' 冲关'
        if(cmdBaseWorker.bIsChaoZhuang && cmdBaseWorker.bGameSetResult && cmdBaseWorker.bGameSetResult[7]==1)
        {
            if(wChairID != INVALID_WORD && wChairID == cmdBaseWorker.wBankerUser)
                charStr += ' 抄庄'
        }

        var strLabel = new cc.LabelTTF("", "Arial", 18)
        strLabel.setColor(cc.color(255,0,0))
        strLabel.setString(charStr)
        strLabel.setAnchorPoint(cc.p(1, 0.5))
        strLabel.setPosition(cc.p(MagicCountLable.getPositionX()*0.83, MagicCountLable.getPositionY()))
        gendBar.addChild(strLabel)
        // var bHasJingGang = false
        // var bHasBaJing = false
        // var JingGangLabel = new cc.LabelTTF("", "Arial", 18)
        // var BaWangUserLabel = new cc.LabelTTF("", "Arial", 18)
        // if(magicCount[4] == 1)
        // {
        //     JingGangLabel.setColor(cc.color(255,0,0))
        //     JingGangLabel.setString('杠精')
        //     //JingGangLabel.setPosition(cc.p(gendBar.getContentSize().width*0.42, MagicCountLable.getPositionY()))
        //     JingGangLabel.setPosition(cc.p(MagicCountLable.getPositionX()*0.80, MagicCountLable.getPositionY()))
        //     gendBar.addChild(JingGangLabel)
        //     bHasJingGang = true
        // }

        // if(cmdBaseWorker.wBaWangUser != INVALID_WORD && wChairID == cmdBaseWorker.wBaWangUser)
        // {
        //     BaWangUserLabel.setString('霸王精')
        //     BaWangUserLabel.setColor(cc.color(255,0,0))
        //     if(bHasJingGang)
        //         BaWangUserLabel.setPosition(cc.p(JingGangLabel.getPositionX()*0.88, JingGangLabel.getPositionY()))
        //     else
        //         BaWangUserLabel.setPosition(cc.p(MagicCountLable.getPositionX()*0.80,MagicCountLable.getPositionY()))

        //      gendBar.addChild(BaWangUserLabel)
        //      bHasBaJing = true
        // } 

        // if(cmdBaseWorker.cbChongGuanFlag[wChairID] == 1)
        // {
        //     var ChongGuanLabel = new cc.LabelTTF("", "Arial", 18)
        //     ChongGuanLabel.setString('冲关')
        //     ChongGuanLabel.setColor(cc.color(255,0,0))
        //     if(bHasBaJing)
        //         ChongGuanLabel.setPosition(cc.p(BaWangUserLabel.getPositionX()*0.88, BaWangUserLabel.getPositionY()))
        //     else if(bHasJingGang)
        //         ChongGuanLabel.setPosition(cc.p(JingGangLabel.getPositionX()*0.9, JingGangLabel.getPositionY()))
        //     else
        //         ChongGuanLabel.setPosition(cc.p(MagicCountLable.getPositionX()*0.80, MagicCountLable.getPositionY()))

        //     gendBar.addChild(ChongGuanLabel)
        // }
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        //////////////////吉安规则，开始////////////////////
        var NextGangLabel = new cc.LabelTTF("", "Arial", 18) 
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            if(wChairID==cmdBaseWorker.wExitUser)
                ...
                //////////////////// 吉安规则，开始 ////////////////////
                var m_gangCountString = cmdBaseWorker.cbNextGangCount==0?"":('连杠' + cmdBaseWorker.cbNextGangCount + '次')
                NextGangLabel.setString(m_gangCountString)               
                NextGangLabel.setPosition(cc.p(gendBar.getContentSize().width*0.84, gendBar.getContentSize().height*0.43))
                NextGangLabel.setVisible(true)
                gendBar.addChild(NextGangLabel)

                playNode.addMagicLabelEveryOne(gendBar, wChairID)
                //////////////////// 吉安规则，结束 ////////////////////
            ...
            if(cmdBaseWorker.wExitUser == INVALID_WORD)
            {
                ...
                control['jingTTF'+wChairID].setString((cmdBaseWorker.lEveryScore[wChairID][1]>0?'+':'') + cmdBaseWorker.lEveryScore[wChairID][1])
                control['HhuTTF'+wChairID].setString((cmdBaseWorker.lEveryScore[wChairID][0]>0?'+':'') + cmdBaseWorker.lEveryScore[wChairID][0])
                control['gangTTF'+wChairID].setString((cmdBaseWorker.lGangScore[wChairID]>0?'+':'') + cmdBaseWorker.lGangScore[wChairID])
            }
    ...
    showLaizi:function()
    {
        for(var i=0;i<TURNOVER_COUNT_MAGIC;i++)
        {
            ...
            mj.x = 50*i + 40
            mj.y = - 40
            mj.setScaleX(bg.width/mj.width)
            mj.setScaleY(bg.height/mj.height)
    ...
    showActionBtns:function(sortedActions)
    {
        ...
        {   
            ...
            else if(action == WIK_LEFT || action == WIK_CENTER || action == WIK_RIGHT || action == WIK_CHI_A || action == WIK_CHI_B)
            {
   ...
   //////////////以下内容是查看选择项
    clickViewSelectResult:function()
    {

        if(cmdBaseWorker.bGameHasSelect)
        {
            playNode.addSprOnClickViewSelect()
        }
        else
        {
            playNode.myGameSetANode.visible = true
            setTimeout(function(){
                playNode.myGameSetANode.visible = false
            }, 3000)
        }
    },
    addSprOnClickViewSelect:function()
    {
        if(!playNode.myViewPopNode.visible)
        {
            if(!myGameSetPop.typeName || !myGameSetPop.itemName || !cmdBaseWorker.bGameSetResult)
                return

            var width = topUINode.gameSetPopBg.getContentSize().width
            var height = topUINode.gameSetPopBg.getContentSize().height
            var typeName = myGameSetPop.typeName
            var itemName = myGameSetPop.itemName

            for(var type=0; type<typeName.length; type++)
            {
                var selectItem = cmdBaseWorker.bGameSetResult[type]-1
                var itemArray = itemName[type]
                for(var item=0; item<itemArray.length; item++)
                {
                    ...
                    var spr = new cc.Sprite(res)
                    spr.setAnchorPoint(cc.p(0, 0))
                    spr.myType = type
                    spr.myItem = item
                    myGameSetPop.setButtonPosition(spr, width, height)
                    playNode.myViewPopBG.addChild(spr)
                }
            }

            var offBtn = new ccui.Button()
            offBtn.loadTextures("modelOff.png", "modelOff.png", "modelOff.png", ccui.Widget.PLIST_TEXTURE)
            offBtn.setAnchorPoint(cc.p(0, 0))
            var btnsize = offBtn.getContentSize()
            offBtn.setPosition(cc.p(width-btnsize.width*1.3, height-btnsize.height*1.2))
            offBtn.addClickEventListener(function(btn){
                playNode.myViewPopNode.visible = false
            });
            playNode.myViewPopBG.addChild(offBtn)

        }        
        playNode.myViewPopNode.visible = true
