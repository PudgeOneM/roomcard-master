...
{   
    ...
    {   
        ...
        for (var i = 0; i < GAME_PLAYER; i++)
            tableNode['userHu_' + i].enableStroke(cc.color(0, 0, 0, 255), 2)
    },
    ...
    onReStart:function()
    {
        ...
        playNode.btn_ChangeCards.setVisible(false)
    ...
    {   
        ...
        playNode.ChangeCardsClick = function()
        {
            if (playNode.currentPopMajiangs.length < 3)
                return
            var deleteCards = getObjWithStructName('CMD_C_ChangeCards')
            for (var i = 0; i < playNode.currentPopMajiangs.length; i++)
                deleteCards.cbDeleteCards[i] = playNode.currentPopMajiangs[i].cardData
            
            socket.sendMessage(MDM_GF_GAME,SUB_S_GAME_CHANGE,deleteCards)
            playNode.btn_ChangeCards.setVisible(false)
        }
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        var isSelAll = true
        var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID
        var isSelChangeCards = false
        if (cmdBaseWorker.stWinType.cbChangeCard == 1 && cmdBaseWorker.bFinishChangeCards == false)
        {
            isSelAll = false
            isSelChangeCards = true
            if (cmdBaseWorker.isSelChange == false)
            {
                playNode.isChanging = true
                playNode.InitChangeCards()
            }
            else
            {
                for (var n = 0; n < GAME_PLAYER; n++)
                    playNode.setChangeCardPicVisible(n, cmdBaseWorker.isUserSelChange[n] == 0)
            }
        }

        if (isSelChangeCards == false)
        {
            for (var i=0;i<GAME_PLAYER;i++)
            {
    ...
    setDingQueVisible:function(chairId, show)
    {
        var self = tableData.getUserWithUserId(selfdwUserID)
        var showDir = tableData.getShowChairIdWithServerChairId(chairId) 
        if (showDir == 0 && self.cbUserStatus != US_LOOKON)
            return
         var dingQue = tableNode['dingQue' + showDir]
         if (dingQue)
            dingQue.setVisible(show)
    },
    setChangeCardPicVisible:function(chairId, show)
    {
        var self = tableData.getUserWithUserId(selfdwUserID)
        var showDir = tableData.getShowChairIdWithServerChairId(chairId) 
        if (showDir == 0 && self.cbUserStatus != US_LOOKON)
            return
         var changeCard = tableNode['changeCardPic_' + showDir]
         if (changeCard)
            changeCard.setVisible(show)
    },
    ...
    onCMD_Call:function()
    {
        ...
        for (var i=0;i<GAME_PLAYER;i++)
        {
            ...
            else 
            {
                ...
                else if (isRecordScene) ////// - - - - - - - - - - - - - -/////
                {
                    cmdBaseWorker.reSortScore(i)
                    cmdBaseWorker.reSortHandMjs(i)
                    ////////////////////////
                    showSel.setVisible(true)
                    var curDir = tableData.getShowChairIdWithServerChairId(i)  
                    var majiangs = playNode.handMajiangs4D[curDir]
                    for(var j=0;j<majiangs[0].length;j++)
                    {
                        var majiang = majiangs[0][j]
                        if (majiang)
                            cmdBaseWorker.setSelMajiangColor(majiang, i)
                    }
    ...
    onCMD_GameStart:function() 
    {
        ...
        tableNode.fangzhuNode.setVisible(false)
        cmdBaseWorker.resetWinnerOrder()
        ...
        function gameStart()
        {
            ...
            if (cmdBaseWorker.stWinType.cbChangeCard == 0)
            {
                for (var i = 0; i < GAME_PLAYER; i++)
                    playNode.setDingQueVisible(i, cmdBaseWorker.cbCallRecord[i] == INVALID_BYTE)
            ...
            {
                playNode.isStartGame = true
                if (playNode.isChangeCard == true)
                {
                    playNode.isChanging = true
                    if (cmdBaseWorker.isinitChangeCards == true)
                    {
                        playNode.currentPopMajiangs = []
                        playNode.InitChangeCards()
                    }
                    else
                        cmdBaseWorker.changeCard()
                }
            }
    ...
    InitChangeCards:function()
    {
        var self = tableData.getUserWithUserId(selfdwUserID)
        for (var n = 0; n < GAME_PLAYER; n++)
            playNode.setChangeCardPicVisible(n, cmdBaseWorker.isUserSelChange[n] == 0)
        if (self.cbUserStatus == US_PLAYING)
        {
            playNode.btn_ChangeCards.setVisible(true)
            var mjHeight = majiangFactory.down_handHeight*majiangFactory.scale_upDown
            var selfChairId = self.wChairID
            var curDir = tableData.getShowChairIdWithServerChairId(selfChairId)  
            var majiangs = playNode.handMajiangs4D[curDir]
            var lastPos = 0
            for (var i = 0; i < 3; i++)
            {
                var isFind = false
                for (var j = lastPos; j < majiangs[0].length; j++)
                {
                    if (majiangs[0][j].cardData == cmdBaseWorker.cbChangeCard[selfChairId].cbDeleteCards[i])
                    {
                        isFind = true
                        majiangs[0][j].y = mjHeight*0.5+20
                        playNode.currentPopMajiangs[playNode.currentPopMajiangs.length] = majiangs[0][j]
                        lastPos = j+1
                        break
                    }
                }
                if (isFind == false && playNode.handMajiangs4D[curDir][1])
                {
                    if (playNode.handMajiangs4D[curDir][1].cardData == cmdBaseWorker.cbChangeCard[selfChairId].cbDeleteCards[i])
                    {
                        playNode.currentPopMajiangs[playNode.currentPopMajiangs.length] = playNode.handMajiangs4D[curDir][1]
                        playNode.handMajiangs4D[curDir][1].y = mjHeight*0.5+20
                    }
                }
            }

            if (playNode.currentPopMajiangs.length >= 3)
                playNode.btn_ChangeCards.setEnabled(true)
            else
                playNode.btn_ChangeCards.setEnabled(false)
        }
    },
    onSMD_ChangeCard:function()
    {
        playNode.isChangeCard = true
        if (playNode.isStartGame == true)
        {
            playNode.isChanging = true
            if (cmdBaseWorker.isinitChangeCards == true)
            {
        ...
        if (cmdBaseWorker.isinitChangeCards == false)
            cmdBaseWorker.changeCard()
    },
    onSMD_Recommend:function()
    {   
        if (isRecordScene && cmdBaseWorker.wVideoUser != INVALID_WORD)
        {
            var operateUserId = tableData.getUserWithChairId(cmdBaseWorker.wVideoUser)
        ...
        playNode.lbl_SelWan.setVisible(false)
        playNode.lbl_SelSuo.setVisible(false)
        playNode.lbl_SelTong.setVisible(false)
        for (var i = 0; i < cmdBaseWorker.cbRecommand.length; i++)
        {
            if (cmdBaseWorker.cbRecommand[i] == 1)
            {
                if (i == 0)
                    playNode.lbl_SelWan.setVisible(true)
                else if (i == 1)
                    playNode.lbl_SelSuo.setVisible(true)
                else if (i == 2)
                    playNode.lbl_SelTong.setVisible(true)
            }
        }
    },
    onSMD_ChangeCardSel:function()
    {   
        var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID
        if (cmdBaseWorker.cbOperateChangeSelUser == selfChairId)
            cmdBaseWorker.isSelChange = true
        cmdBaseWorker.changeCardDelete(cmdBaseWorker.cbChangeCardSel)
    ...
    onCMD_SendCard:function() 
    {
        ...
        for(var i=0;i<cmdBaseWorker.cbSendCardCount;i++)
        {
            ...
            else
            {
                ...
                if (isRecordScene)
                {
                    if ((idx&MASK_COLOR)>>4 == cmdBaseWorker.cbCallRecord[cmdBaseWorker.wTakeCardUser])
                    {
                        var majiangs = playNode.handMajiangs4D[takeDir]
                        for(var j=0;j<majiangs[0].length;j++)
                        {
                            var majiangold = majiangs[0][j]
                            if (majiangold)
                                cmdBaseWorker.setSelMajiangColor(majiangold, cmdBaseWorker.wTakeCardUser)
                        }

                        var majiangNew1 = playNode.handMajiangs4D[takeDir][1]
                        if (majiangNew1)
                            cmdBaseWorker.setSelMajiangColor(majiangNew1, cmdBaseWorker.wTakeCardUser)
                    }
                }
            }
    ...
    onCMD_OperateResult:function() 
    {
        ...
        if(cmdBaseWorker.cbOperateCode != WIK_NULL)
        {
            ...
            if (majiang && operateUser.dwUserID==selfdwUserID)
    ...
    addWeaveMajiangs2:function(majiangsOneDirection, direction, weaveItem, parent, selfDirection)
    {
        ...
        var majiangsOneGroup = playNode.weaveItem2Majiangs2(groupIdx, direction, weaveItem, isSelf, isPublicAnGang)
    ...
    onActionGang:function(cardDatas, operateUser, provideUser, majiangs4W4D, handGroupNode4D)
    {
        ...
        else
        {
            ...
            playNode.addWeaveMajiangs2(operateWeaveMajiangs, operateUserDir, 
                {
    ...
    onCMD_GameEnd:function() 
    {
        ...
        for (var i = 0; i < GAME_PLAYER; i++)
        {
            ...
            playNode.setDingQueVisible(i, false)
            playNode.setChangeCardPicVisible(i, false)
        }
        playNode.gamesetNode.setVisible(false)
    ...
    _gethandGroupNodeListener:function(majiangs, handGroupNode, direction, outCardCall)
    {
        ...
        var touchPosX2TouchedMj2 = function(posX)
        {      
            if(playNode.istianhubahua)
            {
                return false
            ...
            else if(posX<=mjWidth*majiangs[0].length)
            {
                var idx = Math.floor( posX/mjWidth )
                juli_1 = majiangs[0][idx].getPosition().x
                juli_2 = majiangs[0][idx].getPosition().y
        ...
        var onTouch2 = function(locationX)
        {   
            var touchedMj = touchPosX2TouchedMj2(locationX)
            if(!touchedMj)
                return 
        ...
        onTouchBegan: function (touch, event) {
                ...
                else if (cmdBaseWorker.isSelChange == false)
                {
                    if(playNode.currentPopMajiangs.length>0)
                    {
                        var touchedMj = touchPosX2TouchedMj2(locationX)
                        if (touchedMj)
                        {
                            var touchColor = (touchedMj.cardData&MASK_COLOR)>>4 
                            var curColor = 0
                            for (var pos = 0; pos < playNode.currentPopMajiangs.length; pos++)
                            {
                                curColor = (playNode.currentPopMajiangs[pos].cardData&MASK_COLOR)>>4 
                                if (playNode.currentPopMajiangs[pos] == touchedMj || curColor != touchColor)
                                {
                                    playNode.currentPopMajiangs[pos].y = mjHeight*0.5
                                    if (playNode.currentPopMajiangs[pos] == touchedMj)
                                    {
                                        playNode.currentPopMajiangs.splice(pos, 1)
                                        if (playNode.currentPopMajiangs.length >= 3)
                                            playNode.btn_ChangeCards.setEnabled(true)
                                        else
                                            playNode.btn_ChangeCards.setEnabled(false)
                            ...
                            if (curColor == touchColor && playNode.currentPopMajiangs.length >= 3)
                                return true

                            if (curColor != touchColor)
                            {
                                playNode.currentPopMajiangs = []
                                if (playNode.currentPopMajiangs.length >= 3)
                                    playNode.btn_ChangeCards.setEnabled(true)
            ...
            onTouchMoved: function (touch, event) {
                ...
                if(currentMajiang && playNode.isChanging == false)
                {   
            ...
            onTouchEnded: function (touch, event) {
                ...
                if(currentMajiang)
                {  
                    if(((isTouchFromPop && touchedMjNum==1) || (ismoved && oldPosY!=null && (oldPosY+22) < currentMajiang.y)) && playNode.isChanging == false )
                    {
                    ...
                    else if (ismoved == false)
                    {
                        ...
                        {
                            playNode.currentPopMajiangs[playNode.currentPopMajiangs.length] = currentMajiang
                            if (playNode.currentPopMajiangs.length >= 3)
                                playNode.btn_ChangeCards.setEnabled(true)
                            else
                                playNode.btn_ChangeCards.setEnabled(false)
...
/////////////////////////@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    weaveItem2Majiangs2:function(groupIdx, direction, weaveItem, isSelf, isPublicAnGang)
    {
        var cardDatas = weaveItem.cbCardDatas
        var majiangs = []
        var hasAddDirectionSpr = false
    ...
    _showSprsOnGameEnd:function()
    {
        ...
        playNode._scoreShowToHide(cmdBaseWorker.lGameScore)
    ...
    popGameEnd:function(continueCall, szNickName_gameEnd)
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
           ...
           if (cmdBaseWorker.cbWinnerOrder[i] > 0 && cmdBaseWorker.cbWinnerOrder[i] < 4)
           {
                control['winnerOrder_'+i].setVisible(true)
                control['winnerOrder_'+i].setSpriteFrame('huSort_' + cmdBaseWorker.cbWinnerOrder[i] + '.png')
           }
           else
                control['winnerOrder_'+i].setVisible(false)
    ...
    showActionBtns:function(sortedActions)
    {
        ...
        {   
            ...
            btn.setVisible(true)
            btn.setPositionX(-175 * (sortedActions.length-1-i +1))
        ...
        {
            playNode.hideActionBtns()
            playNode.btn_hu.setVisible(true)
        }
        else
