        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                ...
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_C_GameType') 
                ...
                case SUB_S_GAME_CHANGE:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_ChangeCards') 
                    cmdBaseWorker.onSMD_ChangeCard(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onSMD_ChangeCard'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_GAME_CHANGESEL:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_ChangeCardSel') 
                    cmdBaseWorker.onSMD_ChangeCardSel(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onSMD_ChangeCardSel'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_GAME_RECOMMEND:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_Recommend') 
                    cmdBaseWorker.onSMD_Recommend(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onSMD_Recommend'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_GAME_END: 
