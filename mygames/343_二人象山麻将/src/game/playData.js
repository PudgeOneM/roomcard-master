        ...
        switch(head.CommandInfo.wMainCmdID)
        {
            case MDM_GF_GAME:       //游戏消息 200
            {
                switch(head.CommandInfo.wSubCmdID)
                {
                    ...
                    case SUB_S_LISTEN:
                    {
                        var body = buffer2StructObj(msg.slice(8), 'CMD_S_Listen');
                        return mainScene.onSubListenCard(body); 
                    }
                    case SUB_S_LISTEN_NOTIFY:
                    {
                        var body = buffer2StructObj(msg.slice(8), 'CMD_S_ListenNotify');
                        return mainScene.onSubListenNotify(body); 
                    }
                }
                break;
            }
