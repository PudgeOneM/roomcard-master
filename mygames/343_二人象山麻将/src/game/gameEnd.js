var gameEnd = 
{
	...
	setGameResult:function(wWinner)
	{
		...
		else
		{
			...
			if (nType < 12 && gameEnd.result.cbGangCount > 0) 
			{
				gameEnd['type'+nType].setString('杠  '+gameEnd.result.cbGangCount*2+'台');
				gameEnd['type'+nType].visible = true;
				nType = nType + 1;
			}
			if (nType < 12 && gameEnd.result.bListenStatus) 
			{
				gameEnd['type'+nType].setString('听牌  '+'4台');
				gameEnd['type'+nType].visible = true;
				nType = nType + 1;
			}
