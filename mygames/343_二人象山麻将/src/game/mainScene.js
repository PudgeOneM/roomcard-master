...
var mainScene = {
	runThisScene:function()
	{
      ...
      cc.spriteFrameCache.addSpriteFrames(resp.amtListenPlist, resp.amtListen)
      //mainNodeCCB
  ...
  initUiPlayCCB:function()
  {
      ...
      var l = cc.EventListener.create({
          ...
          {   
              ...
              //游戏内容
              tingTipWnd.init(mainScene.uiTopUI);      //初始化听牌提示
              tingWnd.init(mainScene.uiTopUI);      //初始化听牌窗口
  ...
  testCode:function()
  {
      ...
      mainScene._cbTingCount = 1;
      mainScene._tagTingItem = [];
      mainScene._tagTingItem[0] = {};
      mainScene._tagTingItem[0].cbOutCard = 1;
      mainScene._tagTingItem[0].cbTingKouCount = 2;
      mainScene._tagTingItem[0].cbTingKou = [2,3];
      mainScene._tagTingItem[0].cbFanShu = [9,7];
      //tingTipWnd.create(1);
      mainScene._bListenStatus = true;
      tingWnd.create(1);
      ...
      //    topUI['zhenghuaNum'+i].setSpriteFrame('buhuaNum_'+8+'.png');
      //    topUI['zhenghua'+i].visible = true;
      //    topUI['yehuaNum'+i].setSpriteFrame('buhuaNum_'+8+'.png');
      //    topUI['yehua'+i].visible = true;
          // for (var j = 0; j < 17; j++) {
  ...
  onEventScenePlay:function(data)
  {
      ...
      mainScene._bListenStatus   = data.bListenStatus;
      mainScene._cbGangCard      = data.cbGangData;
      for (var i = 0; i < GAME_PLAYER; i++) {
          mainScene._szNickName[i] = tableData.getUserWithChairId(i).szNickName;
          mainScene._cbBao[i] = [0,0,0,0];
      };
      mainScene._cbTingCount = 0;
      mainScene._tagTingItem = [];
      if (data.cbTingCount > 0 && mainScene._wMeChaird != INVALID_CHAIR) 
      {
          mainScene._cbTingCount = data.cbTingCount;
          mainScene._tagTingItem = data.tingItem.slice(0, data.cbTingCount);
      }
      //设置庄家
      ...
      if (mainScene._wCurrentUser == mainScene._wMeChaird || (mainScene._wCurrentUser == 0 && mainScene._wMeChaird == INVALID_CHAIR))
      {
          if (data.cbSendCardData > 0) 
          {
            ...
            {
               if(mainScene._cbCardData[i] == data.cbSendCardData)
               {
                  mainScene._cbCardData[i] = 0;
                  mainScene._cbCardData.sort(function(a,b){return b-a;});
                  break;
               }
            }
            mainScene._cbCardData.unshift(data.cbSendCardData);
      ...
      if (mainScene._wCurrentUser != INVALID_CHAIR)
      {
          if (mainScene._wCurrentUser == mainScene._wMeChaird || (mainScene._wCurrentUser == 0 && mainScene._wMeChaird == INVALID_CHAIR))
          {
              ...
              if (!mainScene.checkBuHua()) 
              {
                  ...
                  //听牌自动出牌
                  if (mainScene._bListenStatus) 
                  {
                      handMahJong.setEnableTing();
                      if (mainScene._bListenStatus && mainScene._wCurrentUser == mainScene._wMeChaird && mainScene._cbActionMask == WIK_NULL)
                      {
                          setTimeout(function()
                          {
      ...
      for (var i = 0; i < GAME_PLAYER; i++) {
          ...
          for (var k = 0; k < MAX_HUA_CARD; k++) {
              ...
              {
                  mainScene._cbHuaCount++;
                  huaCardControl.addCardItem(gameLogic.switchViewChairID(i), cbHuaCardData[k]);
              }
          };
      };
      //设置三包  做生意
      ...
      tingTipWnd.setCardIndex();
      //听牌窗口
      tingWnd.create(data.cbListenOutCard);
  },
  //游戏开始
	onSubGameStart:function(data)
	{
      ...
      mainScene._cbTingCount     = 0;
      mainScene._tagTingItem     = [];
      mainScene._bListenStatus   = false;
      mainScene._cbGangCard      = data.cbGangData;
      for (var i = 0; i < GAME_PLAYER; i++) {
          mainScene._szNickName[i] = tableData.getUserWithChairId(i).szNickName;
          mainScene._cbBao[i] = [0,0,0,0];
  ...
  onSubReplaceCard:function(data)
  {
      ...
      mainScene._cbHuaCount = mainScene._cbHuaCount + nBuHua;
      ...
      {
          otherMahJong.setHuaCard(gameLogic.switchViewChairID(data.wReplaceUser),data.cbHuaCard, data.cbReplaceCard);
      }
  ...
  //听牌
  onSubListenCard:function(data)
  {
      //播放动画
      var wOperateViewID = gameLogic.switchViewChairID(data.wChairId);
      operateAmt.setAmtInfo(wOperateViewID, WIK_LISTEN);
      if(data.wChairId == mainScene._wMeChaird)
      {
          mainScene._bListenStatus = true;
          tingWnd.create(data.cbOutCardData);
      }
  },
  //有玩家出牌
  onSubOutCard:function(data)
  {
      ...
      //出牌音效
      managerAudio.playEffect('gameRes/sound/cardData'+mainScene._cbOutCardData+'.wav');
      tingWnd.updataData();
      return true;
  },
  //发牌
  ...
  //听牌操作
  onSubListenNotify:function(data)
  {
      if (!mainScene._bSetUserMahJong)
      {
          mainScene.scene.unschedule(mainScene.setUserMahJong);
  ...
  onSubOperateResult:function(pOperateResult)
  {
      ...
      //播放动画
      operateAmt.setAmtInfo(wOperateViewID, pOperateResult.cbOperateCode);
      //设置变量
      ...
      {
        //八花全齐 有杠 牌堆数量-2
        mainScene._cbLeftCardCount = mainScene._cbLeftCardCount-2;
        //设置变量
        mainScene._wCurrentUser=INVALID_CHAIR;
  ...
  onsubGameEnd:function(data)
  {
      ...
      if (wWinner == data.wProvideUser)
      {
        if (wWinner != INVALID_CHAIR)
        {
           ...
           operateAmt.setAmtInfo(wViewChairID, WIK_ZIMO);
        }
      }
      else if (wWinner != INVALID_CHAIR)
      {
         ...
         operateAmt.setAmtInfo(wViewChairID, WIK_DIANPAO);
      }
