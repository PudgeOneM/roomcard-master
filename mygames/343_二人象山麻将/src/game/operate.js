...
var operateBtn = 
{
	...
	{
		operateBtn.srcPosX = mainScene.ctPosX*2*0.95;
        ...
        operateBtn._btnListen = new ccui.Button(resp.btnListen);
        operateBtn._btnListen.addTouchEventListener(this.onTouchEvent, this);
        operateBtn._btnListen.visible = false;
        operateBtn._btnListen.y = 180;
        uiPlay.addChild(operateBtn._btnListen,ZORDER_OPERATE_BTN);
        ...
        operateBtn._btnPass.setPosition(cc.p(operateBtn.srcPosX - 100, 180));
	...
	setOperateInfo:function(cbAcitonMask)
	{
		...
		var posX = operateBtn.srcPosX - 230;
		var bOperate = false;
		...
		{
			operateBtn._btnListen.visible = true;
			operateBtn._btnListen.x = posX;
			posX -= 130;
			bOperate = true;
		}
    ...
    onTouchEvent: function (render, type) {
    	...
    	else if (render == operateBtn._btnListen) 
    	{
    		handMahJong.setTingCard();
    		return ;
    	}
    ...
    getSelectCardInfo:function( cbOperateCode,SelectInfo )
    {
		...
		else if( cbOperateCode & WIK_GANG )
		{
			...
			if( mainScene._wCurrentUser == mainScene._wMeChaird )
			{
				...
				{
					if(mainScene._cbGangCard[i] == 0) break;
					SelectInfo[cbSelectCount] = {};
				 	SelectInfo[cbSelectCount].cbCardData = [];
				 	SelectInfo[cbSelectCount].cbActionCard = mainScene._cbGangCard[i];
				 	SelectInfo[cbSelectCount].wActionMask = WIK_GANG;	
					SelectInfo[cbSelectCount].cbCardCount = gameLogic.getWeaveCard(WIK_GANG,mainScene._cbGangCard[i],
						SelectInfo[cbSelectCount].cbCardData);
					cbSelectCount++;
