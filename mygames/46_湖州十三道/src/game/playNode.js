...
{   
    ...
    outCard:function(special)
    {
        ...
        if(specialType.specialType)
        {
            ...
            if(callFlag==special_quanhongyihei || callFlag==special_quanheiyihong 
                || callFlag==special_quanhong || callFlag==special_quanhei)
            {//全红，全黑，全红1黑，全黑1红，比较相同花色最大的一张
                tailType.score = gameLogic.getSpecialScore(cardDatas)
            }
            if(callFlag == special_liuduiban)
            {
                if(special == 0)//判断下是不是直接点的特殊牌型
                {
