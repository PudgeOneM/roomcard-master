...
var gameLogic_laizidoudizhu = {
    resp:'components/gameLogic_laizidoudizhu/res/',
    onPreLoadRes:function()
    {   
        var resp = gameLogic_laizidoudizhu.resp
        cc.spriteFrameCache.addSpriteFrames(resp + 'gameLogic_laizidoudizhu.plist', resp + 'gameLogic_laizidoudizhu.png')
    }
}

var gameLogic = gameLogic_laizidoudizhu
...
gameLogic.getRed10Count = function(idxs)
{
    var count = 0
    for(var i=0;i<idxs.length;i++)
    {
        if(idxs[i] == 0x0A || idxs[i] == 0x2A)
            count++
    }
...
var cardType1 = 
{
    ...
    getIdxsIfHas:function(c_sortedIdx54sExcludeLazi, c_sortedIdx54sOfLaizi, typeScores)
    {
        ...
        {
            var idxs = c_sortedIdx54sExcludeLazi.concat(c_sortedIdx54sOfLaizi)
            for(var i=0;i<idxs.length;i++)
            {
                if(idxs[i] == 0x0A )
                    return [0x0A]
...
[ 
    [
        cardType1,//单张
        {id:1, minLen:3, maxLen:15, minCardNum:1},//顺子
