...
{   
    ...
    onCMD_OutCard:function() 
    {
        ...
        ////////
        gameLog.log('name = ' + cmdBaseWorker.outCardType.name)
        if(cmdBaseWorker.outCardType.name=='zhadan')
        {
...
// ]
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            if ( i == cmdBaseWorker.wExitUser )
            {
                ...
                control['winflag'+i].setSpriteFrame('gendFlag0.png') 
            }
            else if( wWinChairId != INVALID_WORD && cmdBaseWorker.cbHandCardCount[i] == MAX_COUNT )
            {
                control['winflag'+i].setVisible(true)
                control['winflag'+i].setSpriteFrame('gendFlag2.png') 
            }
            else
            {
    ...
    {   
        ...
        {             
            ...
            playNode.outCardCallOut2(selectCardDatas)
        }
        playNode.outCardCallOut2 = function(selectCardDatas)
        {
            //首出判断
            var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID
            if( cmdBaseWorker.cbPlayCount == 1 && selfChairId == cmdBaseWorker.wBankerUser && cmdBaseWorker.cbHandCardCount[selfChairId] == MAX_COUNT )
            {
                for(var i=0;i<selectCardDatas.length;i++)
                {
                    if ( selectCardDatas[i] == 0x33 ) 
                        break
                }
                if ( i == selectCardDatas.length ) 
                {
    ...
    _outCardCall1:function(cardsType)
    {
        ...
        {
            var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID
            if(cmdBaseWorker.wOutCardUser == INVALID_WORD || selfChairId == cmdBaseWorker.wOutCardUser) //上一次没人出牌 或者上一次自己出的牌
                isAllowOut = cardsType.typeLevel > 1 || ( cardsType.typeLevel == 1 && cardsType.cardDatas.length == cmdBaseWorker.cbHandCardCount[selfChairId])
            else
                isAllowOut = gameLogic.compareTwoCardsType(cardsType, cmdBaseWorker.outCardType) == 0

            if ( isAllowOut && cardsType.cardDatas.length == 1 )
            {
                var filtrateCards = gameLogic.filtrateOutCardArray([cardsType.cardDatas])
                var bOut = false;
                for ( var i = 0; i < filtrateCards.length; i++ )
                {
                    if ( filtrateCards[i][0] == cardsType.cardDatas[0] )
                    {
                        bOut = true
                        break
                    }
    ...
    {
        var originCard = gameLogic.getOriginCard(cbOutCardData)
    ...
    popOutCardNode:function(isCan)
    {
        ...
        var tipsArray
        var bSelfMax = cmdBaseWorker.wOutCardUser == INVALID_WORD || selfChairId == cmdBaseWorker.wOutCardUser
        if(bSelfMax) //上一次没人出牌 或者上一次自己出的牌
        {
        ...
        tipsArray = gameLogic.filtrateOutCardArray(tipsArray)
        playNode.tipsArray = tipsArray

        playNode.outCardButton2.currentTipsIdx = 0  
        var isAllowPass = cmdBaseWorker.outCardType.cardDatas && cmdBaseWorker.wOutCardUser != INVALID_WORD && cmdBaseWorker.wOutCardUser != selfChairId
        //&& playNode.tipsArray.length==0  //playData.lastOutCardUser != null && playData.lastOutCardUser!=tableData.getUserWithUserId(selfdwUserID).wChairID
        ...
        {
            //自动出牌
            var needSelectedCardDatas = []
            for ( var i = 0; i < tipsArray.length; i++ )
            {
                if ( tipsArray[i].length == cmdBaseWorker.cbHandCardCount[selfChairId] )
                {
                    needSelectedCardDatas = tipsArray[i]
                    break
                }
            }

            if ( bSelfMax && needSelectedCardDatas.length == 0 )
            {

                var handCardDatas = cardLogic.sortWithNum(clone(cmdBaseWorker.cbHandCardData[selfChairId]))
                var cardsTypeArrray = gameLogic.getCardsTypesWithCardDatas(handCardDatas, cmdBaseWorker.cbMagicCardData)
                if ( cardsTypeArrray.length > 0 )
                    needSelectedCardDatas = cardsTypeArrray[0].cardDatas
            }

            if ( needSelectedCardDatas.length > 0 )
            {
                ...
                for(var i=0;i<cardSprs.length;i++)
                {
                    ...
                    for(var ii=0;ii<needSelectedCardDatas.length;ii++)
                    {
                        if( cardSpr.cardData == needSelectedCardDatas[ii] )
                        {
                            selectCardDatas.push(needSelectedCardDatas[ii])
                            cardSpr.y = cardSpr.originY + cardFactory.selectCardOffsetY
                            needSelectedCardDatas.splice(ii, 1)
                            break
                        }
                ...
                cocos.setTimeout(function() {playNode.outCardCallOut2(selectCardDatas)}, 500);
            }
        }
    },
