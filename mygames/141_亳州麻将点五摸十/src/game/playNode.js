...
//     playNode.popGameEnd(null, ['诶我去无群二', '多撒多群多群大所', '大大大大法师的法师大大大大法师的法师', '大大舒服撒发生发我的'])
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        if(direction==0)
        {
            ...
            currentRoundNode.discardMajiangsNode.y = -chairNodePosInMjTable.y + centerPosY - mjsDiscardHeight*0.5 - 43
    ...
    {
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
        if(cmdBaseWorker.wBankerUser!=INVALID_WORD)
        {
        playNode.leftcard.setVisible(true)
        ...
        playNode.showLaizi()
        // for(var j=0;j<4;j++){
        //     if(cmdBaseWorker.isListen[j]==1||cmdBaseWorker.isListen[j]==2){
        //         playNode.setTingIcon(j,true)
        //     }
        // }
    ...
    onCMD_OutCard:function() 
    {
        ...
        if(sortedActions.length>0)
            playNode.showActionBtns(sortedActions)
        // console.log(handMajiangs4D[0].[0])
    },
    onCMD_SendCard:function() 
    {
        ...
        for(var i=0;i<cmdBaseWorker.cbSendCardCount;i++)
        {
            ...
            // tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wOutCardUser) 
        ...
        else if(cmdBaseWorker.isListen[cmdBaseWorker.wTakeCardUser]==2&&isSelf)
            {
            //要出的牌是提供的牌cmdBaseWorker.cbProvideCardData
            playNode.autosendCard()
    ...
    onCMD_OperateResult:function() 
    {
        ...
        if(cmdBaseWorker.cbOperateCode != WIK_NULL)
        {
            ...
            majiangFactory.onActionResult(cmdBaseWorker.cbOperateCode, cardDatas, operateUser, provideUser,
             ...
             if(cmdBaseWorker.AnGangCard[0][0]>0||cmdBaseWorker.AnGangCard[1][0]>0||cmdBaseWorker.AnGangCard[2][0]>0||cmdBaseWorker.AnGangCard[3][0]>0)
        {
            playNode.onActionTingGang()
        }
        ...
        //动作后动作
        console.log("cmdBaseWorker.wTakeCardUser:"+cmdBaseWorker.wTakeCardUser)
        console.log("当前用户："+cmdBaseWorker.wCurrentUser)
        ...
        }else if(cmdBaseWorker.isListen[self.wChairID]==2&&isSelf&&cmdBaseWorker.wCurrentUser==self.wChairID&&cmdBaseWorker.cbOperateCode==WIK_NULL){
            playNode.autosendCard()
        }
    ...
    //显示听的标志 
    setTingIcon:function(wChairID)
    {   
        // playNode.tingIcon1.setVisible(false)
        if(wChairID == INVALID_WORD) return           
            var self = tableData.getUserWithUserId(selfdwUserID)
            var tingIcon = new cc.Sprite(resp.ting)
            tingIcon.setScale(2)
            if(self.wChairID==wChairID){
                playNode.tingIcon1.setVisible(true)
                
            }else if(self.wChairID==(wChairID+1)%GAME_PLAYER){
                playNode.tingIcon4.setVisible(true)
                
            }else if(self.wChairID==(wChairID+2)%GAME_PLAYER){
                playNode.tingIcon3.setVisible(true)
              
            }else if(self.wChairID==(wChairID+3)%GAME_PLAYER){
               playNode.tingIcon2.setVisible(true)
            }
            else{
                if(wChairID==0)
                    playNode.tingIcon1.setVisible(true)
    ...
    // 听之后所有人的暗杠都可以看到一张
     onActionTingGang:function()
  {


    for(var k=0;k<4;k++){
        if(cmdBaseWorker.AnGangCard[k][0]){
      var  operateUserObj=tableData.getUserWithChairId(k)
     var operateUser = k
     // cmdBaseWorker.wOperateUser==INVALID_WORD?cmdBaseWorker.wCurrentUser:cmdBaseWorker.wOperateUser
      var operateUserDir = tableData.getShowChairIdWithServerChairId(operateUserObj.wChairID)
    var operateWeaveMajiangs = playNode.weaveMajiangs4D[operateUserDir]  //被换的牌
            ...
            for(var i = 0;i<operateWeaveMajiangs.length;i++)
            {
                ...
                if(group.length == 4&&group[0].cardData==0)
                {
                    var cardData = cmdBaseWorker.AnGangCard[operateUser][cardDataNum]
                    cardDataNum++;
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            var id =tableData.getUserWithUserId(selfdwUserID).wChairID
            if(isAllowOut&&cmdBaseWorker.isListen[id]==1)
            {
                
                 
                for(var i =0;i<cmdBaseWorker.cbTingCardArray[id].length;i++)
                {
                    if(!cmdBaseWorker.isTingCard(majiang.cardData,cmdBaseWorker.cbTingCardArray[id]))
                        isAllowOut=false
                
                }             

            }
            if(cmdBaseWorker.isListen[id]==2){
                isAllowOut=false
            }
            if(isAllowOut)
            {
                ...
                for(var i=0;i<playNode.handMajiangs4D[0][0].length;i++)
                 {                                
                    playNode.handMajiangs4D[0][0][i].color =  cc.color(255,255,255)
                }
        ...
        if(tableData.getUserWithUserId(selfdwUserID).wChairID == tableData.getServerChairIdWithShowChairId(0))
        {
            touchEndCalls[0] = function(majiang)
            {
                ...
                var operateHandMajiangs4D = cmdBaseWorker.cbHandCardData[tableData.getUserWithUserId(selfdwUserID).wChairID]  //0为手牌 1是摸到的牌
    ...
    tingaction:function()
    {
        ...
        var id=tableData.getUserWithUserId(selfdwUserID).wChairID
        if(cmdBaseWorker.isListen[id]==1){
            // playNode.setTingIcon(id,true)

         for(var i=0;i<playNode.handMajiangs4D[0][0].length;i++)
         {
              if(!cmdBaseWorker.isTingCard(playNode.handMajiangs4D[0][0][i].cardData,cmdBaseWorker.cbTingCardArray[id]))
                   playNode.handMajiangs4D[0][0][i].color =  cc.color(100,100,100)
        }  
            if(!cmdBaseWorker.isTingCard(playNode.handMajiangs4D[0][1].cardData,cmdBaseWorker.cbTingCardArray[id]))
            {  
                 playNode.handMajiangs4D[0][1].color =  cc.color(100,100,100) 
            }
    ...
    autosendCard:function()
    {
        playNode.autosendAction=cc.sequence(
            cc.delayTime(1),
             cc.callFunc(function()
                {     
                  cmdBaseWorker.wCurrentUser= INVALID_WORD 
        ...
        playNode.node.runAction(playNode.autosendAction)
        return;
    ...
    {   
        ...
        if(cmdBaseWorker.isListen[self.wChairID]!=0){
            playNode.tingaction()
       
        }
        // if(cmdBaseWorker.isListen[self.wChairID]==2&&cmdBaseWorker.cbActionMask==0&&cmdBaseWorker.wCurrentUser==self.wChairID)
        //     playNode.autosendCard(cmdBaseWorker.cbProvideCardData)
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var j=0;j<4;j++){
            ...
            control['baoInfo'].setString(chrStr)

            }else if(cmdBaseWorker.endType==0||cmdBaseWorker.endType==1){
                control['baoInfo'].setString("")
            }
        }
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            if(cmdBaseWorker.endType==0||cmdBaseWorker.endType==1||cmdBaseWorker.dwChiHuKind[wChairID] != WIK_CHI_HU)
                var id=INVALID_WORD
            else if(cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU)
                var id=wChairID
           
            var mahjong = control['mahjong']
           ...
           if(id!=INVALID_WORD)
           {
            ...
            // majiangsNode.scale = 0.8 
            majiangsNode.x = 13
            majiangsNode.y = mahjong.height - 12 - 0.5*(majiangFactory.down_handHeight*majiangFactory.scale_upDown*majiangsNode.scale)
            // majiangsNode.y=mahjong.height
            mahjong.addChild(majiangsNode)

            //吃碰杠的牌
            var weaveItems = cmdBaseWorker.WeaveItemArray[id]
            var groupLen = 0 
            for(var groupIdx=0;groupIdx<weaveItems.length;groupIdx++)
            {
                ...
                for(var idxInGroup=0;idxInGroup<majiangsOneGroup.length;idxInGroup++)
                {
                    ...
                    if(idxInGroup==3){
                        mj.x=majiangsOneGroup[2].x-(45)
                        mj.y=10
                    }else {
                        mj.x=13+groupIdx*(45*3+25)+(45)*idxInGroup
                    }
            ...
            var startPos=13+(groupLen)*(25+45*3)
            //手牌
            var handCardDatas = cmdBaseWorker.cbHandCardData[id]
            ...
            for(var j=0;j<handCardDatas.length;j++)
            {
                ...
                {
                    majiang.setScaleX(0.9)
                    majiang.x=startPos+(handCardDatas.length-1)*45+25
            ...
            if(wChairID==cmdBaseWorker.wExitUser||cmdBaseWorker.endType==1)
            ...
            control['fenTTF'+wChairID].setString(cmdBaseWorker.lGameScore[wChairID]) 
            // control['name'+wChairID].setString(userData_gameEnd[wChairID].szNickName)
    ...
    resetPlayNode:function()
    {
        ...
        playNode.leftcard.setVisible(false)
        playNode.scoreTTF.setVisible(false)
        
        playNode.tingIcon1.setVisible(false)
        playNode.tingIcon2.setVisible(false)
        playNode.tingIcon3.setVisible(false)
        playNode.tingIcon4.setVisible(false)
