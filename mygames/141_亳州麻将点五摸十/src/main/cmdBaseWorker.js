...
{   
    ...
    {	
        console.log("游戏状态"+body)
        cmdBaseWorker.cbTingCardArray    = body.cbTingCardArray   
        cmdBaseWorker.isListen    = body.isListen 
        cmdBaseWorker.isZimoHu    = body.isZimoHu  
        cmdBaseWorker.AnGangCard    = body.AnGangCard    
    ...
    onCMD_GameStart:function(body) 
    {
        ...
        cmdBaseWorker.KaiJu             =body.KaiJu   
    ...
    isTingCard:function(cardData,cbTingCardArray)
    {
        for(var i=0;i<cbTingCardArray.length;i++)
        {
            if(cardData==cbTingCardArray[i])
                return true
        }
        return false

    },
    // isTing:function(isListen)
    ...
    sendMessage_peng:function(operateCards)
    {
        ...
        OperateCard.cbOperateCardData = operateCards
        socket.sendMessage(MDM_GF_GAME,SUB_C_OPERATE_CARD,OperateCard)
        console.log(operateCards+OperateCard.cbOperateCardCount)
    ...
    searchGangCardDatas:function(handMajiangs, weaveMajiangs) //自摸杠时找到杠哪个 服务器传? TODO
    {
        ...
        for(var i=0;i<handCardDatas.length;i++)
        {
            ...
            if(cmdBaseWorker.isListen[self.wChairID]==2&&cmdBaseWorker.cbProvideCardData!=cardData)
                {
                    isHas[0]=false
                }
            if(isHas[0])
    ...
    sortedOperateCardDatas2OperateCardDatas:function(provideCardData, sortedOperateCardDatas)
    {
        ...
        console.log("吃的牌："+operateCardDatas)
    }, 
