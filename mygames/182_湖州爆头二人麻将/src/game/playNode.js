...
{   
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        if (cmdBaseWorker.wOperateUser != INVALID_CHAIR)
            playNode.timer.switchTimer([playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wOperateUser))])
    ...
    onCMD_SendCard:function() 
    {
        ...
        playNode.scoreTTF.setString(cmdBaseWorker.cbLeftCardCount)
        playNode.timer.switchTimer(cmdBaseWorker.wCurrentUser==INVALID_WORD?[]:[playNode.reGetShowChairId( tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wCurrentUser) )])
        playNode.timer.switchTimer([playNode.reGetShowChairId( tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wTakeCardUser) )])
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            //var isAllowOut = (!MAGIC_CARD_ALLOWOUT) && cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID
            var isAllowOut = cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID
            playNode.OutCard = true 

            // if (cmdBaseWorker.MagicAction || cmdBaseWorker.MagicActionOther)
            // {
            //     if (cmdBaseWorker.PreviousCard != majiang.cardData)
            //     {    
            //         a = false
            //     }
            // }

            // if (cmdBaseWorker.ActionChiCardDate == majiang.cardData)
            //     a = false

            // if ((cmdBaseWorker.MagicAction[cmdBaseWorker.ForbidUserAction[0]] || cmdBaseWorker.MagicAction[cmdBaseWorker.ForbidUserAction[1]]) && (!(cmdBaseWorker.ForbidUserAction[0] == cmdBaseWorker.wCurrentUser) || !(cmdBaseWorker.ForbidUserAction[1] == cmdBaseWorker.wCurrentUser)))
            //     a = false
            for (var i = 0; i < cmdBaseWorker.ActionChiCardDate.length; i++)
            {
                ...
                {
                    playNode.OutCard = false
                    break
                }
            }

            if(isAllowOut && playNode.OutCard)
            {
                cmdBaseWorker.wCurrentUser = INVALID_WORD
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            for(var groupIdx=0;groupIdx<weaveItems.length;groupIdx++)
            {
                ...
                weaveItem.provideDirection = 0//这样就不显示箭头了      有问题
                var majiangsOneGroup = majiangFactory.weaveItem2Majiangs(groupIdx, 0, weaveItem, true, true)    // weaveItem结构体重没有provideDirection这个成员
            ...
            else if(wChairID==cmdBaseWorker.wProvideUser && cmdBaseWorker.endType != 1)
        ...
        for(var chairid=0;chairid<GAME_PLAYER;chairid++)
        {
            ...
            for(var i = 0;i<MAX_WEAVE;i++)
            {
                for(var j = 0;j<4;j++)
                {
                    ...
                    var cardIdx = weaveItems[i].cbCardDatas[j]    //修改
                    if (cardIdx>0) 
                    {
                        var aCard
    ...
    showActionBtns:function(sortedActions)
    {
        for(var i=sortedActions.length-1;i>=0;i--)
        {
            if(sortedActions[i] == WIK_REPLACE)
            {
                ...
                var handIdxs = ( handMajiangs[1]||isRecordScene )?[handMajiangs[1].cardData]:[]
