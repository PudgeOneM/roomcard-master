    ...
    onCMD_GameEnd:function() 
    {
        ...
        actions_campareHead[actions_campareHead.length] = cc.callFunc(
                ...
                {   
                      ...
                      var str = "";
                      if(baseScore>0)
                        str = "赢"+Math.abs(baseScore)+"家"
                      else if(baseScore<0)
                        str = "输"+Math.abs(baseScore)+"家"
                      else
                        str = "不输不赢"
                      var label = cc.LabelTTF.create(str, "Helvetica", 30)
                      label.x = 80
                      label.setFontFillColor( cc.color(236, 236, 0, 255) )
                      playNode.headScoreNode.addChild(label)
                })
        //中道动画
        callBodys.sort( function(a, b)
        {
        ...
        actions_campareCenter[actions_campareCenter.length] = cc.callFunc(
                ...
                {   
                      ...
                      label.x = 80
                      label.setFontFillColor( cc.color(236, 236, 0, 255) )
                      playNode.centerScoreNode.addChild(label)
                })
        //尾道动画
        callBodys.sort( function(a, b)
        {
        ...
        actions_campareTail[actions_campareTail.length] = cc.callFunc(
                ...
                {   
                    ...
                    label.x = 80
                    label.setFontFillColor( cc.color(236, 236, 0, 255) )
                    playNode.tailScoreNode.addChild(label)
                    ...
                    for(showid=1;showid<GAME_PLAYER;showid++)
                    {
                        ...
                        var score = cmdBaseWorker.lUserScore[chairid0][wChairID]
                        if(score != 0)
                    ...
                    {
                        var score = userScore[k]
                        var extraScorePng = score>=0?resp.nums5:resp.nums4
                        var extraScoreLabel = new ccui.TextAtlas(';:' + Math.abs(score) + '<', extraScorePng, 24.6, 32, "0")
