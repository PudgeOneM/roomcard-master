...
{   
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        if(direction==0)
        {
            ...
            // heapnode.x = currentRoundNode.heapMajiangsNode.x
            // heapnode.y = currentRoundNode.heapMajiangsNode.y
            // heapnode.LocalZOrder = currentRoundNode.heapMajiangsNode.LocalZOrder
            // heapnode.rotation = 90
        }
        else if(direction==2)
        {       
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        // get heapIdxsArray
        var heapIdxsArray = playNode.getHeapCardDatasArray(cmdBaseWorker.cbHeapCardInfo, cmdBaseWorker.TurnoverCard)
    ...
    onCMD_SendCard:function() 
    {
        ...
        console.log(cmdBaseWorker.wTakeCardUser)
        ...
        for(var i=0;i<cmdBaseWorker.cbSendCardCount;i++)
        {
            ...
            if(!heapMajiangsDir)
            {
                for(var k=0;k<4;k++)
                {
                    if(item.wHeapDir == (cmdBaseWorker.wBankerUser+k)%4)
                    {
                        heapMajiangsDir = k
                        break
                    }
        ...
        console.log(55555555553333333333,CHI_HU)
        if(sortedActions.length>0)
        {
        ...
        var touchEndCall = function(direction, majiang)
        {
           ...
           var isAllowOut = !(!MAGIC_CARD_ALLOWOUT && cmdBaseWorker.isMagicCard(majiang.cardData, cmdBaseWorker.cbMagicCardData) ) && cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID
                ...
                if ((cmdBaseWorker.cbAutocardUser[cmdBaseWorker.cbAutocard[0]] || cmdBaseWorker.cbAutocardUser[cmdBaseWorker.cbAutocard[1]]) && (!(cmdBaseWorker.cbAutocard[0] == cmdBaseWorker.wCurrentUser) || !(cmdBaseWorker.cbAutocard[1] == cmdBaseWorker.wCurrentUser)))
                 {
                    control = true
                    console.log(3232323,control)
                 }

                console.log(2222222222,control)
                ...
                if(isAllowOut && control == false)
                {
                    ...
                    console.log(OutCard.cbOutCardData)
                    socket.sendMessage(MDM_GF_GAME,SUB_C_OUT_CARD,OutCard)
                    playNode.hideActionBtns()  
    ...
    getHeapCardDatasArray:function(cbHeapCardInfo, TurnoverCard)
    {
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            //for(var i=0;i<4;i++)
            //{
            var temp = (cmdBaseWorker.wBankerUser+showChairid)%4
                
            //}
            var serverChairid = temp
    ...
    getHeapMajiangsArray:function(heapCardDatasArray)
    {
        ...
        for(showChairid=0;showChairid<4;showChairid++)//direction 0down 1right 2up 3left
        {
            var direction = showChairid
            ...
            for(var j=0;j<cardDatas.length;j++)
            {
                ...
                var pos = playNode.getHeapMajiangPosAndTag(j, direction)
                majiang.x = pos.x
                majiang.y = pos.y
    ...
    deleteHeapMajiangs:function(heapMajiangs, wSendHeapPosArray)
    {
        ...
        {
            if(heapMajiangs[wSendHeapPosArray[i]] != null)
    ...
    {   
        ...
        GAME_PLAYER = 3
        playNode.heapMajiangs4D = playNode.getHeapMajiangsArray(heapIdxsArray)
    ...
    getDisplayHandMajiangsArray:function(displayHandCardDatasArray)
    {
        ...
        for(showChairid=0;showChairid<GAME_PLAYER;showChairid++)//direction 0down 1right 2up 3left
        {
            // if (showChairid != 0 && showChairid != 1)
            //     continue
            var direction = showChairid
            direction = playNode.reGetShowChairId(direction)
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        console.log(2222222222222222,cmdBaseWorker.wWinner )
        //for(var chairId=0;chairId<GAME_PLAYER;chairId++)
        //{
        ...
        for(var showChairid=0;showChairid<GAME_PLAYER;showChairid++)
        {
            ...
            {
                console.log(handCardDatas.length)
            ...
            ///////////
            console.log(cmdBaseWorker.endType)
