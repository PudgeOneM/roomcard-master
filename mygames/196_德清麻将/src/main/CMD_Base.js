...
//游戏属性
var KIND_ID = 196								//游戏 I D
var GAME_NAME = "德清麻将"					//游戏名字

//组件属性
var SERVER_CHAIR = 4									//游戏人数
var SHOW_CHAIR = 4									//游戏人数
...
var MAX_FLOWER_COUNT = 20	//无花时设置为1 m_cbFlowerIndex[MAX_FLOWER_COUNT]不赋值即可   
var MAX_RIGHT_COUNT = 3	//最大权位DWORD个数 允许几个人同时胡(一炮多响)

var VAILID_REPERTORY = 144	//有效麻将数
...
//财神
var MAX_MAGIC_COUNT = 4//无财神时设置为1 m_cbMagicIndex[MAX_FLOWER_COUNT]不赋值即可    整副麻将里的最大财神麻将数 
var TURNOVER_COUNT_MAGIC = 0//无翻牌时设置为0 翻财神时翻的麻将数 		
var MAGIC_CARD_ALLOWOUT = false	

//其他
var MAX_SEND_COUNT = 20//摸到所有花补花 要发一样多的牌 MAX_FLOWER_COUNT											
var MAX_OPERATE_COUNT = 20//客户端执行动作时发给服务器的麻将最大数 如果有补花操作 可能出现摸到所有花补花 MAX_FLOWER_COUNT		
...
var CHR_HunYiSe = 0x00000020	//混一色  由同一种花色及风牌组成的胡牌 
...
// var CHR_QuanFengZi = 0x00000002	//全风字 14张全部由风组成的牌型（可以是乱牌） 
// var CHR_QiFengDao = 0x00000002	//七风倒 牌里包含7张不同风字（不可吃冲、可自摸） 
var CHR_QiXiaoDui = 0x00000200	//七小对 任意花色的七个对子组成 
var CHR_QiXiaoDuiHaoHua = 0x00000400	
var CHR_QiXiaoDuiShuangHaoHua = 0x00000800	
var CHR_QiXiaoDuiSanHaoHua = 0x00001000	
...
[
	['bool', 'bPlayerStatus', SERVER_CHAIR, 2],//杠后状态/听牌状态/财飘状态 这些状态可以在客户端标识出来
]
...
[
	...
	// ['BYTE', 'cbHandCardCount', SERVER_CHAIR],			//手上扑克数量
...
[
	['WORD', 'wCallUser'],						
	['BYTE', 'cbCallRecord', SERVER_CHAIR, 2],			//叫分信息
	['WORD', 'wCurrentUserCall'],						//当前玩家				
]
...
[
	...
	['BYTE', 'cbCallRecord', SERVER_CHAIR, 2],			//下跑 加顶分
]
...
[
	...
	['WORD', 'wProvideUser'],						//供应用户
	['BYTE', 'cbProvideCardData'],				//供应扑克
	['DWORD', 'dwChiHuKind', SERVER_CHAIR],			//胡牌类型
	['DWORD', 'dwChiHuRight', SERVER_CHAIR],		//胡牌类型

	['LONGLONG', 'lGameTaiCount', SERVER_CHAIR],			
	['LONGLONG', 'lGameScore', SERVER_CHAIR],				//游戏积分
	['BYTE', 'cbHandCardCount', SERVER_CHAIR],			//扑克数目
	['BYTE', 'cbHandCardData', SERVER_CHAIR, MAX_COUNT],	//扑克数据

	//游戏结束 直接重新传WeaveItemArray 
	//抢杠这类的客户端维护起来麻烦
	['BYTE', 'cbWeaveCount', SERVER_CHAIR],				//组合数目
	['tagWeaveItem', 'WeaveItemArray', SERVER_CHAIR, MAX_WEAVE],		//组合扑克
]
...
[
	...
	['BYTE', 'cbOutCardData'],						//出牌扑克
	['BYTE', 'cbDiscardCount', SERVER_CHAIR],			//丢弃数目
	['BYTE', 'cbDiscardCardData', SERVER_CHAIR, 60],	//丢弃记录
	...
	['BYTE', 'cbPlayerFlowerCount', SERVER_CHAIR],		//丢弃花牌数目
	['BYTE', 'cbPlayerFlowerCardData', SERVER_CHAIR, MAX_FLOWER_COUNT],		//丢弃花牌记录
