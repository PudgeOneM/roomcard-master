...
{   
    ...
    {   
        ...
        //抓鸟
        playNode.ruleCall1 = function()
        {    
            playNode.rule_box11.setVisible(false)   
            playNode.rule_box12.setVisible(true)   
            playNode.rule_box21.setVisible(true)   
            playNode.rule_box22.setVisible(false)   
        }

        //不抓
        playNode.ruleCall2 = function()
        {             
            playNode.rule_box11.setVisible(true)   
            playNode.rule_box12.setVisible(false)   
            playNode.rule_box21.setVisible(false)   
            playNode.rule_box22.setVisible(true)   
        }

        //不洗牌
        playNode.ruleCall3 = function()
        {             
            playNode.rule_box31.setVisible(false)   
            playNode.rule_box32.setVisible(true)   
            playNode.rule_box41.setVisible(true)   
            playNode.rule_box42.setVisible(false)  
        }

        //洗牌
        playNode.ruleCall4 = function()
        {             
            playNode.rule_box31.setVisible(true)   
            playNode.rule_box32.setVisible(false)   
            playNode.rule_box41.setVisible(false)   
            playNode.rule_box42.setVisible(true)  
        }

        //规则确定
        playNode.ruleSetCall = function()
        {
            var call = getObjWithStructName('CMD_C_RULE_SET')
            call.bZhuaNiao = playNode.rule_box12.isVisible()
            call.bRandCard = playNode.rule_box42.isVisible()
    
            socket.sendMessage(MDM_GF_GAME, SUB_C_RULE_SET, call)
            playNode.showRuleSetUi(false)
        }
    },
    ...
    showRuleSetUi:function(bVisible)
    {
        playNode.ruleSetNode.setVisible(bVisible)
    },
    ...
    updateOnFree:function()
    {
        ...
        playNode.updateRuleSet()
    },
    onCMD_StatusFree:function() 
    {
        cocos.setInterval(playNode.updateOnFree, 1000, playNode.node)

        if ( cmdBaseWorker.cbNiaoCard == INVALID_BYTE && tableData.createrUserID == selfdwUserID )
            playNode.showRuleSetUi(true)
    },
    ...
    {
        //鸟牌
        var cbNiaoCard = cmdBaseWorker.cbNiaoCard
        if( cbNiaoCard == 0 || cbNiaoCard == INVALID_BYTE )
        {
            playNode.NiaoCardNode.setVisible(false)
        }
        else
        {
            playNode.NiaoCardNode.removeAllChildren()
            var NiaoCard = cardFactory.getOne(cbNiaoCard, 0, 0)
            playNode.NiaoCardNode.addChild(NiaoCard)
            playNode.NiaoCardNode.setVisible(true)
        }

        //是否翻牌
        var bRandCard = cmdBaseWorker.bRandCard
        tableNode.randCardTTF.setVisible(!bRandCard)
    },
...
// ]
    ...
    _outCardCall1: function(cards) 
    {
        ...
        var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID;
        if (1 == cmdBaseWorker.cbPlayCount && 
            selfChairId == cmdBaseWorker.wBankerUser && 
            cmdBaseWorker.cbHandCardCount[selfChairId] == MAX_COUNT && 
            cards.indexOf(BLACK_3) == -1) 
        {
        ...
        var bSelfOut = cmdBaseWorker.wOutCardUser == INVALID_WORD || cmdBaseWorker.wOutCardUser == selfChairId 
        var CardType = gameLogic.GetCardType(cards, cmdBaseWorker.cbHandCardData[selfChairId], cmdBaseWorker.cbHandCardCount[selfChairId], bSelfOut)
        ...
        if ( !bSelfOut && !gameLogic.CompareCard(CardType, cmdBaseWorker.outCardType) )
        {
            showError()
            return
        }

        if ( playNode.FiltrateOutCardArray([cards]) == 0 )
        {
            showError()
            return
        }  
    ...
    {
        var cbHandTotalCount = cmdBaseWorker.cbHandCardCount[cmdBaseWorker.wOutCardUser] + cmdBaseWorker.cbOutCardCount
        var CardType = gameLogic.GetCardType(cmdBaseWorker.cbOutCardData, null, cbHandTotalCount, true)

        playNode.outCard_Show(cmdBaseWorker.wOutCardUser, CardType.cbCards)
    ...
    popOutCardNode: function(isCan) 
    {
        ...
        if (cmdBaseWorker.cbHandCardCount[selfChairId] < MAX_COUNT) 
        {
            ...
            if ( AnalyzeResult.cbFourCardData.length == 0 && !gameLogic.Has3ABomb(AnalyzeResult.cbThreeCardData) ) //炸弹不自动出
            {
                ...
                var bSelfOut = cmdBaseWorker.wOutCardUser == INVALID_WORD || cmdBaseWorker.wOutCardUser == selfChairId
                if ( 0 == autoOutCards.length && bSelfOut ) 
                { 
                    var cardType = gameLogic.GetCardType(handCardDatas, cmdBaseWorker.cbHandCardData[selfChairId], cmdBaseWorker.cbHandCardCount[selfChairId], bSelfOut)
                    if ( cardType.cbType != CT_ERROR )
