...
{   
    ...
    {   
        //嘉兴规则
        playNode.selectAbandonCall = function()
        {
            var select = getObjWithStructName('CMD_C_SELECT')
            select.isAbandon = 1
            socket.sendMessage(MDM_GF_GAME, SUB_C_SELECT, select)

            playNode.gameSelectNode.setVisible(false)
        }

        playNode.selectContinueCall = function()
        {
            var select = getObjWithStructName('CMD_C_SELECT')
            select.isAbandon = 2
            socket.sendMessage(MDM_GF_GAME, SUB_C_SELECT, select)
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        playNode.showLaizi()
        ///////////嘉兴规则  开始///////////
        if(cmdBaseWorker.cbIsNeedChengBao && !cmdBaseWorker.cbAlreadySelect)
        {
            var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID            
            if(selfChairId!=cmdBaseWorker.cwUserChengBao[4] && selfChairId!=cmdBaseWorker.cwUserChengBao[5] && selfChairId != INVALID_WORD)
            {
                if(cmdBaseWorker.cbSelectResult[selfChairId] == 0)
                    playNode.gameSelectNode.setVisible(true)
            }
        }   
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            if(cmdBaseWorker.cbSelectResult[i] == 1)
            {
                var user = tableData.getUserWithTableIdAndChairId(self.wTableID, i)
                user.userNodeInsetChair.currentRoundNode.upTTF.setString('放弃')
            } 
        }
    ...
    onCMD_SendCard:function() 
    {
        ...
        //选择放弃游戏的玩家，直接托管
        var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID
        if(cmdBaseWorker.cbSelectResult[selfChairId]==1 && cmdBaseWorker.wCurrentUser == selfChairId){
            setTimeout(function(){
                cmdBaseWorker.wCurrentUser = INVALID_WORD
                var OutCard = getObjWithStructName('CMD_C_OutCard')
                OutCard.cbOutCardData = cmdBaseWorker.sendCardArray[0].cbCardData
                socket.sendMessage(MDM_GF_GAME,SUB_C_OUT_CARD,OutCard),1200} )            
        }else if(cmdBaseWorker.cbIsNeedChengBao && !cmdBaseWorker.cbAlreadySelect){
                ///////////////////////
        }else{
            var sortedActions = majiangLogic.getSortedActionsWithMask(cmdBaseWorker.cbActionMask)
            if(sortedActions.length>0)
    ...
    onCMD_OperateResult:function() 
    {
        ...
        {
            var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID
            if(selfChairId != cmdBaseWorker.wOperateUser && selfChairId != cmdBaseWorker.wProvideUser && selfChairId != INVALID_WORD)
                playNode.gameSelectNode.setVisible(true)  
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            if((cmdBaseWorker.cbIsNeedChengBao == 0) || (cmdBaseWorker.cbAlreadySelect==1) )
            {
                if(isAllowOut)
                {
                    cmdBaseWorker.wCurrentUser = INVALID_WORD
    ...
    popGameEnd:function(continueCall, szNickName_gameEnd)
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            if(cmdBaseWorker.cbIsNeedChengBao)
            {
                if((cmdBaseWorker.cbSelectResult[i] != 1 && cmdBaseWorker.lGameScore[i]<0) ) 
                {
                    var t = new cc.Sprite(resp.chengbao)
                    t.setScale(0.5)
                    var t_pointX = control['score'+i].getPosition().x
                    t.setPositionX(t_pointX +  t.getContentSize().width*0.8)
