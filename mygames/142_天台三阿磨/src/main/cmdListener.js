        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                ...
                {  
                 ...
                 case SUB_S_SELECT_MOPAI: 
                {  
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_SelectMoPai') 
                    cmdBaseWorker.onCMD_SelectMoPai(body)
                    if(body)
                    {
                        gameLog.log('body:' + head.CommandInfo.wMainCmdID + '-' + head.CommandInfo.wSubCmdID, body)
                    }
                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_SelectMoPai'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                 case SUB_S_UPDATE_DATA: 
                {  
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_UpdataData') 
                    cmdBaseWorker.onCMD_UpdataData(body)
                    if(body)
                    {
                        gameLog.log('body:' + head.CommandInfo.wMainCmdID + '-' + head.CommandInfo.wSubCmdID, body)
                    }
                    playNode.onCMD_UpdataData()
                    break
                }
                case SUB_S_GAME_END: 
                {   
                ...
                case SUB_S_PINGCUO_RESULT: 
                {  
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_Ping_Result') 
                    cmdBaseWorker.onCMD_PingCuoResult(body)
                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_Ping_Result'])
                    cc.eventManager.dispatchEvent(event)
                ...
                case SUB_S_SENDLOG: 
                {  
                   sendLogToServer(gameLog.logS + 'wtms142 wtms');
                ...
                {  
                    ...
                    cmdBaseWorker.onCMD_OperateNotify(body)
                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_OperateNotify'])
                    cc.eventManager.dispatchEvent(event)
                    if(body)
                    {
                        gameLog.log('body:' + head.CommandInfo.wMainCmdID + '-' + head.CommandInfo.wSubCmdID, body)
