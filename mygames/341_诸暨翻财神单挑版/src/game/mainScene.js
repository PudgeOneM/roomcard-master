...
var mainScene = {
	runThisScene:function()
	{
      ...
      //跑马灯
      infoBar.create("1、当牌剩70张时为流局。2、计分方式：以平庄自摸或点炮1台为例，平庄1台记2分，以此类推，30分封顶。3、没有清一色三摊承包。", 30, 30);
  },
  ...
  //游戏开始
	onSubGameStart:function(data)
	{
      ...
      mainScene._wMeChaird       = gameLogic.getMeChaird();
  		...
  		mainScene._cbLeftCardCount = 108;
      mainScene._cbCardData      = [];
      if(mainScene._wMeChaird != INVALID_CHAIR)
      {
  ...
  setUserMahJong:function()
  {
      ...
      if (isRecordScene) 
      {
          ...
          for(var i = 0; i < GAME_PLAYER; i++)
          {
              cbRecordData[gameLogic.switchViewChairID(i)] = mainScene._cbRecordData[i].concat();
  ...
  onSubOutCard:function(data)
  {
      ...
      var str = ' onSubOutCard: ' + ' wOutCardUser: ' + data.wOutCardUser + ' cbOutCardData: ' + data.cbOutCardData;
      handMahJong.handDataExcEx(str, handMahJong.cbCardData, handMahJong.cbCardCount);
      return true;
  },
  //发牌
  onSubSendCard:function(data)
  {
      ...
      var str = ' onSubSendCard: ' + ' wCurrentUser: ' + mainScene._wCurrentUser + ' cbCardData: ' + data.cbCardData;
      handMahJong.handDataExcEx(str, handMahJong.cbCardData, handMahJong.cbCardCount);
  },
  //有玩家操作
  onSubOperateNotify:function(data)
  { 
      var str = ' onSubOperateNotify: ' + ' cbActionMask: ' + data.cbActionMask + ' cbActionCard: ' + data.cbActionCard;
      handMahJong.handDataExcEx(str, handMahJong.cbCardData, handMahJong.cbCardCount);
      if(!mainScene._bSiceAniEnd) mainScene.hideSice();
      //用户界面
  ...
  {
      var str = ' onSubOperateResult1: ' + ' wOperateUser: ' + pOperateResult.wOperateUser + ' cbOperateCode: ' + pOperateResult.cbOperateCode
       + ' cbOperateCard: ' + pOperateResult.cbOperateCard[0] + " " + pOperateResult.cbOperateCard[1] +  " " +
       pOperateResult.cbOperateCard[2] + " " + pOperateResult.cbOperateCard[3];
      handMahJong.handDataExcEx(str, handMahJong.cbCardData, handMahJong.cbCardCount);
      //变量定义
      operateBtn.hideOperate();
      mainScene._cbActionMask = WIK_NULL;
