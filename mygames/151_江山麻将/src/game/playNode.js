...
{   
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        if(direction==0)
        {
            ...
            currentRoundNode.playerStatusNode.setPosition( cc.p(45, 20) )  
            //discard
    ...
    {   
        ...
        playNode.actionCall_gang = function()
        {
            ...
            else
            {
                ...
                var cardDatas = cmdBaseWorker.searchGangCardDatas(playNode.handMajiangs4D[direction], playNode.weaveMajiangs4D[direction],selfChairId)
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            cbPlayerFlowerCardData[direction] = cmdBaseWorker.cbPlayerFlowerCardData[i]
        }
        // console.log(6666, handCardDatasArray, discardCardDatasArray, weaveItemArray)
        for(var i = 0;i<handCardDatasArray[0][0].length;i++)
        {
            var cardData = handCardDatasArray[0][0][i]

            if(cardData == cmdBaseWorker.m_RightCardData && handCardDatasArray[0][1] != null)
            {
                handCardDatasArray[0][0][i] = handCardDatasArray[0][1]
                handCardDatasArray[0][1] = cardData
                break
            }
        }
    ...
    onCMD_SendCard:function() 
    {
        ...
        else if(isSelf && cmdBaseWorker.wOutCardState[cmdBaseWorker.wTakeCardUser] >= 1)
        {
            playNode.Auto_card(cmdBaseWorker.cbProvideCardData)
        }
    },
    Auto_card:function(provideCardData){
        var operateHandMajiangs4D = playNode.handMajiangs4D[0][0]

        for(var i = 0 ;i<operateHandMajiangs4D.length;i++)
        {
            operateHandMajiangs4D[i].color = cc.color(100,100,100)
        }
    ...
    onCMD_OperateResult:function() 
    {
        ...
        {
            if(cmdBaseWorker.wOutCardState[cmdBaseWorker.wOperateUser] >= 1)
                    playNode.Auto_card(cmdBaseWorker.cbProvideCardData)
        }
    },
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            if(cmdBaseWorker.wOutCardState[User.wChairID] >= 1)
            {
                console.log("dsadasdas")
                if(majiang != playNode.handMajiangs4D[0][1])
                    isAllowOut = false
    ...
    {   
        ...
        if(cmdBaseWorker.wOutCardState[self.wChairID] >= 1 && tableData.getUserWithUserId(selfdwUserID).wChairID == cmdBaseWorker.wCurrentUser)
        {
            playNode.Auto_card(cmdBaseWorker.m_RightCardData)
        }

        for(var showChairid=0;showChairid<4;showChairid++)
        {
            var direction = showChairid
            //tableData.getShowChairIdWithServerChairId(i)
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            for (var i = 0; i < map_mask2Name.length; i++) 
            {
                ...
                if (cmdBaseWorker.dwChiHuRight[wChairID] & chr_type)
                {
                    ...
                    /*if(i == 11 && cmdBaseWorker.lGameTaiCount[wChairID] != 0)
                        chrStr+= '*' + cmdBaseWorker.lGameTaiCount[wChairID]*/
                }
            }
