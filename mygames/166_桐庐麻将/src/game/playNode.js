...
{   
    ...
    {   
        ...
        playNode.actionCall_chi = function()
        {
            ...
            var provideIdx = cmdBaseWorker.cbProvideCardData//cmdBaseWorker.cbProvideCardData
        ...
        playNode.actionCall_gang = function()
        {
            ...
            {
                console.log(11111111111,sortedOperateIdxs)
    ...
    onCMD_GameStart:function() 
    {
        ...
        {
            tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
            var siceNum1 = cmdBaseWorker.cbSiceCount[0]//getRandNum(Math.max(cmdBaseWorker.cbSiceCount-6, 1), Math.min(cmdBaseWorker.cbSiceCount-1, 6))
            var siceNum2 = cmdBaseWorker.cbSiceCount[1]//cmdBaseWorker.cbSiceCount - siceNum1
    ...
    onCMD_SendCard:function() 
    {
        ...
        var change = ( (self==cmdBaseWorker.wForbidActionUser[0] || !cmdBaseWorker.bIsOutMagic[cmdBaseWorker.wForbidActionUser[0]]) && !(cmdBaseWorker.wForbidActionUser[0]==cmdBaseWorker.wForbidActionUser[1]?false:cmdBaseWorker.bIsOutMagic[cmdBaseWorker.wForbidActionUser[1]]) )
        // console.log(111111111,self,cmdBaseWorker.wForbidActionUser,cmdBaseWorker.bIsOutMagic)
        // console.log(222222222,change)
        if (!change)
        {
            var majiang = playNode.handMajiangs4D[outDir]
            for(var id = 0; id<majiang[0].length;id++)
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            for (var i = 0; i < cmdBaseWorker.cbForbidEatAndOutCardData.length; i++)
            {
            ...
            if(isAllowOut)
            {
                ...
                socket.sendMessage(MDM_GF_GAME,SUB_C_OUT_CARD,OutCard)
                //cmdBaseWorker.sendMessage_gang([majiang.cardData,majiang.cardData,majiang.cardData,majiang.cardData])
                playNode.hideActionBtns()
               // playNode.hideActionBtns()
            }
        }
            ...
            case 1:
            {
                ...
                var listener = cc.EventListener.create({
                    ...
                    onTouchBegan: function (touch, event) {
                        ...
                        var isSelf = cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID
                        var self = tableData.getUserWithUserId(selfdwUserID)
                        var isopen = ( (self.wChairID==cmdBaseWorker.wForbidActionUser[0] || !cmdBaseWorker.bIsOutMagic[cmdBaseWorker.wForbidActionUser[0]]) && !(cmdBaseWorker.wForbidActionUser[0]==cmdBaseWorker.wForbidActionUser[1]?false:cmdBaseWorker.bIsOutMagic[cmdBaseWorker.wForbidActionUser[1]]) )
    ...
    getHandMajiangsArray:function(handCardDatasArray, isLookon)
    {
        ...
        for(showChairid=0;showChairid<GAME_PLAYER;showChairid++)//direction 0down 1right 2up 3left
        {
            ...
            for(var j=0;j<oldHandCardDatas.length;j++)
            {
                ...
                //杭州麻将    重连后将不能出的的牌变色
                var self = tableData.getUserWithUserId(selfdwUserID)
                var change = ( (self.wChairID==cmdBaseWorker.wForbidActionUser[0] || !cmdBaseWorker.bIsOutMagic[cmdBaseWorker.wForbidActionUser[0]]) && !(cmdBaseWorker.wForbidActionUser[0]==cmdBaseWorker.wForbidActionUser[1]?false:cmdBaseWorker.bIsOutMagic[cmdBaseWorker.wForbidActionUser[1]]) )
        ...
        else
        {
            ...
            for (var i = 0; i < map_mask2Name.length; i++) 
            {
                ...
                if (cmdBaseWorker.dwChiHuRight[cmdBaseWorker.wWinner] & chr_type)
                {
                    ...
                    if (map_mask2Name[i][1] == "飘财" && cmdBaseWorker.cbMagicCount[cmdBaseWorker.wWinner]!=0)
                        chrStr += '*' + cmdBaseWorker.cbMagicCount[cmdBaseWorker.wWinner]
                    chrStr += ' '
                }
            }
            if (cmdBaseWorker.cbFengNum[cmdBaseWorker.wWinner] != INVALID_CARD_DATA && cmdBaseWorker.cbFengNum[cmdBaseWorker.wWinner]!=0)
                chrStr += "风*" + cmdBaseWorker.cbFengNum[cmdBaseWorker.wWinner] + ' '
            if (cmdBaseWorker.cbGangCount[cmdBaseWorker.wWinner]!=0)
                chrStr += "杠*" + cmdBaseWorker.cbGangCount[cmdBaseWorker.wWinner] + ' '
