...
majiangFactory._gethandMajiangsListener = function(majiangs, parent, touchEndCall)
{
    ...
    var listener = cc.EventListener.create({
        ...
        onTouchBegan: function (touch, event) {
            ...
            //gameLog.log(111111111, target, locationInNode, touchedMj, isSelf)
            if (!isopen&&locationInNode.x<(majiangs[0].length*83*1) && isSelf)
                return false
        ...
        onTouchMoved: function (touch, event) {
            ...
            var self = tableData.getUserWithUserId(selfdwUserID)
            var isopen = ( (self.wChairID==cmdBaseWorker.ForbidUserAction[0] || !cmdBaseWorker.MagicAction[cmdBaseWorker.ForbidUserAction[0]]) && !(cmdBaseWorker.ForbidUserAction[0]==cmdBaseWorker.ForbidUserAction[1]?false:cmdBaseWorker.MagicAction[cmdBaseWorker.ForbidUserAction[1]]) )
            if (!isopen&&locationInNode.x<(majiangs[0].length*83*1.010) && isSelf)
                return false
