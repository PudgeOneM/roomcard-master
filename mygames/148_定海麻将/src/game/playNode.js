...
//     playNode.popGameEnd(null, ['诶我去无群二', '多撒多群多群大所', '大大大大法师的法师大大大大法师的法师', '大大舒服撒发生发我的'])
    ...
    onCMD_StatusFree:function()  //空闲状态
    {
        cocos.setInterval(playNode.updateOnFree, 1000, playNode.node)
    },
    onCMD_StatusCall:function()   //加顶分  断线重连
    {
    ...
    onCMD_StatusPlay:function()    //游戏进行
    {
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
    ...
    onCMD_CallResult:function(body)   //叫分结果  点击确定调用
    {     
        var callUser = tableData.getUserWithChairId(cmdBaseWorker.wCallUser)
        chairFactory.hideFiredCircle.call(callUser.userNodeInsetChair)      //点击确定的玩家不显示倒计时火焰
        callUser.userNodeInsetChair.currentRoundNode.upTTF.setString( '下跑:'+cmdBaseWorker.cbCallRecord[cmdBaseWorker.wCallUser][0]+' 加顶:' + cmdBaseWorker.cbCallRecord[cmdBaseWorker.wCallUser][1] ) //点击确定的玩家显示

        if(cmdBaseWorker.wCurrentUserCall!=INVALID_WORD)  //下一个玩家火焰圈圈
        {
            var currentUser = tableData.getUserWithChairId(cmdBaseWorker.wCurrentUserCall)
            chairFactory.showFiredCircle.call(currentUser.userNodeInsetChair, 20)
        }   
    ...
    onCMD_OperateResult:function()   //操作命令
    {
        playNode.hideActionBtns()
