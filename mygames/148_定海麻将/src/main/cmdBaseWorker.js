...
{   
    ...
    onCMD_StatusFree:function(body)   //空闲状态
    {
        cmdBaseWorker.lCellScore = body.lCellScore
    },
    onCMD_StatusCall:function(body)   //加顶分  断线重连
    {
        cmdBaseWorker.lCellScore = body.lCellScore
    ...
    onCMD_StatusPlay:function(body)   //游戏进行  //断线重连
    {	
        cmdBaseWorker.cbActionMask    = body.cbActionMask   //动作掩码
        cmdBaseWorker.wProvideUser    = body.wProvideUser   //供应用户
        cmdBaseWorker.cbProvideCardData    = body.cbProvideCardData  //动作扑克 

        cmdBaseWorker.wBankerUser     = tableData.getUserWithUserId(body.dwBankerUserId).wChairID   
        cmdBaseWorker.lCellScore      = body.lCellScore      //单元积分
        cmdBaseWorker.wCurrentUser    = body.wCurrentUser    //当前需要出牌用户
 
        cmdBaseWorker.cbLeftCardCount = body.cbLeftCardCount //剩余数目
        cmdBaseWorker.wOutCardUser    = body.wOutCardUser    //出牌完成用户
        cmdBaseWorker.cbOutCardData   = body.cbOutCardData   //出牌扑克
        cmdBaseWorker.cbDiscardCount  = body.cbDiscardCount  //丢弃数目
        cmdBaseWorker.cbDiscardCardData   = body.cbDiscardCardData   //丢弃记录
        cmdBaseWorker.cbHandCardCount = body.cbHandCardCount     //扑克数目
        cmdBaseWorker.cbHandCardData = [[],[],[],[]]            //扑克列表
        ...
        cmdBaseWorker.cbWeaveCount    = body.cbWeaveCount    //组合数目
        cmdBaseWorker.WeaveItemArray  = body.WeaveItemArray  //组合扑克

        cmdBaseWorker.cbPlayerFlowerCount    = body.cbPlayerFlowerCount   //丢弃花牌数目
        for(var i=0;i<GAME_PLAYER;i++)
        {   
            cmdBaseWorker.cbPlayerFlowerCardData[i]  = body.cbPlayerFlowerCardData[i].slice(0, body.cbPlayerFlowerCount[i])  
        }

        cmdBaseWorker.cbMagicCardData  = body.cbMagicCardData  //财神变量
        cmdBaseWorker.cbFlowerCardData    = body.cbFlowerCardData  //

        cmdBaseWorker.wHeapHead     = body.wHeapHead   //堆立头部
        cmdBaseWorker.wHeapTail     = body.wHeapTail   //堆立尾部
        cmdBaseWorker.cbHeapCardInfo     = body.cbHeapCardInfo  //堆牌信息
        cmdBaseWorker.TurnoverCard     = body.TurnoverCard   

        cmdBaseWorker.cbCallRecord = body.cbCallRecord  //下跑 加顶分
    ...
    onCMD_CallNotify:function(body)   //叫分通知？
    {        
        cmdBaseWorker.wBankerUser       = tableData.getUserWithUserId(body.dwBankerUserId).wChairID    
        cmdBaseWorker.wCurrentUserCall = body.wCurrentUserCall
    },
    onCMD_CallResult:function(body)  //叫分结果
    {     
        cmdBaseWorker.wCallUser = body.wCallUser
        cmdBaseWorker.cbCallRecord = body.cbCallRecord
        cmdBaseWorker.wCurrentUserCall = body.wCurrentUserCall
    },
    onCMD_GameStart:function(body)   //游戏开始
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            {
                cmdBaseWorker.cbHandCardData[i][j] = isSelf?body.cbHandCardData[j]:0   //麻将列表
            }
        }
    ...
    onCMD_OutCard:function(body)   //出牌命令
    {
    ...
    onCMD_SendCard:function(body)   //发送扑克
    {        
        cmdBaseWorker.cbActionMask    = body.cbActionMask   
    ...
    onCMD_OperateResult:function(body)   //操作命令
    {
        cmdBaseWorker.cbActionMask    = body.cbActionMask   
        cmdBaseWorker.wProvideUser    = body.wProvideUser   
        cmdBaseWorker.cbProvideCardData    = body.cbProvideCardData    
    ...
    onCMD_GameEnd:function(body)    //游戏结束
    {
