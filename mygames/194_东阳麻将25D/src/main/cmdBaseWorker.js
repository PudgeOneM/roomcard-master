...
{   
    ...
    wNotifyOperateUser:INVALID_WORD,
    ...
    onReStart:function()
    {
        ...
        cmdBaseWorker.wNotifyOperateUser= INVALID_WORD
    ...
    onCMD_GameStart:function(body) 
    {
        ...
        {
            var serverChairid = tableData.getServerChairIdWithShowChairId(wChairID)
            if (cmdBaseWorker.wBankerUser == serverChairid)
            {
                cmdBaseWorker.cbHandCardData[serverChairid] = body.cbHandCardData[serverChairid].slice(0, MAX_COUNT)
                cmdBaseWorker.cbHandCardCount[serverChairid] = MAX_COUNT
            }
            else
            {
                cmdBaseWorker.cbHandCardData[serverChairid] = body.cbHandCardData[serverChairid].slice(0, MAX_COUNT-1)
                cmdBaseWorker.cbHandCardCount[serverChairid] = MAX_COUNT-1
            }
        }
        // - - - - - - - - - - - - - - - - - - - - - - - - - - -
		cmdBaseWorker.cbMagicCardData   = body.cbMagicCardData      
    ...
    {
        cmdBaseWorker.wNotifyOperateUser = body.wOperateUser
    ...
    onCMD_GameEnd:function(body) 
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            var weaveItems = cmdBaseWorker.WeaveItemArray[i]
            //组合牌
            for(var j=0;j<weaveItems.length;j++)
            {
                var t = weaveItems[j].cbCardData || weaveItems[j].cbCardDatas
                for(var jj=0;jj<t.length;jj++)
                {
