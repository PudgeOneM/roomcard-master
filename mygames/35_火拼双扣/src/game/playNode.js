...
{   
    ...
    adaptUi:function()
    {
        if(GAME_PLAYER == 4)
        {
            ...
            tableNode.chairNode0.y = cardFactory.handGroupNodeHeight + cardFactory.selectCardOffsetY + 0.5*cardFactory.down_outHeight*cardFactory.scale_out + 15
    ...
    initCurrentRoundNode:function(currentRoundNode)
    {
        ...
        //组队牌精灵展示的父节点
        currentRoundNode.turnoverCardNode = new cc.Node()
        currentRoundNode.addChild(currentRoundNode.turnoverCardNode)
    ...
    {           
        ...
        currentRoundNode.turnoverCardNode.removeAllChildren()
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        if(direction==0)
        {
            ...
            //turnoverCardNode
            currentRoundNode.turnoverCardNode.x = currentRoundNode.outCardsNode.x - 200
            currentRoundNode.turnoverCardNode.y = currentRoundNode.outCardsNode.y
        }
        else if(direction==2)
        {
            //out
            ...
            //turnoverCardNode
            currentRoundNode.turnoverCardNode.x = currentRoundNode.outCardsNode.x + 250
            currentRoundNode.turnoverCardNode.y = currentRoundNode.outCardsNode.y
        } 
        else if(direction==1)
        { 
            ...
            //turnoverCardNode
            currentRoundNode.turnoverCardNode.x = currentRoundNode.outCardsNode.x + 150
            currentRoundNode.turnoverCardNode.y = currentRoundNode.outCardsNode.y
        }
        else if(direction==3)
        {   
            ...
            //turnoverCardNode
            currentRoundNode.turnoverCardNode.x = currentRoundNode.outCardsNode.x + 50 
            currentRoundNode.turnoverCardNode.y = currentRoundNode.outCardsNode.y
        } 
    ...
    onCMD_ChairChange:function()
    {
        // tableData.showChairId2ServerChairIdMap[0] = 1
        // tableData.showChairId2ServerChairIdMap[1] = 0

        // tableNode._onSelfChangeChair()
        // console.log("onCMD_ChairChange")
    },
    ...
    onCMD_StatusPlay:function() 
    {
       ...
       playNode.updateTurnoverCard(cmdBaseWorker.cbShowCardData ) 
       console.log(tableData.showChairId2ServerChairIdMap)

       //------------------换座位------------------
       var self =  tableData.getUserWithUserId(selfdwUserID)
       var startwChairID = self.wChairID == INVALID_WORD?0:self.wChairID 
        for(var i = 0;i<GAME_PLAYER;i++)
        {
            var index = (startwChairID + i) % GAME_PLAYER
            tableData.showChairId2ServerChairIdMap[index] = i
        }
        ...
        for(var i= 0 ;i<GAME_PLAYER;i++)
        {
            if(cmdBaseWorker.wGamePlayer[i] == startwChairID)
            {
                for(var j =0;j<GAME_PLAYER;j++)
                {
                    var index = ( j + i ) % GAME_PLAYER
                    tableData.showChairId2ServerChairIdMap[j] = cmdBaseWorker.wGamePlayer[index] 
                }
       ...
       //playNode.updatePlayerChair(cmdBaseWorker.wGamePlayer,false)
       playNode.resetScoreInfo()


       var firendChairId = cmdBaseWorker.wFriendUser[cmdBaseWorker.wBankerUser]
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true,'#firendIcon.png')
        tableNode.setBankerIcon(firendChairId, true,'#firendIcon.png')
        ...
        playNode.updateScoreInfo(cmdBaseWorker.lOperateScore)
    ...
    onCMD_CallNotify:function()
    {
        ...
        for(var i= 0 ;i<GAME_PLAYER;i++)
        {
            if(cmdBaseWorker.wNewGamePlayeIndex[i] == startwChairID)
            {
                for(var j =0;j<GAME_PLAYER;j++)
                {
                    var index = ( j + i ) % GAME_PLAYER
                    tableData.showChairId2ServerChairIdMap[j] = cmdBaseWorker.wNewGamePlayeIndex[index] 
                }
        ...
        //playNode.updatePlayerChair(cmdBaseWorker.wFriendUser,true)
        playNode.showTurnoverCard()
        playNode.updateTurnoverCard(cmdBaseWorker.cbShowCardData ) 
    ...
    onCMD_GameStart:function() 
    {
       ...
       // playNode.showDiCard()
        ...
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true,'#firendIcon.png')
        tableNode.setBankerIcon(cmdBaseWorker.wFriendUser[cmdBaseWorker.wBankerUser], true,'#firendIcon.png')
        ...
        playNode.showTurnoverCardAnimate(cmdBaseWorker.cbShowCardData,cmdBaseWorker.wFriendUser)
    ...
    onCMD_OutCard:function() 
    {
        ...
        if(cardsType.typeLevel>1)
        {
            ...
            playNode.updateScoreInfo(cmdBaseWorker.lOperateScore)
            playNode.showScoreAnimation(cmdBaseWorker.cbOutCardScore)
...
// ]
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var wChairID=0;wChairID<GAME_PLAYER;wChairID++)
        {
            ...
            else if(cmdBaseWorker.cbWinType[wChairID] == 4)
                control['winflag' + wChairID].setSpriteFrame('gendFlag1.png')
            else if(cmdBaseWorker.cbWinType[wChairID] == 2)
                control['winflag' + wChairID].setSpriteFrame('gendFlag2.png')
            else if(cmdBaseWorker.cbWinType[wChairID] == 1)
                control['winflag' + wChairID].setSpriteFrame('gendFlag3.png')
            else
                control['winflag'+wChairID].setSpriteFrame('empty.png') 
  
            control['fenTTF'+wChairID].setString((cmdBaseWorker.lShowGameScore[wChairID]>0?'+':'') + cmdBaseWorker.lShowGameScore[wChairID]) 
            control['taiTTF'+wChairID].setString((cmdBaseWorker.lShowGameScore[wChairID] - cmdBaseWorker.lGameScore[wChairID])) 

            
            //
            var boomKind = ['五星','六星','七星','八星','九星','十星','十一星']
            var resultTTF = cc.LabelTTF.create('', "Helvetica", 16)
            resultTTF.setFontFillColor( cc.color(244, 230, 159) )
            var str = ''
            for(var boomIndex = 0;boomIndex<cmdBaseWorker.cbOutBombCount[wChairID].length;boomIndex++)
            {
                if(cmdBaseWorker.cbOutBombCount[wChairID][boomIndex] == 0)
                    continue

                str += boomKind[boomIndex] + '(' + cmdBaseWorker.cbOutBombCount[wChairID][boomIndex] + ')'
                str += ' '

            }

            resultTTF.setString(str) //结算界面 手牌下面的字
            resultTTF.anchorX = 0
            resultTTF.x = 102
            resultTTF.y = 20
    ...
    popOutCardNode:function(isCan)
    {
        ...
        if(tipsArray != null )
        {
            var len = tipsArray.length
            for(var i = 0,count = 0;count < len;++i,++count)
            {
                var array = []
                for(var j = 0;j < tipsArray[i].length;++j)
                {
                    if(tipsArray[i][j] == 79)
                    {
                        array = tipsArray.splice(i,1)
                        i--
                        break
                    }
                }
                ...
                if(array.length != 0)
                {
                    tipsArray[tipsArray.length] = array[0]
                }
        ...
        if(cmdBaseWorker.outCardType.typeLevel == 18)
    ...
    ///////////////playEffect start////////
    updatePlayerChair:function(wCahirIdArray,isStart)
    {
        var user = tableData.getUserWithUserId(selfdwUserID)

        var exchangeChair = function(wCahirIdArray,self)
        {
            
            for(var i = 0;i<GAME_PLAYER;i++)
            {
                var chairUser = tableData.getUserWithChairId(i)
                if(chairUser == undefined)
                    return
                ...
                if(i == self.wChairID)
                {
                    ...
                    for(var j = 0;j<GAME_PLAYER;j++)
                    {
                        tableData.showChairId2ServerChairIdMap[j] = beg
                        beg = (beg + 1)%GAME_PLAYER
                    }
                }
            }
            tableNode._onSelfSitChair()
            ...
            var firendChairId = wCahirIdArray[self.wChairID]
            console.log("wCahirIdArray " + wCahirIdArray)
            console.log("firendChairId " + firendChairId)
            console.log("self.wChairID " + self.wChairID)

            for(var i= 1 ;i<GAME_PLAYER;i++)
            {
                 // --> i : 0 1 2 3 相对本地玩家的位置信息
                //tableData.showChairId2ServerChairIdMap[i] 0 1 2 3 服务器对应的玩家chairid
                if(firendChairId == i )
                {
                    if(i != 2)
                    {
                        console.log("11111")
                        var temp = tableData.showChairId2ServerChairIdMap[i]
                        tableData.showChairId2ServerChairIdMap[i] = tableData.showChairId2ServerChairIdMap[2]
                        tableData.showChairId2ServerChairIdMap[2] = temp
        ...
        if(isStart)
        {
            exchangeChair(wCahirIdArray,user)
            console.log(tableData.showChairId2ServerChairIdMap)
            if(tableData.showChairId2ServerChairIdMap[0] == 0)
            {
                
                var CallCard = getObjWithStructName('CMD_C_ORDER') 
                CallCard.wNewGamePlayer = tableData.showChairId2ServerChairIdMap
                console.log("isStart = true : " + CallCard.wNewGamePlayer)
                socket.sendMessage(MDM_GF_GAME, SUB_C_PLAYER_ORDER, CallCard)
            }
        }
        else
        {
            for(var i = 0;i<GAME_PLAYER;i++)
            {
                if(tableData.showChairId2ServerChairIdMap[0] == wCahirIdArray[i])
                {
                    var beg = i
                    for(var j = 0;j<GAME_PLAYER;j++)
                    {
                        tableData.showChairId2ServerChairIdMap[j] = wCahirIdArray[beg]
                        beg = (beg + 1)%GAME_PLAYER
                    }

                    console.log("isStart = false : " + tableData.showChairId2ServerChairIdMap)
                    tableNode._onSelfSitChair()
                    return
                }
            }
    ...
    ///////////////playEffect end////////
    showScoreAnimation:function(score)
    {
        if(score == INVALID_BYTE)
            return

        var operateWidth = 100 // 分数和人物头像之间的距离
        var self = tableData.getUserWithChairId(cmdBaseWorker.wOutCardUser)
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            var scoreLabel = new ccui.TextAtlas()
            var isOutCardUser = self.wChairID == user.wChairID

            var scoreNode = user.userNodeInsetChair.currentRoundNode.scoreChange
            scoreNode.removeAllChildren()
            scoreLabel.setProperty(isOutCardUser?score*3:score,isOutCardUser?resp.nums2:resp.nums3,22,33,"0")
            scoreNode.addChild(scoreLabel)


            var sign = isOutCardUser?new cc.Sprite('#plus.png'):new cc.Sprite('#minus.png')
            sign.setAnchorPoint(cc.p(0,0.5))
            scoreNode.addChild(sign)
            ...
            if( chairFactory.isRight(chair.node) )
            {
                signPosx = - swidth - operateWidth
            }
            else
            {   
                signPosx = operateWidth 
            }
            sign.setPositionX(signPosx) 
            scoreLabel.setPositionX(signPosx + scoreLabel.getContentSize().width * 0.5 + sign.getContentSize().width)
        ...
        playNode.node.runAction(action)

    },
    updateScoreInfo:function(OperateScores)
    {

        var self = tableData.getUserWithUserId(selfdwUserID)
        if(self.wChairID == INVALID_WORD)
            self = tableData.getUserWithChairId(0)

        // if(OperateScores[self.wChairID][2] == 0)
        //     return

        playNode.winFanTTF.setString("x" + OperateScores[self.wChairID][0] )
        playNode.loseFanTTF.setString("x" + OperateScores[self.wChairID][1] )
        // if(OperateScores[self.wChairID][2] >= 0)
        //     playNode.difenTTF.setString("+" + OperateScores[self.wChairID][2])
        // else
        //     playNode.difenTTF.setString("-" + OperateScores[self.wChairID][2])
        playNode.difenTTF.setString(((OperateScores[self.wChairID][2] >= 0)?"+":"-") + OperateScores[self.wChairID][2])


        //console.log("updateScoreInfo :: " + OperateScores[self.wChairID])

    },
    resetScoreInfo:function()
    {
        playNode.winFanTTF.setString("x1")
        playNode.loseFanTTF.setString("x1")
        playNode.difenTTF.setString("0")
    ...
    updateTurnoverCard:function(cbTurnoverCardData)
    {
        ...
        var node = playNode['turnoverCard0']
        var card = node.getChildByTag(100)
        cardFactory.updateIdxOfCardSpr(card, cbTurnoverCardData) 
        cardFactory.decorateCard(card)
    },
    showTurnoverCardAnimate:function(cbTurnoverCardData,wFriendArray)
    {
        var action = cc.sequence(
            cc.callFunc(
                function()
                {
                    ...
                    {
                        var user = tableData.getUserWithChairId(i)
                        var isShowPlayer = (user.wChairID == cmdBaseWorker.wBankerUser || user.wChairID == wFriendArray[cmdBaseWorker.wBankerUser])
                        if(!isShowPlayer)
                            continue

                        var outCardsNode = user.userNodeInsetChair.currentRoundNode.turnoverCardNode
                        var card = cardFactory.getOne(cbTurnoverCardData,1,0)
                        outCardsNode.addChild(card)
                    }
                }
            ),cc.delayTime(3),
            cc.callFunc(
                function()
                {
                    ...
                    {
                        var user = tableData.getUserWithChairId(i)
                        user.userNodeInsetChair.currentRoundNode.turnoverCardNode.removeAllChildren()
                    }
    ...
    showTurnoverCard:function()
    {
        if(TURNOVER_COUNT_MAGIC>0)
        {
            ...
            playNode.ScoreInfoNode.setVisible(true)
        }
    },
    hideTurnoverDiCard:function()
    {
        playNode.turnoverCardNode.setVisible(false)
        playNode.ScoreInfoNode.setVisible(false)
    },
