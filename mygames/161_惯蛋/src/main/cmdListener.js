        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                ...
                case SUB_S_TRIBUTE_RESULT:
                {
                    var body = buffer2StructObj(msg.slice(8),'CMD_S_TributeResult')
                    cmdBaseWorker.onTributeResult(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_TributeResult'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_SORT_CARD:
                {
                    var body = buffer2StructObj(msg.slice(8),'CMD_S_Sort_Card')
                    cmdBaseWorker.onSortCard(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_SortCard'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_ARRANGE_CARD:
                {
                    var body = buffer2StructObj(msg.slice(8),'CMD_S_Arrange_Card')
                    cmdBaseWorker.onArrangeCard(body)
