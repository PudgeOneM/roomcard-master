        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                ...
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_ReplaceHua') 
                    cmdBaseWorker.onCMD_ReplaceHua(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_ReplaceHua'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_OUT_CARD_LIMIT:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_OutCardLimit') 
                    cmdBaseWorker.onCMD_OutCardLimit(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_OutCardLimit'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                }
