...
{   
    ...
    checkTableCards:function(des)
    {
        ...
        var errorStr = '牌桌数据出错：tableId=' + tableData.tableID + ',selfdwUserID=' + selfdwUserID + ';'
        ...
        var cardInfo = []
        var pushCard = function(cardData, src)
        {
            if ( cardData <= 0 )
                return
            ...
            if ( cardInfo[cardData] )
            {
                ...
                cardInfo[cardData].src += ',' + src
            }
            else
            {
                cardInfo[cardData] = { count:1, src:src }
            }
        }

        var pushGroup = function(group, src)
        {
            if ( !group )
                return
            ...
            for( var j = 0; j < group.length; j++ )
            {
                ...
                if ( cc.isArray(group[j]) )
                {
                    ...
                    {
                        pushCard(group[j][n].cardData, src)
                        totalCount++
                    }
                }
                else
                {
                    pushCard(group[j].cardData, src)
                    totalCount++
                }
            }
        }
        ...
        for ( var i = 0; i < GAME_PLAYER; i++ )
        {
            pushGroup(this.flowerMajiangs4D[i],     'flower')
            pushGroup(this.heapMajiangs4D[i],       'heap')
            pushGroup(this.discardMajiangs4D[i],    'discard')
            pushGroup(this.weaveMajiangs4D[i],      'weave')

            if ( this.handMajiangs4D[i] )
            {
                pushGroup(this.handMajiangs4D[i][0],    'hand[0]')
                pushGroup([this.handMajiangs4D[i][1]],  'hand[1]')
            }
        }

        var bError = false
        errorStr += 'des=' + des
        if ( des == 'onCMD_OperateResult' )
            errorStr += ',cbOperateCode = ' + cmdBaseWorker.cbOperateCode
        errorStr += ';'
        ...
        for ( var i = 0; i < cardInfo.length; i++ )
        {
            ...
            {
                errorStr += 'cardData=' + i + ', count=' + cardInfo[i].count + ', src=' + cardInfo[i].src + ';'
        ...
        var sendLog = function(msg)
        {
            ...
            sendLogToServer(msg + 'wtms' + KIND_ID + ' ' + selfdwUserID + ' ' + playNode.checkTableIndex + ' wtms')
