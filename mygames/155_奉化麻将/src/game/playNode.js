...
{   
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            gameLog.log('isAllowOut===:' + cmdBaseWorker.wCurrentUser)
            if(isAllowOut)
            {
    ...
    popGameEnd:function(continueCall, szNickName_gameEnd)
    {
        ...
        if(cmdBaseWorker.endType == 1 || cmdBaseWorker.endType == 0)
        {
            if(cmdBaseWorker.endType == 1){
                ...
                if (cmdBaseWorker.cbLeftCardCount==0 ) 
                {
                   //currentRoundNode.heapMajiangsNode.removeAllChildren()
                };
            }
        ...
        else
        {
            ...
            for (var i = 0; i < map_mask2Name2.length; i++) 
            {
                var chr_type = window[ map_mask2Name2[i][0] ] 
                if (cmdBaseWorker.dwChiHuAssist[cmdBaseWorker.wWinner] & chr_type)
                {
                    chrStr += map_mask2Name2[i][1] + ' ';
                }
            }
        ...
        var isChengBao = false;
        for(var i = 0; i<GAME_PLAYER;i++)
        {
            if (cmdBaseWorker.lGameScore[i]==0) 
            {
                isChengBao = true;
                break;
            };
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            if (isChengBao == true && cmdBaseWorker.lGameScore[chairid]< 0) 
            {
                control['winflag'+i].setVisible(true)
                control['winflag'+chairid].setSpriteFrame('chengbao' + '.png')
            };
            //
        ...
        var mj = majiangFactory.getOne(cmdBaseWorker.cbMagicCardData[0], 1, 0, true)
        mj.setScaleX(1.5)
        mj.setScaleY(1.5)
...
*/
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        else if(cmdBaseWorker.endType == 3)
        {
            ...
            playNode.playActionEffect('hu0', tableData.getUserWithChairId(winner).cbGender)
        }
    },
    /////other ui end////////
