...
{   
    ...
    isPlaying:false,
    isOpenWxMenuShare:false,  //打开微信连接分享时的信息显示
    ...
    {
        // if( ! playNode.isOpenWxMenuShare )
        //     return;

        var strDesc = "";
        if(typeof(wxData.data.hall) != "undefined")
        {
            if (typeof(wxData.data.hall.name) != "undefined")
            {
                strDesc = wxData.data.hall.name + '---' +  tableNode.maCount1.getString() + '---' +tableNode.roundTimeTTF.getString();
            }
        }
        else
            strDesc = tableNode.maCount1.getString() + '---' +tableNode.roundTimeTTF.getString();
    ...
    {
         var s = new cc.Sprite("#ma.png")
         s.setPosition(cc.p(mj.getPosition.x + mj.getContentSize().width*0.6*0.5, mj.getPosition.y + mj.getContentSize().height*0.5*0.6))
         s.setScale(0.6)
         mj.addChild(s);
    },
    ...
    onCMD_StatusFree:function() 
    {
        ...
        if(tableData.isCreateTable)
        {
            ...
            {
                tableNode.maCount1.setString('爆炸马')
            }
        }else
        {
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        majiangFactory.initCardData2ScoreMap( [cmdBaseWorker.cbMagicCardData, null], [0, cmdBaseWorker.cbMagicCardData] )
     ...
     onSubPsResult:function()
    {
        ...
        {
            tableNode.maCount1.setString(cmdBaseWorker.maFen + '马')
            playNode.onListerMenuShare()
        }
    },
    ...
    {
        playNode.isPlaying = false
        //游戏结束就给客户端发结束消息了，这个时候数据库可能还没完成写分，如果客户端立即请求战绩就会不准
        setTimeout(function()
        {
        ...
        function onPlayAnimationOfGameEnd()
        {
            var a = cc.sequence( 
                ...
                cc.callFunc(function()
                {
                    if (cmdBaseWorker.endType == 2 )
                    {
                        var spr = actionFactory.getSprWithAnimate('km' + '_', true)
                        playNode.KaiMaNode.addChild(spr)
                        spr.x = playNode.laiziNode.width
                        spr.y = 0
                        managerAudio.playEffect('gameRes/sound/kaima.wav')
                    }
                }),
                cc.delayTime(1),
                cc.callFunc(function()
                {
                    if (cmdBaseWorker.endType == 2 && cmdBaseWorker.MaCount == 0)
                    {
                        var horseIdx = cmdBaseWorker.cbHorseIndex
                        playNode.playHorseEffect(horseIdx)
                        playNode.showLaizi(cmdBaseWorker.cbHorseCardData)        //by  gx
                    }    
                }),
                cc.delayTime(1),
                cc.callFunc(function()
                {   
                    ...
                    for (var i = 0; i < cmdBaseWorker.MaCount; i++) 
                    {
                       var mj = majiangFactory.getOne(cmdBaseWorker.cbMafenData[i], 0, 0, false)
                       mj.setAnchorPoint(0, 1)
                       mj.setScale(0.6)
                       if(cmdBaseWorker.cbMafenData[i] == 1 || cmdBaseWorker.cbMafenData[i] == 5 || cmdBaseWorker.cbMafenData[i] == 9 || cmdBaseWorker.cbMafenData[i] == 17 || cmdBaseWorker.cbMafenData[i] == 21 || cmdBaseWorker.cbMafenData[i] == 25 || cmdBaseWorker.cbMafenData[i] == 33 || cmdBaseWorker.cbMafenData[i] == 37 || cmdBaseWorker.cbMafenData[i] == 41 || cmdBaseWorker.cbMafenData[i] == 49 || cmdBaseWorker.cbMafenData[i] == 53)
                       {
                        playNode.decorateMafenData(mj)
                       }

                       mj.y = 165
                       mj.x = 280 + mj.getContentSize().width * i * 0.6
    ...
    _showSprsOnGameEnd:function()
    {
        ...
        //     mj.y = 155
        //     mj.x = 280 + mj.getContentSize().width * i * 0.85
        //     playNode.MaPaiNode.addChild(mj)
        // }
    ...
    popGameEnd:function(continueCall, szNickName_gameEnd)
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            // if((cmdBaseWorker.endType==2||cmdBaseWorker.endType==3) && cmdBaseWorker.wWinner == i)
             if((cmdBaseWorker.endType==2||cmdBaseWorker.endType==3) && cmdBaseWorker.dwChiHuKind[i])
                cbWinFlag[i] = 7

            if(cmdBaseWorker.cbGangStatus && cmdBaseWorker.wOutCardUser==i && cmdBaseWorker.endType == 2)
                cbWinFlag[i] = 8
        }
        ...
        if(cmdBaseWorker.endType == 1 || cmdBaseWorker.endType == 0)
        {
            ...
            // control.resultTTF.setVisible(false) by  gx
        }
        else
        { 
            // control.resultTTF.setVisible(true)   by  gx
            var chr_type = 
            [
                CHR_PengPengHu,
                CHR_HunPeng,
            ...
            // control.resultTTF.setString(chrStr)   by   gx
            control.endType.setVisible(false)
        }
    ...
    {
        idx = cmdBaseWorker.cbHorseCardData
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        {
            var spr = actionFactory.getSprWithAnimate('lj', true, 0.15, call)   //动作
        ...
        {
            playNode.playAnimationWithDirection('zim', tableData.getShowChairIdWithServerChairId(winner), call) //动作
            playNode.playGenderEffect('zimo0', tableData.getUserWithChairId(winner).cbGender)  //声音
        }
        else if(cmdBaseWorker.endType == 3)
        {
            playNode.playAnimationWithDirection('hu', tableData.getShowChairIdWithServerChairId(winner), call)  //动作
            playNode.playAnimationWithDirection('dp', tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wProvideUser)) //动作
            for(var i = 0; i < GAME_PLAYER; i++)
            {
                if (i == cmdBaseWorker.wProvideUser)
                {
                    var count = cmdBaseWorker.cbHuCount
                    switch(count)
                    {
                    case 1:
                       managerAudio.playEffect('gameRes/sound/1jiaqiang.wav')
                       break
                    case 2:
                       managerAudio.playEffect('gameRes/sound/2jiaqiang.wav')
                       break
                    case 3:
                       managerAudio.playEffect('gameRes/sound/3jiaqiang.wav')
                       break
                    }
                }
            }
    ...
    ///马牌音效start////
    getHorseSoundName:function(idx)
    {
        if (idx >= 0 && idx <= 26)
        {
            return idx % 9 + 1
    ...
    playHorseEffect:function(idx)
    {
        var name = playNode.getHorseSoundName(idx)
        playNode.playHorseVoice(name)
    },
    playHorseVoice:function(name)
    {
        var resPrefix = 'gameRes/sound/'
        managerAudio.playEffect(resPrefix + name + '.wav')
    },
    /////马牌音效end/////
