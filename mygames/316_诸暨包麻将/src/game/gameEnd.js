var gameEnd = 
{
	setGameEndInfo:function(data)
	{
		...
		for (var i = 0; i < GAME_PLAYER; i++) 
		{
			...
			if(wLogicChairID == mainScene._wBaoUser)
			{
				gameEnd['spriteFlagBao'+i].visible = true;
			}
			else gameEnd['spriteFlagBao'+i].visible = false;
	...
	setChiHuResult:function(wWinner)
	{
		...
		if(gameEnd.result.cbDongCount)
		{
			chrStr += "东风"+gameEnd.result.cbDongCount+"台   ";
		}
		if(gameEnd.result.cbZFBCount)
		{
			chrStr += "中发白"+gameEnd.result.cbZFBCount+"台   ";
		}
