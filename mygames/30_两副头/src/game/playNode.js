...
{   
    ...
    {   
        ...
        playNode.callButton0.x -= 90 //不报
        playNode.callButton1.x += 90 //边报
        playNode.callButton3.x -= 90 //独报

        var node = new cc.Node() //明牌
        playNode.node.addChild(node,0,202)
    ...
    adaptUi:function()
    {
        if(GAME_PLAYER == 4)
        {
            ...
            console.log("坐标................")
            console.log(tableNode.chairNode0.x + "," + tableNode.chairNode0.y) //下
            console.log(tableNode.chairNode1.x + "," + tableNode.chairNode1.y) //右
            console.log(tableNode.chairNode3.x + "," + tableNode.chairNode3.y) //左
            console.log(tableNode.chairNode2.x + "," + tableNode.chairNode2.y) //上
        }
    ...
    _registEvent:function() 
    {
        var l = cc.EventListener.create({
            ...
            {   
                console.log("userNodeInsetChairInit")
        ...
        var l = cc.EventListener.create({
            ...
            callback: function(event)
            {   
                console.log("userNodeInsetChairUpdate")
    ...
    onCMD_GameType:function()
    {
        console.log("onCMD_GameType")
        playNode.setTypeName(cmdBaseWorker.hasGameType)
        selTypeNode._setStartBtnEnabled()
    },
    onCMD_StatusFree:function() 
    {
        cocos.setInterval(playNode.updateOnFree, 1000, playNode.node)
        if(cmdBaseWorker.hasGameType != null)
        {
            selTypeNode.hasSetGameType = cmdBaseWorker.hasGameType == 0?false:true
            selTypeNode._addUiToMainScene()
            playNode.setTypeName(cmdBaseWorker.hasGameType)
        }
        selTypeNode._setStartBtnEnabled()
        console.log("onCMD_StatusFree")
    },
    onCMD_StatusCall:function()
    {
        console.log("onCMD_StatusCall")
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        if(cmdBaseWorker.wShowHandUser != INVALID_WORD)
        {
            playNode.showHandCardData(tableData.getUserWithChairId(cmdBaseWorker.wShowHandUser),cmdBaseWorker.cbShowHandCardData)
        }
        else
        {
            for(var i= 0 ;i<GAME_PLAYER;i++)
            {
                if(cmdBaseWorker.wisShowFriend[i] == 1)
                    tableNode.setBankerIcon(i,true)
            }
        }
    ...
    {
        console.log("onCMD_GameStart")
        console.log("cmdBaseWorker.hasGameType:" + cmdBaseWorker.hasGameType)
        //有叫分的话这里注释
        // cocos.clearInterval(playNode.updateOnFree, playNode.node)
        // playNode.node.stopAction(playNode.gameEndAction)
        // playNode.resetPlayNode()
        ...
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
        //统一用sendCardsAction todoonCMD_GA
        var outCardDatasArray = []
        var handCardDatasArray = []
        for(var showChairId=0;showChairId<GAME_PLAYER;showChairId++)
        {
    ...
    onCMD_OutCard:function() 
    {
        ...
        var lastUser = cmdBaseWorker.wCurrentUser - 1 < 0?3:cmdBaseWorker.wCurrentUser - 1
        if(cmdBaseWorker.wOutCardUser == cmdBaseWorker.wShowHandUser && cmdBaseWorker.wShowHandUser != INVALID_WORD)
            playNode.showHandCardData(tableData.getUserWithChairId(cmdBaseWorker.wShowHandUser),cmdBaseWorker.cbShowHandCardData)
        else if(cmdBaseWorker.wisShowFriend== 1)
        {
            tableNode.setBankerIcon(lastUser, true)
        }
        playNode.refreshWinOrder(cmdBaseWorker.wWinOrder)
    },
    onCMD_PassCard:function() 
    {
        console.log("onCMD_PassCard")
...
// ]
        ...
        console.log("selfdwUserID : ",selfdwUserID)
        console.log("selfDir : ",selfDir)
        console.log("handGroupNode4D:",playNode.handGroupNode4D)
        ...
        for(var showChairId=0;showChairId<GAME_PLAYER;showChairId++)
        {
            ...
            if(playNode.handGroupNode4D[direction]._children[0] != null)
                playNode.PokerTouchHeight = playNode.handGroupNode4D[direction]._children[0].y
        }
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        var node = playNode.node.getChildByTag(202)
        ...
        for(var wChairID=0;wChairID<GAME_PLAYER;wChairID++)
        {
            ...
            ///////////
            ///hasGameType  游戏类型
            console.log("winTypeArray....." + cmdBaseWorker.cbWinType)
            ...
            //     control['winflag'+wChairID].setSpriteFrame('gendFlag2.png') 
            else if(cmdBaseWorker.cbWinType[wChairID] > 0 )
            {
                // 1 为赢 2 大封门 3 小封门 
                control['winflag'+wChairID].setSpriteFrame('gendFlag'+ cmdBaseWorker.cbWinType[wChairID]  + '.png') 
    ...
    //明牌
    showHandCardData:function(operateUserObj,handCardDatasArray)
    {
        if(operateUserObj.dwUserID != selfdwUserID)
        {
            var self = tableData.getShowChairIdWithServerChairId(operateUserObj.wChairID)
            console.log("sdasdasda",self)
            
            var parent = operateUserObj.userNodeInsetChair.currentRoundNode
            ...
            for(var i = 0;i<handCardDatasArray.length ;i++)
            {
                if(handCardDatasArray[i] != INVALID_BYTE)
                    handArray[handArray.length] = handCardDatasArray[i]
            }
            ...
            {
                case 1:
                node.x = tableNode.chairNode1.x - 200
                node.y = tableNode.chairNode1.y - 100
                handArray = [0,handArray,0,0]
                break
                case 2:
                node.x = tableNode.chairNode2.x - 350
                node.y = tableNode.chairNode2.y 
                handArray = [0,0,handArray,0]
                break
                case 3:
                node.x = tableNode.chairNode3.x 
                node.y = tableNode.chairNode3.y - 100
                handArray = [0,0,0,handArray]
                break
            }
            
            handArray[self] = cmdBaseWorker.sortHandCardDatas(handArray[self])
            //handArray[self].length
            for(var i= handArray[self].length- 1 ;i >= 0;i--)
            {
                if(handArray[self][i] == INVALID_BYTE)
                    continue

                var spr = cardFactory.getOne(handArray[self][i],0,0)

                var count = node.childrenCount + 1 
                var Index = 0
            ...
            node.scaleX = cardFactory.outIntervalXScale
            node.scaleY = cardFactory.outIntervalYScale


            // var showHandGroup4d = cardFactory.getHandCardsArray(handCardDatasArray)
            // parent.removeAllChildren()
            // parent.addChild(showHandGroup4d)
          //  var node  = playNode[]
    ...
    {
        console.log("_initCallCall")
        var call = function(cbParam1)
        {
            console.log("点击的是按钮",cbParam1)
            ...
            if(cbParam1 != 1)
            {
                ...
                CallCard.cbCallCard = INVALID_BYTE
            ...
            else 
            {
                //边报
                var TouchCardData = []
                var handCardDatas = playNode.handGroupNode4D[0]._children
                var index = 0
                for(var i= 0;i< handCardDatas.length;i++)
                {
                    if(handCardDatas[i].y > playNode.PokerTouchHeight)
                    {
                        TouchCardData[TouchCardData.length] = handCardDatas[i]
                        index = i 
                    }
                }

                if(TouchCardData.length == 1)
                {
                    var left = index == 26? true : TouchCardData[0].cardData != handCardDatas[index + 1].cardData
                    var right = index == 0 ? true : TouchCardData[0].cardData != handCardDatas[index - 1].cardData
                    if(left && right)
                    {
                        console.log("发送到服务器的牌:",TouchCardData[0].cardData)
                        CallCard.cbCallCard = TouchCardData[0].cardData
                        CallCard.cbCallScore = cbParam1 
                        socket.sendMessage(MDM_GF_GAME, SUB_C_CALL, CallCard)
                        playNode.hideCallNode()
                        return
    ...
    popCallNode:function(isCan, cbCallScore) // 0 当前0分 叫分
    {
        console.log("popCallNode")
        if(!isCan) return;
        ...
        // playNode.callButton3.setEnabled(cbCallScore<3) 改了会变暗 只能往上面叫
        
        

        playNode.callButton0.setEnabled(cbCallScore != 0)
        playNode.callButton1.setEnabled(cbCallScore < 1)
    ...
    callCard:function(callUser, cbCallScore)
    {
        ...
        playNode.playGenderEffect('jiaofen'+cbCallScore, user.cbGender) //叫分语音？
    },
    clearCallCard:function(userChair)
    {
        console.log("clearCallCard")
    ...
    {
        console.log("hideCallNode")
        playNode.callNode.setVisible(false)
    },  
    startCallClock:function(isCan, time)
    {   
        console.log("startCallClock")
    ...
    playCardTypeEffect:function(cardsType, isMan)
    {
        ...
        if(name == undefined)
        {
            //managerAudio.playEffect('gameRes/sound/daxiaowangEffect.mp3')
            name = 'daxiaowang'
        }

        console.log("语音名字........",name)
    ...
    setTypeName:function(TypeIndex)
    {
        switch(TypeIndex)
        {
            case 2:
            playNode.difenTTF.setString('有平局 非全讨')
            break
            case 3:
            playNode.difenTTF.setString('无平局 非全讨')
            break
            case 4:
            playNode.difenTTF.setString('有平局 全讨')
            break
            case 6:
            playNode.difenTTF.setString('无平局 全讨')
            break
        }

    },
    ////////////////DiCard start/////////////////
    addListenerToSpr:function(Node,ControlNode)
    {
        cc.eventManager.addListener({
            ...
            onTouchBegan:function(touch,event)
            {
                ...
                var size = ControlNode._children[0].getContentSize()
                var rect = cc.rect(0,0,size.width,size.height)
                if(!cc.rectContainsPoint(rect,locationInNode))
                    return false
                ControlNode._children[0].color = new cc.color(255,255,255)
                return true
            },
            onTouchEnded:function(touch,event)
            {
                Node._children[0].color = new cc.color(100,100,100)
             //   return true
            }
        },ControlNode._children[0])
    },
    initDiCard:function() 
    {
    ...
    showDiCard:function()
    {
        ...
        if(cmdBaseWorker.cbTurnoverFirendCardData[0] != INVALID_BYTE)
        {
            selTypeNode._setFanpaiVisable(cardFactory.getOne(cmdBaseWorker.cbTurnoverFirendCardData[0],0,0))
        }
    },
    hideDiCard:function()
    {
