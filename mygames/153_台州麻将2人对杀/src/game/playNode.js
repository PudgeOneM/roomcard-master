...
{   
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        var fengweiId = ( cmdBaseWorker.wCurrentUser==INVALID_WORD?[]:[playNode.reGetShowChairId ( tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wCurrentUser) ) ] )
        playNode.timer.switchTimer( fengweiId )
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            for(var j=0;j<MAX_WEAVE;j++)
            {
                ...
                var proDirection = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(weaveItems[j].wProvideUser))
                weaveItems[j].provideDirection = proDirection
            }
    ...
    onCMD_OutCard:function() 
    {
        ...
        playNode.setCurrentDiscardMj(outIdx, playNode.reGetShowChairId(outDir) )
    },
    onCMD_SendCard:function() 
    {
        ...
        playNode.timer.switchTimer([ playNode.reGetShowChairId(dir) ])
    ...
    _showSprsOnGameEnd:function()
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            direction = playNode.reGetShowChairId(direction)
            showChairid = playNode.reGetShowChairId(direction)
    ...
    popGameEnd:function(continueCall, szNickName_gameEnd)
    {
        ...
        var mj = majiangFactory.getOne(cmdBaseWorker.cbMagicCardData, 1, 0, true)
        mj.setScaleX(1.5)
        mj.setScaleY(1.5)
        control.laiziNode.addChild(mj)

        control.endType.setVisible(true)
        var selfChairId = tableData.getUserWithUserId(selfdwUserID).wChairID
        if(cmdBaseWorker.wWinner == selfChairId)
            control.endType.setSpriteFrame('win.png')
        else
            control.endType.setSpriteFrame('lose.png')
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            {
                //调整位置到当前胡的玩家
                control.resultTTF.x = 3*40 +160+10
                control.resultTTF.y = control['resultNode'+chairid].y+50
            };
            ...
            fNames[2] = "img_nan.png"
            fNames[3] = "img_nan.png"
