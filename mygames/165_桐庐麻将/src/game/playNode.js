...
{   
    ...
    {
        console.log(222222222222,cmdBaseWorker.wCurrentUser)
        playNode.isPlaying = true
        //playNode.MaxscoreTTF.setVisible(true)
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
        ...
        // console.log(111111111111111,cmdBaseWorker.wCurrentUser)
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
           ...
           var isAllowOut = !(!MAGIC_CARD_ALLOWOUT && cmdBaseWorker.isMagicCard(majiang.cardData, cmdBaseWorker.cbMagicCardData) ) && cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID
                ...
                {
                    console.log(1111111111,cmdBaseWorker.cbAutocard[0],cmdBaseWorker.cbAutocardUser[cmdBaseWorker.cbAutocard[0]])
                    console.log(2222222222,cmdBaseWorker.cbAutocard[1],cmdBaseWorker.cbAutocardUser[cmdBaseWorker.cbAutocard[1]])
    ...
    popGameEnd:function(continueCall, szNickName_gameEnd)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < map_mask2Name.length; i++) 
            {
             ...
             console.log(44444444444444,cmdBaseWorker.Contract)
             var gangcount = cmdBaseWorker.nGangcount+1
             console.log(gangcount)
            if(gangcount >0)
                chrStr += "杠*"+gangcount
    ...
    {
        cc.console(cmdBaseWorker.Contract)
        var cbWinFlag = []
        for(var i=0;i<GAME_PLAYER;i++)
        {
            if(cmdBaseWorker.endType==0 && i==cmdBaseWorker.wExitUser)
    ...
    setcolor:function(outDir)
    {
        ...
        //     for(var t = 0;t<playNode.Eatsendcontrol.length;t++)
        //     {
        //         if(mj[1].cardData == playNode.Eatsendcontrol[t] )
        //         {
        //             var majiang = mj[1]
        //             majiang.color = cc.color(122,122,122)
        //             var zi = majiang.getChildByTag(101)
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        {
            var kind = cmdBaseWorker.cbkind 
            console.log(kind)
            playNode.playAnimationWithDirection('hu', tableData.getShowChairIdWithServerChairId(winner), call)
            playNode.playGenderEffect(kind, tableData.getUserWithChairId(winner).cbGender)
        }
        else if(cmdBaseWorker.endType == 3)
        {
            ...
            playNode.playActionEffect(kind, tableData.getUserWithChairId(winner).cbGender)
        }
    },
    /////other ui end////////
