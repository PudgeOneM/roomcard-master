        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                ...
                case SUB_S_CALL: 
                {   
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_Call') 
                ...
                case SUB_S_LOOK_BAO: 
                {  
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_LookBao') 
                    cmdBaseWorker.CMD_S_LookBao(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['CMD_S_LookBao'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_CHANGE_BAO: 
                {  
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_ChangeBao') 
                    cmdBaseWorker.CMD_S_ChangeBao(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['CMD_S_ChangeBao'])
                    cc.eventManager.dispatchEvent(event)
                    break
                    
                }
                case SUB_S_OPERATE_EGG:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_OperateEggRes') 
                    cmdBaseWorker.onCMD_S_OperateEggRes(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_S_OperateEggRes'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                case SUB_S_OPERATEEGG_END:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_OperateEggEnd') 
                    cmdBaseWorker.onCMD_S_OperateEggEnd(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_S_OperateEggEnd'])
                    cc.eventManager.dispatchEvent(event)
                    break
                }
                ...
                case SUB_S_GAME_TYPE:
                {
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_GameType') 
                    cmdBaseWorker.onSMD_GameType(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onSMD_GameType'])
                    cc.eventManager.dispatchEvent(event)
                    break;
                }
                case SUB_S_GAME_END: 
