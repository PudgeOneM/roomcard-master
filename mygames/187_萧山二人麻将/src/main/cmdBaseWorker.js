...
{   
    ...
    {	
        cmdBaseWorker.cbweavePublicPlay     = body.Ispublic
    ...
    {        
        cmdBaseWorker.cbWeaveCount    = body.cbWeaveCount    
        cmdBaseWorker.WeaveItemArray  = body.WeaveItemArray  
        cmdBaseWorker.cbweavePublic     = body.Ispublic
    ...
    {
        cmdBaseWorker.cbFengNum           = body.cbFengNum
        cmdBaseWorker.cbGangCount         = body.cbSRECount
        cmdBaseWorker.cbMagicCount        = body.cbMagicCount
        cmdBaseWorker.cbAnGang              = body.cbAnGang
        cmdBaseWorker.cbMingGang            = body.cbMingGang
        cmdBaseWorker.cbLianZhuang          = body.cbWinUserBaseScore
    ...
    searchGangCardDatas:function(handMajiangs, weaveMajiangs) //自摸杠时找到杠哪个 服务器传? TODO
    {
        ...
        for(var i=0;i<handCardDatas.length;i++)
        {
            ...
            if(i>0 && cardData == handCardDatas[i-1])
                continue
            if(i>0 && cardData == cmdBaseWorker.cbMagicCardData[0])
                continue
            //下面这条代码有个BUG，不过只会在测试的时候可能出现，当手牌上的某张牌数量大于4张的时候，而且此时又是能杠，那么系统会将那张错误数量的牌也认为能杠
            //当选择那张错误数量的牌时，服务器不承认这个杠，但是客户端执行了杠，牌已经删掉，此时出现了没有摸牌的情况
            //目前没有解决方法，或者在进行下面这条判定的时候，再加个数量的条件，先这样吧
