...
{   
    ...
    {   
        ...
        majiangFactory.discardCountOneLine = 14
        majiangFactory.discardCountOneRow = 14
    ...
    {   
        ...
        playNode.actionCall_gang = function()
        {
            ...
            else
            {
                ...
                var direction = playNode.reGetShowChairId( tableData.getShowChairIdWithServerChairId(selfChairId) )
        ...
        playNode.actionCall_guo = function()
        {
            ...
            cmdBaseWorker.sendMessage_guo([cmdBaseWorker.cbProvideCardData,0,0,0])
            playNode.hideActionBtns()  
        }
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        playNode.timer.initFenwei( playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wBankerUser)) )
        ...
        for(var wChairID=0;wChairID<GAME_PLAYER;wChairID++)
        {
            ...
            var direction = playNode.reGetShowChairId( tableData.getShowChairIdWithServerChairId(wChairID) )
        ...
        {  
            publiccount = 0
            var dir = j
            var ServerChairID = tableData.getServerChairIdWithShowChairId(dir)
            dir = playNode.reGetShowChairId(dir)
            for (var i=0;i<cmdBaseWorker.cbWeaveCount[ServerChairID];i++)
            {
                  //var b = a[i]
                var Item = cmdBaseWorker.WeaveItemArray[ServerChairID][i]

                if (Item.cbWeaveKind ==WIK_GANG)
                {
                    
                    var ispublicopen = cmdBaseWorker.cbweavePublicPlay[ServerChairID][publiccount++]
                    playNode.changeWeaveShowValue(Item.cbCenterCardData,playNode.weaveMajiangs4D[dir][i],dir,ispublicopen)
                }
            }
        }
    },
    ...
    {        
        ...
        var bankerShowChairid = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wBankerUser))
        ...
        var takerShowChairid = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(takerChairid))
        ...
        // {
            ...
            playNode.playDiceOneDirection(gameStart, cmdBaseWorker.cbSiceCount[0], 
                cmdBaseWorker.cbSiceCount[1], takerShowChairid)
        }
    ...
    onCMD_OutCard:function() 
    {
        ...
        var outDir = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wOutCardUser))  
    ...
    onCMD_SendCard:function() 
    {
        ...
        var takeDir =  playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wTakeCardUser) )
        ///////
        ...
        //var a = playNode.weaveMajiangs4D[takeDir]
        var publiccount = 0
        if (!isSelf)
        {
            //console.log(0000000000,cmdBaseWorker.cbweavePublic)
            for (var i=0;i<cmdBaseWorker.cbWeaveCount[cmdBaseWorker.wCurrentUser];i++)
            {
                //var b = a[i]
                var Item = cmdBaseWorker.WeaveItemArray[cmdBaseWorker.wCurrentUser][i]
                if (Item.cbWeaveKind ==WIK_GANG)
                {
                    var ispublicopen = cmdBaseWorker.cbweavePublic[publiccount++]
                    //console.log(11111111,ispublicopen)
                    playNode.changeWeaveShowValue(Item.cbCenterCardData,playNode.weaveMajiangs4D[takeDir][i],takeDir,ispublicopen)
                }



            // var majiang = majiangFactory.getOne(cardData, 2, takeDir)

            // var pos = majiangFactory.getWeaveMajiangPosAndTag(groupIdx, idxInGroup, takeDir)
            // majiang.x = pos.x
            // majiang.y = pos.y
            // majiang.setLocalZOrder(pos.zOrder)
            // if(weaveItem.provideDirection!=takeDir && !hasAddDirectionSpr && cardData == weaveItem.cbCenterCardData)
            // {
            ...
            //changeWeaveShowValue(carddata,playNode.weaveMajiangs4D[takeDir][i],takeDir)

                // if (a[i].cbWeaveKind == WIK_GANG)
                // {
                //     console.log(11111111,c)
                //     a[i].ispublic = true
                //     var b = a[i][3].getChildByTag(101)
                //     b.setSpriteFrame(("mf_" + 49 + '.png'))
                //     a[i][3].cbCardData = 49
                // }
            }
        }            //获得精灵失败
    },
    changeWeaveShowValue:function(cardData,group,direction,ispublicopen)
    {
        if (!ispublicopen)
            return
        ...
        // console.log(222222,frameName,data)
        // data.setSpriteFrame(frameName)
        // console.log(111111,frameName,data)
    ...
    onCMD_OperateResult:function() 
    {
        ...
        if(cmdBaseWorker.cbOperateCode != WIK_NULL)
        {
            ...
            playNode.onActionResult(cmdBaseWorker.cbOperateCode, cardDatas, operateUser, provideUser,
                majiangs4W4D, playNode.handGroupNode4D)

            playNode.playAction(cmdBaseWorker.cbOperateCode, operateUser)
        }
        
        var takeDir =  playNode.reGetShowChairId( tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wOperateUser) )
        var isSelf = tableData.getUserWithChairId(cmdBaseWorker.wOperateUser).dwUserID == selfdwUserID
        var publiccount = 0
        if (!isSelf)
        {
            for (var i=0;i<cmdBaseWorker.cbWeaveCount[cmdBaseWorker.wOperateUser];i++)
            {
                //var b = a[i]
                var c = cmdBaseWorker.WeaveItemArray[cmdBaseWorker.wOperateUser][i]
                if (c.cbWeaveKind ==WIK_GANG)
                {
                    var ispublicopen = cmdBaseWorker.cbweavePublic[publiccount++]
                    playNode.changeWeaveShowValue(c.cbCenterCardData,playNode.weaveMajiangs4D[takeDir][i],takeDir,ispublicopen)
                }
            }
        }   
    ...
    onActionResult:function(action, cardDatas, operateUser, provideUser, majiangs4W4D, handGroupNode4D)
    {
        if(action==WIK_REPLACE)
            playNode.onActionReplace(cardDatas, operateUser, provideUser, majiangs4W4D, handGroupNode4D)
        if(action==WIK_GANG)
    ...
    {
        var operateUserDir = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(operateUser.wChairID))
    ...
    onActionGang:function(cardDatas, operateUser, provideUser, majiangs4W4D, handGroupNode4D)
    {
        ...
        var operateUserDir = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(operateUser.wChairID))
        var provideUserDir = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(provideUser.wChairID))
    ...
    getHandGroupNodes:function(handMajiangs4D, outCardCalls)
    {
        ...
        {
            var direction = playNode.reGetShowChairId( showChairid )
            ...
            case 1:
            {
                ...
                var listener = cc.EventListener.create({
                    ...
                    onTouchBegan: function (touch, event) {
                        ...
                        // if (!isopen&&locationInNode.x<(majiangs[0].length*65) && isSelf)
                        //     return false
                    ...
                    onTouchMoved: function (touch, event) {
                        ...
                        var isSelf = cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID
                        // var self = tableData.getUserWithUserId(selfdwUserID)
                        // var isopen = ((self.wChairID==cmdBaseWorker.bIsForbidActionUser[0] || !cmdBaseWorker.bIsOutMagic[cmdBaseWorker.bIsForbidActionUser[0]]) && !(cmdBaseWorker.bIsForbidActionUser[0]==cmdBaseWorker.bIsForbidActionUser[1]?false:cmdBaseWorker.bIsOutMagic[cmdBaseWorker.bIsForbidActionUser[1]]) )
                        // if (!isopen&&locationInNode.x<(majiangs[0].length*65) && isSelf)
                        //     return false
                        if (cc.rectContainsPoint(rect, locationInNode)) {
                            onTouch(locationInNode.x<0?0:locationInNode.x)
    ...
    {   
        ...
        playNode.flowerMajiangs4D = playNode.getFlowerMajiangsArray(flowerCardDatasArray)
        GAME_PLAYER = 3
        ...
        playNode.handMajiangs4D = playNode.getHandMajiangsArray(handCardDatasArray, tableData.getUserWithUserId(selfdwUserID).wChairID == INVALID_WORD)
        ...
        for(var showChairid=0;showChairid<GAME_PLAYER;showChairid++)
        {
            ...
            for (var i = 0; i < map_mask2Name.length; i++) 
            {
                ...
                if (cmdBaseWorker.dwChiHuRight[wChairID] & chr_type)
                {
                    ...
                    //console.log(111111111,map_mask2Name[i][1])
                    if (map_mask2Name[i][1] == "飘财" && cmdBaseWorker.cbMagicCount[wChairID]!=0)
                        chrStr += '*' + cmdBaseWorker.cbMagicCount[wChairID]
                    chrStr += ' '
                }
            }
            if (cmdBaseWorker.cbFengNum[wChairID] != INVALID_CARD_DATA && cmdBaseWorker.cbFengNum[wChairID]!=0)
                chrStr += "风*" + cmdBaseWorker.cbFengNum[wChairID] + ' '
            // if (cmdBaseWorker.cbGangCount[wChairID]!=0)
            //     chrStr += "杠*" + cmdBaseWorker.cbGangCount[wChairID] + ' '
            if (cmdBaseWorker.cbAnGang[wChairID]!=0)
                chrStr += "暗杠*" + cmdBaseWorker.cbAnGang[wChairID] + ' '
            if (cmdBaseWorker.cbMingGang[wChairID]!=0)
                chrStr += "明杠*" + cmdBaseWorker.cbMingGang[wChairID] + ' '
            // if (cmdBaseWorker.cbMagicCount[wChairID]!=0)
            //     chrStr += "财*" + cmdBaseWorker.cbMagicCount[wChairID] + ' '
            
            if (cmdBaseWorker.cbLianZhuang == 4 && chrStr != '' && cmdBaseWorker.wBankerUser==wChairID)
                chrStr += " 二连庄"
            if (cmdBaseWorker.cbLianZhuang > 4 && chrStr != '' && cmdBaseWorker.wBankerUser==wChairID)
                chrStr += " 三连庄"

            if ( chrStr=='' && cmdBaseWorker.wBankerUser==wChairID && cmdBaseWorker.cbLianZhuang == 4)
                chrStr += " 二连庄"
            if ( chrStr=='' && cmdBaseWorker.wBankerUser==wChairID && cmdBaseWorker.cbLianZhuang > 4)
                chrStr += " 三连庄"
    ...
    setCurrentDiscardMj:function(cardData, direction)
    {
        ...
        var dir = playNode.reGetShowChairId(d)
        if(dir == direction && d!= null)
        {
            majiangFactory.hideCurrentDiscardMj()
            playNode.reMoveDecorate()
        }
        else
        {
            majiangFactory.setCurrentDiscardMj(cardData, direction)
            //playNode.setCurrentDiscardMj1(cardData, direction)
            ...
            if( cmdBaseWorker.isMagicCard(cardData, cmdBaseWorker.cbMagicCardData) ) //|| mj.cardData == REPLACE_CARD_DATA)
            {
                if(zi)
                {
                    s.y = 0.5*zi.height + 5
            ...
            if( !cmdBaseWorker.isMagicCard(cardData, cmdBaseWorker.cbMagicCardData) )
            {
                var rm = zi.getChildByTag(102)
                if (rm)
                    zi.removeChild(rm)
            }
        }
    },
    reMoveDecorate:function()
    {
        var mj = majiangFactory.currentDiscardMjNode.getChildByTag(101)
        var zi = mj.getChildByTag(101)
        var rm = zi.getChildByTag(102)
    ...
    setCurrentDiscardMj1:function(cardData, direction)
    {   
        majiangFactory.currentDiscardMjNode.setVisible(true)
        var mj = majiangFactory.currentDiscardMjNode.getChildByTag(101)
    ...
    showActionBtns:function(sortedActions)
    {
        for(var i=sortedActions.length-1;i>=0;i--)
        {
            ...
            {
                var self = tableData.getUserWithUserId(selfdwUserID)
                var selfDir = playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(self.wChairID))
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        {
            playNode.playAnimationWithDirection('hu', playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wProvideUser)), call)
            playNode.playGenderEffect('hu0', tableData.getUserWithChairId(cmdBaseWorker.wProvideUser).cbGender)
        }
            ...
            for(var i=0;i<GAME_PLAYER;i++)
            {
                if(cmdBaseWorker.dwChiHuKind[i] == WIK_CHI_HU)
                {
                    if(!hasCall)
                        ...
                        call = null
                    playNode.playAnimationWithDirection('hu', playNode.reGetShowChairId(tableData.getShowChairIdWithServerChairId(i)), call)
