        ...
        case MDM_GF_GAME:  //游戏消息 200
            {
                ...
                case SUB_S_SHOWONE_RESULT: 
                {   
                    var body = buffer2StructObj(msg.slice(8), 'CMD_S_ShowOneResult') 
                    cmdBaseWorker.onCMD_ShowOneResult(body)

                    var event = new cc.EventCustom("cmdEvent")
                    event.setUserData(['onCMD_ShowOneResult'])
                    cc.eventManager.dispatchEvent(event)
                    break
                } 
