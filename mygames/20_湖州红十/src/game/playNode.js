...
{   
    ...
    onCMD_CallResult:function()
    {
        ...
        if(cmdBaseWorker.wCurrentUserCall!=INVALID_WORD 
            && tableData.getUserWithUserId(selfdwUserID).wChairID == cmdBaseWorker.wCurrentUserCall)
            playNode.popCallNode(true)
    },
    ...
    popGameEnd:function(continueCall, szNickName_gameEnd)
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            if(i==cmdBaseWorker.wExitUser)
                cbWinFlag[i] = 6
            else if(cmdBaseWorker.cbCallRecord[i] == 100)
                cbWinFlag[i] = 8
        }
    ...
    _initCallCall:function()
    {
        ...
        //客户端发call
        // 0:有1个红10 不叫  1:有1个红十亮1个红10  2:有2个红10 不叫  3:有2个红十亮2个红10  100:投降
    
        //服务端会根据客户端发的call 计算存储    
        //0:不叫 1:主叫1张红10 2:叫两张红10 3:副叫1张红10    100:投降

        playNode.callCall1 = function(){ callCard(1) }
        playNode.callCall2 = function(){ callCard(0) }
        playNode.callCall3 = function(){ callCard(3) }
        playNode.callCall4 = function(){ callCard(2) }
        playNode.callCall5 = function(){ callCard(100) }
    },
    popCallNode:function(isCan) // 0 当前0分
    {
        ...
        if(red10Count==1)
        {
            playNode.callNode1.setVisible(true)
            playNode.callNode2.setVisible(false)
        }
        else if(red10Count==2)
        {
            playNode.callNode1.setVisible(false)
            playNode.callNode2.setVisible(true)
        }
    },
    ...
    updateCallRecord:function()
    {
        for(var i=0;i<GAME_PLAYER;i++)
        {
            if(cmdBaseWorker.cbCallRecord[i] == 0 || cmdBaseWorker.cbCallRecord[i] == INVALID_BYTE)
                continue
            var user = tableData.getUserWithChairId(i)
            ...
            //0:不叫 1:主叫1张红10 2:叫两张红10 3:副叫1张红10    100:投降
            var str = ''
            if(cmdBaseWorker.cbCallRecord[i]==100)
                str = 'x0(投降)'
            else if(cmdBaseWorker.cbCallRecord[i]==1)
                str = 'x1'
            else if(cmdBaseWorker.cbCallRecord[i]==2)
                str = 'x2'
            else if(cmdBaseWorker.cbCallRecord[i]==3)
                str = 'x1(副)'
            
            user.userNodeInsetChair.currentRoundNode.flowerTTF.setString(str)
        }
    }
}
