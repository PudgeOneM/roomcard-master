...
{   
    ...
    onCMD_GameStart:function() 
    {
        ...
        for(var i=0;i<magicCardData.length;i++)
        {
            ...
            var isCaiHua = cmdBaseWorker.isCaiHua(magicCardData[0]) // 财神是不是花
            var cbValue = magicCardData[0]&MASK_VALUE;
            if (isCaiHua)
            {
                var cbFirstMagic = 0x41
                if (cbValue <= 4)
                    cbFirstMagic = 0x41
                else
                    cbFirstMagic = 0x45
                ...
                for (var j = 0; j < 4; j++)
                {
                    var cbMagic = cbFirstMagic + j
                    idxs[idxs.length] = cbMagic
                    scores[scores.length] = -1000+cbMagic
                }
            }
            else
            {
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        {            
            var isHua = false
            if (majiang.cardData >= 0x35 && majiang.cardData <= 0x48)
                isHua = true

            // var isFengMagic = cmdBaseWorker.isMagicCard(majiang.cardData, cmdBaseWorker.cbMagicCardData)&&isFeng
            // var isHuaMagic  = cmdBaseWorker.isCaiHua(majiang.cardData)

            var isAllowOut = cmdBaseWorker.wCurrentUser == tableData.getUserWithUserId(selfdwUserID).wChairID && isHua == false
            if(isAllowOut)
            {
...
/////////////////////////@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    showLaizi:function()
    {
        ...
        if (cmdBaseWorker.cbDoubleDot != 0)
        {
            gameLog.log("双筛子!!!!!!!!!!!!!")
            var dian = new cc.Sprite('#dian_'+cmdBaseWorker.cbDoubleDot+'.png')
            dian.x = (50 + 25 + 50)*MAX_MAGIC_COUNT/2+8
            dian.y = -20
            playNode.laiziNode.addChild(dian)

            var num = new cc.Sprite('#double.png')
            num.x = (50 + 25 + 50)*MAX_MAGIC_COUNT/2+8
            num.y = -47
            playNode.laiziNode.addChild(num)
        }
    },
    hideLaizi:function()
    {
