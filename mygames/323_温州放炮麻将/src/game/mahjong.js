            ...
            swallowTouches: true,
           	onTouchBegan: function (touch, event) {
				//outMahJong.hideOutCard();
           		//gameLog.log("mainScene._wMeChaird"+mainScene._wMeChaird)
           		if(mainScene._wMeChaird == INVALID_CHAIR) return false;
           		if(handMahJong.control[1].visible == false) return false;
       	        for (var i = 0; i < MAX_COUNT; i++) {
       	        	gameLog.log("visible"+handMahJong.control[i].visible)
       	        	gameLog.log("enable"+handMahJong.control[i].enable)
...
var weaveControl = 
{
	...
	addWeaveItem:function(wChaird, cbWeaveIndex, cbCardData, cbCardCount, visible)
	{
 		...
 		{
 			 if(wChaird == 0)
	 			weaveControl.arrow[wChaird][cbWeaveIndex] = new cc.Sprite("gameRes/pic/arrow.png");
			else
			{
	 			weaveControl.arrow[wChaird][cbWeaveIndex] = new cc.Sprite("gameRes/pic/arrow.png");
	 			weaveControl.arrow[wChaird][cbWeaveIndex].scale = 0.9
			}
		...
		if (cbWeaveKind & WIK_GANG)
		{
			if (cbPublicCard) 
			{
				...
				if (bShowArrow)
				{
					...
					weaveControl.arrow[wChaird][cbWeaveIndex].setScale(0.7)
				}
			}
			else
			{
