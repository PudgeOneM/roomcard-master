                    ...
                    case SUB_GR_TABLE_OWNER:
                    {
                        ...
                        tableData.managerUserID = body.dwTableOwner
                         ...
                         else
                         {
                            ...
                            var sendData = getObjWithStructName('CMD_C_FangZhu');
                            sendData.wChairID = gameLogic.getMeChaird();
                            socket.sendMessage(MDM_GF_GAME, SUB_C_FANG_ZHU, sendData); 
                         }
                        break
                    }
                }
        ...
        switch(head.CommandInfo.wMainCmdID)
        {
            case MDM_GF_GAME:       //游戏消息 200
            {
                switch(head.CommandInfo.wSubCmdID)
                {
                    ...
                    case SUB_S_ZHUA_MA_RESULT:
                    {
                         var body = buffer2StructObj(msg.slice(8), 'CMD_S_ZhuaMaResult');
                        return mainScene.OnZhuaMaResult(body);  
                    }
