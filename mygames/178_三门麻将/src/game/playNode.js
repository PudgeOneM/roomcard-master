...
{   
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        if(direction==0)
        {
            ...
            currentRoundNode.playerStatusNode.setPosition( cc.p(30, 20) )  
            //discard
    ...
    {        
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            gameLogic.sortHandCardDatas(handCardDatasArray[direction][0]) 
        }
        //gameLogic.sortHandCardDatas(handCardDatasArray[0][0]) 
        //get heapCardDatasArray 
        var heapCardDatasArray = majiangFactory.getHeapCardDatasArray(cmdBaseWorker.cbHeapCardInfo, cmdBaseWorker.TurnoverCard)       
    ...
    onCMD_SendCard:function() 
    {
        ...
        for(var i=0;i<cmdBaseWorker.cbSendCardCount;i++)
        {
            ...
            if(item.cbCardData == HAS_DISPATCH_CARD_DATA)//丢弃牌
                continue
            var cardData = (tableData.getUserWithChairId(cmdBaseWorker.wTakeCardUser).dwUserID == selfdwUserID||isRecordScene)?item.cbCardData:0
        ...
        else if(cmdBaseWorker.wGetSendCardUser!=INVALID_WORD && cmdBaseWorker.wGetSendCardUser==cmdBaseWorker.wTakeCardUser)
            switchUsers[0] = tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wTakeCardUser)        
    ...
    {    
        ...
        {
            var deleteCardData = (operateUser.dwUserID == selfdwUserID||isRecordScene)?cbReplaceCardData[i]:0
        ...
        if(cmdBaseWorker.bIsNxReplaceFlower)
            cmdBaseWorker.bIsNxReplaceFlower[operateUser.wChairID] = 1
    ...
    onCMD_GameEnd:function() 
    {
        ...
        if(cmdBaseWorker.endType == 3 || cmdBaseWorker.endType == 2)//翻马牌
        {
            var func = function(tmp)
            {
                ...
                {
                    playNode._showSprsOnGameEnd()
                    playNode.maDataBGA.setVisible(true) 
                    var selfID = tableData.getUserWithUserId(selfdwUserID).wChairID     
                    var gender = tableData.getUserWithChairId(selfID).cbGender                        
                    if(isOpenPTH == false)
                        playNode.playGenderEffect('maima', tableData.getUserWithChairId(selfID).cbGender)                    

                    for(var i=0; i<2; i++)
                    {
                        if(!cmdBaseWorker.cbMaCardData || typeof(cmdBaseWorker.cbMaCardData)=='undefined')
                            break;

                        var cardData = cmdBaseWorker.cbMaCardData[i]
                        var majiang = majiangFactory.getOne(cardData, 2, 0)
                        majiang.idxInHandMajiangs = i

                        majiang.x = playNode.MaDataNode.getContentSize().width/2
                        majiang.y = playNode.MaDataNode.getContentSize().height/2
                        majiang.setScale(0.1)
                      
                        playNode.MaDataNode.addChild(majiang)  
                        var action = cc.spawn(
                            cc.scaleTo(0.4, 1.3),
                            cc.moveTo(0.4, cc.p(552+i*96, playNode.MaDataNode.getContentSize().height/2))
                        );
                        if(majiang)
                            majiang.runAction(action);
                    }
                  
                    setTimeout(function(){
                        var HmaCount = playNode.getMaDataCountOnGameEnd()
                        for(var i=0; i<2; i++)
                        {
                            if(!HmaCount || typeof(HmaCount)=='undefined')
                                break;

                            var mlabel = new cc.LabelTTF('', 'Arial', 24)
                            mlabel.setString(HmaCount[i] + '冲')
                            mlabel.color = cc.color(255,255,130)
                            mlabel.setPosition(cc.p(552+i*96, playNode.MaDataNode.getContentSize().height*0.6))
                            playNode.MaDataNode.addChild(mlabel)
                        }
                ...
                setTimeout(function() //延时写入分数
                {
                    onPlayAnimationOfGameEnd()
                },2000)
            }
            playNode.playAnimationOfGameEnd(func)
        }
        else
    ...
    {
        /*
         *   吃碰杠之后先执行 onCMD_OperateResult， 在听的状态下点击出牌，会执行 onCMD_ListenResult
        */
        //吃牌之后，不能打的牌显示灰色
        var selfID = tableData.getUserWithUserId(selfdwUserID).wChairID
        if(selfID == INVALID_WORD)
            return

        if((cmdBaseWorker.wCurrentUser!=INVALID_WORD && cmdBaseWorker.wCurrentUser == selfID) || cmdBaseWorker.wOperateUser == selfID)
        {
            if(!cmdBaseWorker.cbHForbidOutData || typeof(cmdBaseWorker.cbHForbidOutData)=='undefined')
                return

            var data0 = cmdBaseWorker.cbHForbidOutData[selfID][0]
            var data1 = cmdBaseWorker.cbHForbidOutData[selfID][1]
            var handDatas = playNode.handMajiangs4D[0]

            //当手里的牌全部是不能出的牌，不显示灰色
            var count = 0
            if(handDatas[1] && (handDatas[1].cardData == data0 || handDatas[1].cardData == data1))
                count++
            ...
            if(handDatas[1] && ( handDatas[1].cardData == data0 || handDatas[1].cardData == data1))
            {
                handDatas[1].color = cc.color(155, 155, 155)
                handDatas[1].touchEnable = false
            }
            ...
            for(var i=0; i<handDatas[0].length; i++)
            {
                if(handDatas[0][i].cardData == data0 || handDatas[0][i].cardData == data1)
                {
                    handDatas[0][i].color = cc.color(155, 155, 155)
                    handDatas[0][i].touchEnable = false
                }
            }            
        }
    },
    ...
    setEnabledCardColorAfterOut:function()
    {
        ...
        if(selfID == INVALID_WORD)
            return
        if(!playNode.handMajiangs4D || typeof(playNode.handMajiangs4D)=='undefined')
            return

        var handDatas = playNode.handMajiangs4D[0]
        if(handDatas[1] && handDatas[1].color.r == 155){
            handDatas[1].touchEnable = true
            handDatas[1].color = cc.color(255, 255, 255)
        }

        for(var i=0; i<handDatas[0].length; i++)
        {
            if(!handDatas || typeof(handDatas)=='undefined')
                break;

            if(handDatas[0][i].color.r == 155){
                handDatas[0][i].touchEnable = true
                handDatas[0][i].color = cc.color(255, 255, 255)
            }
        }
    },

    getMaDataCountOnGameEnd:function()
    {
        var maFanCount = [0, 0]
        for(var i=0; i<2; i++)
        {
            if(cmdBaseWorker.cbMaCardData == null || typeof(cmdBaseWorker.cbMaCardData) == 'undefined')
                return

            var mIndex = cmdBaseWorker.cbMaCardData[2+i]
            var mNumber = mIndex % 9
            if(mIndex >= 27 || mNumber == 0)
                maFanCount[i] = 10
            else if(mNumber >= 1 && mNumber <= 4)
                maFanCount[i] = 5
            else
                maFanCount[i] = mNumber + 1
        }
        return maFanCount
    },
    ...
    _showSprsOnGameEnd:function()
    {
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            if(!cmdBaseWorker.cbHandCardData || typeof(cmdBaseWorker.cbHandCardData)=='undefined')
                continue;
            if(!cmdBaseWorker.cbHandCardCount || typeof(cmdBaseWorker.cbHandCardCount)=='undefined')
                continue; 

            displayHandIdxsArray[direction] = [cmdBaseWorker.cbHandCardData[chairid].slice(0, cmdBaseWorker.cbHandCardCount[chairid]), null]

            if(!cmdBaseWorker.dwChiHuKind || typeof(cmdBaseWorker.dwChiHuKind)=='undefined')
                continue;

            if(cmdBaseWorker.dwChiHuKind[chairid] == WIK_CHI_HU)
            {   
                var displayHandIdxs = displayHandIdxsArray[direction]
                displayHandIdxs[1] = cbProvideCardData
            }
        }
        playNode.handMajiangs4D = playNode.getHandMajiangsArray2(displayHandIdxsArray, tableData.getUserWithUserId(selfdwUserID).wChairID == INVALID_WORD)
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            if(!playNode.handGroupNode4D || typeof(playNode.handGroupNode4D)=='undefined')
                continue;
    ...
    getHandMajiangsArray2:function(handCardDatasArray, isLookon)
    {
        var handMajiangs4D = []
        for(showChairid=0;showChairid<GAME_PLAYER;showChairid++)//direction 0down 1right 2up 3left
        {
            var direction = showChairid
            if(!handCardDatasArray || typeof(handCardDatasArray)=='undefined')
                continue;
            ...
            for(var j=0;j<oldHandCardDatas.length;j++)
            {
                ...
                pos = playNode.getScaleNumber(majiang, pos, direction)
                majiang.x = pos.x
                majiang.y = pos.y
    ...
    getScaleNumber:function(majiang, pos, direction)
    {
        switch(direction)
        {
            case 0:
            {
                pos.x *= 0.8
                pos.y = majiang.getContentSize().height/2+5
                break;
            }
            case 1:
            case 3:
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            //胡型
            var mbHIsQiyiseOrHunyise = [false, false]
            ...
            for (var i = 0; i < map_mask2Name.length; i++) 
            {
                ...
                if (cmdBaseWorker.dwChiHuRight[wChairID] & chr_type)
                {
                    ...
                    if(map_mask2Name[i][1] == '清一色')
                        mbHIsQiyiseOrHunyise[0] = true
                    else if(map_mask2Name[i][1] == '混一色')
                        mbHIsQiyiseOrHunyise[1] = true
                }
            }
            if(cmdBaseWorker.bHIsFlag4Magic && cmdBaseWorker.bHIsFlag4Magic[wChairID])
            ...
            majiangsNode.scale = 0.5
            majiangsNode.x = 100
            //majiangsNode.y = gendBar.height - 12 - 0.5*(majiangFactory.down_handHeight*majiangFactory.scale_upDown*majiangsNode.scale)
            majiangsNode.y = gendBar.height - 7 - 0.5*(majiangFactory.down_handHeight*majiangFactory.scale_upDown*majiangsNode.scale)
            ...
            cmdBaseWorker.bShowFlowerBigger = true
            if(cmdBaseWorker.bShowFlowerBigger)
            {
                flowersNode.scale = 0.55
                if(cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU)
                    flowersNode.x = 200
                else
                    flowersNode.x = 100
                flowersNode.y = (gendBar.height - 12 - 0.5*(majiangFactory.down_handHeight*majiangFactory.scale_upDown*majiangsNode.scale))*0.88
            }
            else
            {
            ...
            {
                if(!flowerCardDatas[j] || typeof(flowerCardDatas[j])=='undefined')
                    continue;

                var cardData = flowerCardDatas[j]
                var mHNumber = cmdBaseWorker.bShowFlowerBigger?2:4
                var majiang = majiangFactory.getOne(cardData, mHNumber, 0)
                var pos = majiangFactory.getFlowerMajiangPosAndTag(j, 0)
                majiang.x = cmdBaseWorker.bShowFlowerBigger? (majiang.getContentSize().width*(flowerCardDatas.length-j)*1.35):pos.x
             ...
             //添加承包
            if(cmdBaseWorker.wUndertakeUser && cmdBaseWorker.wUndertakeUser[wChairID] != 0)
                playNode.addChengbaoSpriteOnGameEnd(gendBar, wChairID)  
            ...
            else if(cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU)
            {
                ...
                //添加马牌
                playNode.addMaCardSpriteOnGameEnd(gendBar, majiangsNode, flowerCardDatas.length)
                
                //添加番数                
                playNode.addShowLabelOnGameEnd(gendBar, wChairID, mbHIsQiyiseOrHunyise)
            }
    ...
    addChengbaoSpriteOnGameEnd:function(gendBar, wChairID)
    {
        if(wChairID == INVALID_WORD || cmdBaseWorker.endType != 2)
            return;

        var spr = new cc.Sprite('gameRes/pic/chengbao.png')
        if(spr)
        {
            spr.setScale(0.55)
            spr.anchorX = 0
            spr.setPosition(cc.p(gendBar.getContentSize().width * 0.72, 32))
            gendBar.addChild(spr)
        }
       
    },

    addMaCardSpriteOnGameEnd:function(gendBar, majiangsNode, mlength)
    {
        var mMaDataNode = new cc.Node()
        if(!gendBar || !majiangsNode)
            return

        mMaDataNode.scale = 0.7
        mMaDataNode.x = gendBar.getContentSize().width*0.78
        mMaDataNode.y = (gendBar.height - 12 - 0.5*(majiangFactory.down_handHeight*majiangFactory.scale_upDown*majiangsNode.scale))*1.72
  
        gendBar.addChild(mMaDataNode)
        ...
        for(var m=0; m<2; m++)
        {
            ...
            var mCardData = cmdBaseWorker.cbMaCardData[m]
            var mMajiang = majiangFactory.getOne(mCardData, 2, 0)
            mMajiang.idxInHandMajiangs = m

            var pos = majiangFactory.getFlowerMajiangPosAndTag(m, 0)
            mMajiang.x = (mMajiang.getContentSize().width*(2-m)*1.35)//cmdBaseWorker.bShowFlowerBigger? (mMajiang.getContentSize().width*(2-m)*1.05 + mlength * mMajiang.getContentSize().width):(mMajiang.getContentSize().width*(2-m)*1.1)
            mMajiang.y = 5-0.5*(majiangFactory.down_handHeight*majiangFactory.scale_upDown*majiangsNode.scale)-0.5*mMajiang.height*mMajiang.scale-15
            if(mMaDataNode && mMajiang)
                mMaDataNode.addChild(mMajiang)
        }
    },

    addShowLabelOnGameEnd:function(gendBar, wChairID, mbHIsQiHunyise)
    {
        if(!gendBar)
            return

        //添加台数
        //cbFlowerFlag[GamePlayer][4], 0花牌，1四花，2八花，3花牌总番数
        var mShowTaiLabel = cc.LabelTTF.create('', "Helvetica", 16)
        mShowTaiLabel.anchorX = 0
        mShowTaiLabel.setFontFillColor( cc.color(244, 230, 159) )
        mShowTaiLabel.setPosition(cc.p(gendBar.getContentSize().width * 0.5, 35))  

        if(!cmdBaseWorker.cbFlowerFlag || !cmdBaseWorker.cbZiKeCount)
            return
        if(typeof(cmdBaseWorker.cbFlowerFlag) == 'undefined' || typeof(cmdBaseWorker.cbZiKeCount) == 'undefined')
            return

        var mFlowerFlag = cmdBaseWorker.cbFlowerFlag[wChairID]
        var mZiCount = cmdBaseWorker.cbZiKeCount[wChairID]
        var taiString = '台数:'
        if(mFlowerFlag[2] == 1)
            taiString += '  八花3'
        else
        ...
        if(mZiCount != 0)
            taiString += '  字牌' + mZiCount
        if(mbHIsQiHunyise[0])
            taiString += '  清一色3'
        if(mbHIsQiHunyise[1])
            taiString += '  混一色2'  

        if(!cmdBaseWorker.cbMagicCount || typeof(cmdBaseWorker.cbMagicCount)=='undefined')
            return;

        var mMagicCount = cmdBaseWorker.cbMagicCount[wChairID]
        if(mMagicCount == 3)
            taiString += '  3财神'
        else if(mMagicCount == 4)
            taiString += '  4财神'
        ...
        if(mShowTaiLabel)
        {
            mShowTaiLabel.setString(taiString)        
            gendBar.addChild(mShowTaiLabel) 
        }

        //添加买马数
        var mShowMaLabel = cc.LabelTTF.create('', "Helvetica", 16)
        mShowMaLabel.anchorX = 0
        mShowMaLabel.setFontFillColor( cc.color(244, 230, 159) )
        mShowMaLabel.setPosition(cc.p(gendBar.getContentSize().width * 0.5, 15))
        var mMaCount = playNode.getMaDataCountOnGameEnd()
        if(!mMaCount || typeof(mMaCount)=='undefined')
            return;

        var maString = '冲数:'
        if(!cmdBaseWorker.cbFlagMaxCount || !mMaCount)
            return

        if(cmdBaseWorker.cbFlagMaxCount[wChairID] == 3)
            maString += '  8冲封顶'
        else           
            maString += '  ' + Math.pow(2, cmdBaseWorker.cbFlagMaxCount[wChairID]) + '冲'

        maString += '  买马' + (mMaCount[0] + mMaCount[1]) + '冲'
        if(mShowMaLabel)
        {
            mShowMaLabel.setString(maString)
            gendBar.addChild(mShowMaLabel)
        }
    },
    ...
    showLaizi:function()
    {
        ...
        bg.x = 45
        bg.y = - 45
        var mj = majiangFactory.getOne(0x37, 1, 0, false, 1)
        mj.x = 45
        mj.y = - 45
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        else if(cmdBaseWorker.endType == 2)
        {
            ...
            var name = isOpenPTH?'zimo0':'ZMchong'
            playNode.playGenderEffect(name, tableData.getUserWithChairId(cmdBaseWorker.wProvideUser).cbGender)
        ...
        else if(cmdBaseWorker.endType == 3)
        {
            ...
            for(var i=0;i<GAME_PLAYER;i++)
            {
                if(cmdBaseWorker.dwChiHuKind[i] == WIK_CHI_HU)
                {
                    ...
                    var name = isOpenPTH? 'hu0':'FPchong'
                    playNode.playGenderEffect(name, tableData.getUserWithChairId(i).cbGender)
    ...
    {
        //var resPrefix = 'gameRes/sound/' + 'pth'/*(isOpenPTH?'pth':'fy')*/
        var resPrefix = 'gameRes/sound/' + (isOpenPTH?'pth':'fy')
        if(isOpenPTH == false)
            name = playNode.getGenderEffectNameType(name)
    ...
    getGenderEffectNameType:function(name)
    {
        //只有方言的时候，才会执行
        if(isOpenPTH)
            return name
        ...
        {
            var operateUser = tableData.getUserWithChairId(cmdBaseWorker.wOperateUser)
            if(cmdBaseWorker.bIsNxReplaceFlower && cmdBaseWorker.bIsNxReplaceFlower[operateUser.wChairID] == 0)
                return name
            else
                return 'replace1'
        }
        ...
        if(name == 'maima')
         ...
         //出牌
        if(name=='2' || name=='4' || name=='8' || name=='17' || name=='19'|| name=='21' || name=='24' || name=='37' ||
           name=='39'|| name=='41'|| name=='49'|| name=='50' || name=='51' || name=='52' || name=='53'  )  
           {
                var num = Math.round(Math.random()*1)
                return  name += '_' + num
           }    

        if(cmdBaseWorker.bmyselfGameEndtype == false)
            return name

        if(cmdBaseWorker.endType != 2 &&  cmdBaseWorker.endType != 3)   
            return name       
        ...
        {            
            ...
            {
                var chr_type = window[ map_mask2Name[i][0] ] 
                if(!cmdBaseWorker.dwChiHuRight || typeof(cmdBaseWorker.dwChiHuRight)=='undefined')
                    continue;

                if ((cmdBaseWorker.dwChiHuRight[wChairID] & chr_type))
                {
                    //hasType = true
                    if(map_mask2Name[i][1] == '混一色')
                        return 'hu1'                    
                }
            }
        } 
        ...
        if(name == 'ZMchong' || name == 'FPchong')
        {
            ...
            for(var showChairid=0;showChairid<4;showChairid++)
            {
                ...
                if(cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU)
                {
                    if(!cmdBaseWorker.cbFlagMaxCount || typeof(cmdBaseWorker.cbFlagMaxCount)=='undefined')
                        continue;
                    
                    var count = cmdBaseWorker.cbFlagMaxCount[wChairID] 
                    if(count == 0 && cmdBaseWorker.wProvideUser == wChairID)
                        return 'hu2'
                    else if(count == 0)
                        return 'hu0'
                    ...
                    {                    
                        if(count == 3)
                            return 'ZMchong_' + cmdBaseWorker.cbFlagMaxCount[wChairID]+ '_' + num
                        else
                            return 'ZMchong_' + cmdBaseWorker.cbFlagMaxCount[wChairID]
                    }
                    else if(name == 'FPchong')
                    {
                        if(count == 3)
                            return 'FPchong_' + cmdBaseWorker.cbFlagMaxCount[wChairID]+ '_' + num
                        else
                            return 'FPchong_' + cmdBaseWorker.cbFlagMaxCount[wChairID]
                    }                   
                }
            }            
        }
    ...
    {
        playNode.MaDataNode.removeAllChildren()
        playNode.maDataBGA.setVisible(false)
    ...
    //////////////////// 12.21-pm, start////////////////////////
    resetPlayNode2:function()
    {
        if(!cmdBaseWorker.bIsFreeStatus)
            return
        ...
        {   
            var user = tableData.getUserWithTableIdAndChairId(null, chairId)
            if(user)
                playNode.clearCurrentRoundNode2(user.userNodeInsetChair.currentRoundNode)
        }
    },
    clearCurrentRoundNode2:function(currentRoundNode)
    {
        currentRoundNode.handMajiangsNode.removeAllChildren()
        currentRoundNode.discardMajiangsNode.removeAllChildren()
        currentRoundNode.weaveMajiangsNode.removeAllChildren()
    ...
    showActionBtns:function(sortedActions)
    {
        for(var i=sortedActions.length-1;i>=0;i--)
        {
            if(sortedActions[i] == WIK_REPLACE)
            {
                ...
                var mdelayTime = cmdBaseWorker.bIsFirstReplace ? 2800:350
                setTimeout(function()
                {
                    cmdBaseWorker.sendMessage_operateCard(operateCards, WIK_REPLACE)
                },  mdelayTime)  //测试时候，时间长一点，方便测试查看
        ...
        {   
            ...
            btn.setVisible(true)
            btn.setPositionX(-122 * (sortedActions.length-1-i +1))
    ...
    bindActionListener:function()
    {
        ...
        var actionCall_penggang = function()
        {
            ...
            else
            {
                ...
                weaveItems = gameLogic.analyseCard_PengGang(cmdBaseWorker.WeaveItemArray[selfChairId], handCardDatas)
            }
    ...
    onActionMinGang:function(operateWeaveIdx, operateWeaveItem, operateUserChairId, provideUserChairId, provideCardData)
    {
        ...
        var addHandCardData = (operateUser.dwUserID == selfdwUserID||isRecordScene)?provideCardData:0
        ...
        for(var i=0;i<4;i++)
        {
            var deleteCardData = (operateUser.dwUserID == selfdwUserID||isRecordScene)?operateWeaveItem.cbValidCardDatas[i]:0
    ...
    {        
        ...
        majiangFactory.deleteHandMajiangs(operateHandMajiangs, operateDir, maxRightCardData)
    ...
    {        
        ...
        var addHandCardData = (operateUser.dwUserID == selfdwUserID||isRecordScene)?provideCardData:0
        majiangFactory.addHandMajiang(operateHandMajiangs, operateDir, addHandCardData, operateHandGroupNode, playNode.weaveMajiangs4D[operateDir].length)

        for(var i=0;i<3;i++)
        {
            var deleteCardData = (operateUser.dwUserID == selfdwUserID||isRecordScene)?operateWeaveItem.cbValidCardDatas[i]:0
            majiangFactory.deleteHandMajiangs(operateHandMajiangs, operateDir, deleteCardData)
        }
