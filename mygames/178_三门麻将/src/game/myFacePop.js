...
var myFacePoP = 
{
    ...
    PreLoadResOnMyfacePop:function()
    {
        myFacePoP.onPreLoadRes()
        var pngArray = [
            'animationFace.plist', 
            'animationFace.png',
            'pageBtn0_1.png',
        ...
        var respArray = myFacePoP.getPreLoadRes()
        for(var count=2; count<respArray.length; count++)
        {
            var t = new cc.SpriteFrame(respArray[count], cc.rect(0, 0, 2, 2))
            cc.spriteFrameCache.addSpriteFrame(t, pngArray[count])
        } 
    },
    ...
    {  
        if(myFacePoP.pop)
            myFacePoP.pop.removeAllChildren()
        myFacePoP.pop = null
    },
    ...
    {  
        ...
        myFacePoP.onReStart()  //断线重连语音播放不了，
        ...
        button.addClickEventListener(function(button) {
            ...
            if(!myFacePoP.pop)
            {
                var pop =  myFacePoP.getPop(uc_faceIdArray, rowNum, lineNum)
        ...
        var l = cc.EventListener.create({
            ...
            {   
                ...
                var spr = actionFactory.getSprWithAnimate('face' + (faceId+1) + '_', true, null, null, null, 2)
    ...
    {  
        ...
        for(var i=0;;i++)
        {
            ...
            listView.pushBackCustomItem( myFacePoP._getOneLineFaceItem( 
                {
                    faceIds:s
                } ) )
        }
        ...
        var listener = cc.EventListener.create({
            ...
            onTouchEnded: function (touch, event) {
                myFacePoP.pop.setVisible(false)
            }
        })
    ...
    _getOneLineFaceItem:function(params)
    {
        ...
        for(var i=0;i<faceIds.length;i++)
        {
            ...
            var btn = myFacePoP._getFaceBtn(faceId, params.callback)
            btn.setPosition( cc.p( myFacePoP.width/2 + i*myFacePoP.width, myFacePoP.height/2 ) )
    ...
    {
        var self = this
        var resp = faceId>100?'gameRes/pic/':myFacePoP.resp //公共表情最多100个
    ...
    showCommonVoice:function(user, voiceIndex)
    {
        ...
        var str = '_' + num
        if( user.cbGender )
            managerAudio.playEffect('gameRes/sound/commonVoice/man/commonVoice' + soundId + str + '.mp3')
        else
            managerAudio.playEffect('gameRes/sound/commonVoice/woman/commonVoice' + soundId + str + '.mp3')
