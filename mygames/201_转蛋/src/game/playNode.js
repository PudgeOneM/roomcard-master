...
{   
    ...
    SortCardWithType:function(sortType)
    {
        ...
        switch( sortType )
        {
            case 1:
            {
                ...
                {
                    var logicValueA = gameLogic.GetCardLogicValue(a)
                    var logicValueB = gameLogic.GetCardLogicValue(b) 
            ...
            case 2:
            {
                handCards.sort(function(a,b)
                {
                    var cmpValueA = gameLogic.GetCardColor(a) * 1000 + gameLogic.GetCardLogicValue(a)
                    var cmpValueB = gameLogic.GetCardColor(b) * 1000 + gameLogic.GetCardLogicValue(b)
            ...
            case 3:
            {
                handCards = gameLogic.SortCardWithPriorityLv(handCards).sortCards
                break
            }
        }
    ...
    _outCardCall1: function(cards) 
    {
        ...
        {
            var lastOutType = (cmdBaseWorker.wOutCardUser != INVALID_WORD && cmdBaseWorker.wOutCardUser != t) ? cmdBaseWorker.outCardType.cbType : CT_ERROR
            if ( gameLogic.GetCardType(cards, lastOutType, cmdBaseWorker.cbHandCardCount[t]).cbType == CT_ERROR )
                isAllowOut = false
            else if ( cmdBaseWorker.wOutCardUser != INVALID_WORD && cmdBaseWorker.wOutCardUser != t && !gameLogic.CompareCard(cards, cmdBaseWorker.cbHandCardCount[t], cmdBaseWorker.outCardType) )
                isAllowOut = false
        }
    ...
    popOutCardNode: function(isCan) 
    {
        ...
        playNode.outCardButton3.setEnabled( cmdBaseWorker.wOutCardUser != INVALID_WORD && cmdBaseWorker.wOutCardUser != selfChairId )
    ...
    playCardTypeEffect: function(cardType, isMan) 
    {
        ...
        else if ( cardType.cbType == CT_BOMB || ( typeof(CT_FLUSH_LINE) != 'undefined' && cardType.cbType == CT_FLUSH_LINE ) )
            name = "zhadan"
        ...
        if ( cardType.cbType == CT_BOMB || (typeof(CT_FLUSH_LINE) != 'undefined' && cardType.cbType == CT_FLUSH_LINE) )
