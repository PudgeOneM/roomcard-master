...
{   
    ...
    {   
        ...
        // playNode.scoreTTF.x = playNode.timerNode.x
        // playNode.scoreTTF.y = playNode.timerNode.y
...
//     playNode.popGameEnd(null, ['诶我去无群二', '多撒多群多群大所', '大大大大法师的法师大大大大法师的法师', '大大舒服撒发生发我的'])
    ...
    initCurrentRoundNode:function(currentRoundNode)
    {
        ...
        currentRoundNode.listenMajiangNode = new cc.Node()
        currentRoundNode.addChild(currentRoundNode.listenMajiangNode)
    ...
    {           
        ...
        currentRoundNode.listenMajiangNode.removeAllChildren()
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        if(direction==0)
        {
            ...
            currentRoundNode.listenMajiangNode.x = currentRoundNode.handMajiangsNode.x
            currentRoundNode.listenMajiangNode.y = currentRoundNode.handMajiangsNode.y
            currentRoundNode.listenMajiangNode.setLocalZOrder(3)
            //weave
    ...
    _registEvent:function() 
    {
        var l = cc.EventListener.create({
            ...
            {   
                ...
                if(user.wChairID == tableData.getServerChairIdWithShowChairId(0) && cmdBaseWorker.cbListenState[user.wChairID] == 1)
                {
                    playNode.majiangTouchOne(user)
                }
            }
        })
    ...
    {   
        ...
        playNode.actionCall_chi = function()
        {
            ...
            if(actions.length>1)
            {
                ...
                majiangFactory.showChoosePopOfAction(cardDatasArray, actions, sendChi)
            }
            else
            {
...
console.log(888888, isMinGang, clone(cmdBaseWorker.wProvideUser) )
            ...
            else
            {
                ...
                var cardDatas = cmdBaseWorker.searchGangCardDatas(playNode.handMajiangs4D[direction], playNode.weaveMajiangs4D[direction])
                ...
                if(cardDatas.length>1)
                {
                    ...
                    majiangFactory.showChoosePopOfAction(cardDatasArray, actions, sendGang)
                }
                else
                {
        ...
        playNode.actionCall_ting = function()
        {
           ...
           // playNode.showGangCard(cmdBaseWorker.wOperateUser)
            playNode.hideActionBtns()
        }
    ...
    showGangCard:function(operateUserObj)
    {
        var operateUser = cmdBaseWorker.wOperateUser == INVALID_WORD?cmdBaseWorker.wCurrentUser:cmdBaseWorker.wOperateUser
        var sChairId = operateUserObj.wChairID //获取服务器中自身位置的序号
        var operateUserDir = tableData.getShowChairIdWithServerChairId(sChairId) //客户端中的位置
       // console.log("operateUserDir:" + operateUserDir)

        if(cmdBaseWorker.cbListenState[operateUserObj.wChairID] == 2 )
        {
            var isListenOutCardDatas = cmdBaseWorker.cbListenCardData[operateUserObj.wChairID]
            var parent = topUINode.listenerTop
            parent.removeAllChildren()
            var currentShowNode = new cc.Node()
            currentShowNode.scale = 0.7
            var mj = majiangFactory.getOne(1, 0, 0, true, true)

            var count = 0;
            while(isListenOutCardDatas[count] != 0)
                count++
            for(var i = 0 ;i<count;i++)
            {
                playNode.showListenCard(parent,isListenOutCardDatas[i],mj,currentShowNode,count)
            }
            parent.addChild(currentShowNode)  
            parent._children[0].setVisible(false)

            var listenSpr = new cc.Sprite('#btn_ListenTs.png')
            parent.addChild(listenSpr)

            listenSpr.x = parent.width - listenSpr.width/2 - 30
            listenSpr.y += listenSpr.height/2 + 130

            playNode.addlistenerToListenTs(listenSpr,parent)

            if(tableData.getUserWithUserId(selfdwUserID).wChairID == cmdBaseWorker.wCurrentUser)
            {
                playNode.Auto_card()
            }
        ...
        {
           /* var listenTag = cc.Sprite('#ting.png')
            topUINode.listenerTop.addChild(listenTag)*/

            if(operateUserObj.dwUserID == selfdwUserID)
            {
                
                console.log(" 抽到的牌？: "+ cmdBaseWorker.cbProvideCardData)
                var isListenOutCardDatas = cmdBaseWorker.cbOperateListenCardData[sChairId]
                var operateHandMajiangs4D = []
                operateHandMajiangs4D = playNode.handMajiangs4D[0][0]  //0为手牌 1是摸到的牌

                console.log("operateHandMajiangs4D : " + operateHandMajiangs4D)
                console.log("要比较的牌 : " + isListenOutCardDatas)
                for(var j=0;j<operateHandMajiangs4D.length;j++)
                {
                    var cardData = operateHandMajiangs4D[j].cardData
                    var isAllowOut = false
                    for(var x= 0;x<isListenOutCardDatas.length ;x++)
                    {
                        if(isListenOutCardDatas[x] == cardData && isListenOutCardDatas[x] != 0)
                        {
                            isAllowOut = true
                            console.log("可以出的牌 :"  + j + "   "+ operateHandMajiangs4D[j].cardData)
                        }
                    }
                    if(!isAllowOut)
                        operateHandMajiangs4D[j].color = cc.color(100,100,100)
                }
                operateHandMajiangs4D = playNode.handMajiangs4D[0][1]
                operateHandMajiangs4D.color = cc.color(100,100,100)
                for(var x= 0;x<isListenOutCardDatas.length ;x++)
                {
                    if(isListenOutCardDatas[x] == operateHandMajiangs4D.cardData)
                    {
                        operateHandMajiangs4D.color = cc.color(255,255,255)
                        break
                    }
                }
                return
            ...
            var operateWeaveMajiangs = playNode.weaveMajiangs4D[operateUserDir]  //被换的牌
            var cardDataNum = 0;
            for(var i = 0;i<operateWeaveMajiangs.length;i++)
            {
                ...
                if(group.length == 4)
                {
                    var cardData = cmdBaseWorker.sendCardAnGang[sChairId][cardDataNum]
                    cardDataNum++;
                    var parent = operateUserObj.userNodeInsetChair.currentRoundNode.weaveMajiangsNode
                    parent.removeChild(group[0])
                    parent.removeChild(group[1])
                    parent.removeChild(group[2])
                    parent.removeChild(group[3])
                    var majiangsOneGroup = majiangFactory.weaveItem2Majiangs(i, operateUserDir, {
                    ...
                    {
                        parent.addChild(majiangsOneGroup[j])
                    }
                    operateWeaveMajiangs[i] = majiangsOneGroup
                }
            }
        }
    },
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        //playNode.scoreTextTTF._visible = true
        playNode.showLaizi()
        topUINode.listenerTop.removeAllChildren()
        
        //初始化CardData2ScoreMap
        var cardDatas = []
        var scores = []
        var magicCardData = cmdBaseWorker.cbMagicCardData 
        //tableNode.fengQuanTTF.x += 50
        ...
        playNode.scoreTTF._visible = true
        playNode.ScoreSprTTF._visible = true
        ...
        if(cmdBaseWorker.wOutCardUser!=INVALID_WORD)
        {
            ...
            console.log(12121212,cmdBaseWorker.cbOutCardData)
        ...
        playNode.sendCardsAction(handCardDatasArray, discardCardDatasArray, weaveItemArray, heapCardDatasArray, cmdBaseWorker.cbPlayerFlowerCardData) 
    ...
    onCMD_GameStart:function() 
    {
        ...
        var scores = []
        var magicCardData = cmdBaseWorker.cbMagicCardData 
        var flowerCardData = cmdBaseWorker.cbFlowerCardData
        //tableNode.fengQuanTTF.x += 50
        switch(flowerCardData[0])
        {
            case 49:tableNode.fengQuanTTF.setString("东风圈");break;
            case 50:tableNode.fengQuanTTF.setString("南风圈");break;
            case 51:tableNode.fengQuanTTF.setString("西风圈");break;
            case 52:tableNode.fengQuanTTF.setString("北风圈");break;
        }
        ...
        cmdBaseWorker.sortHandCardDatas(handCardDatasArray[0][0]) 
        cmdBaseWorker.cisShowGangData = 2
        //get heapCardDatasArray 
        var heapCardDatasArray = majiangFactory.getHeapCardDatasArray(cmdBaseWorker.cbHeapCardInfo, cmdBaseWorker.TurnoverCard)       
    ...
    onCMD_OutCard:function() 
    {
        ...
        console.log("outCardData:" + outCardData)
    ...
    onCMD_SendCard:function() 
    {
        ...
        else if(isSelf && cmdBaseWorker.cbListenState[cmdBaseWorker.wTakeCardUser] == 2)
        {
            playNode.Auto_card()
        }
    },
    onCMD_OperateResult:function() 
    {
        ...
        var isSelf = cmdBaseWorker.wOperateUser == tableData.getUserWithUserId(selfdwUserID).wChairID
        if(cmdBaseWorker.cbOperateCode != WIK_NULL)
        {
            ...
            majiangFactory.onActionResult(cmdBaseWorker.cbOperateCode, cardDatas, operateUser, provideUser,
                majiangs4W4D, playNode.handGroupNode4D)

            playNode.playAction(cmdBaseWorker.cbOperateCode, operateUser)

        }
        console.log(cmdBaseWorker.cisShowGangData)
        if(cmdBaseWorker.cbListenState[cmdBaseWorker.wOperateUser] == 1)
            playNode.showGangCard(tableData.getUserWithChairId(cmdBaseWorker.wOperateUser))
        //动作后动作
        var sortedActions = majiangLogic.getSortedActionsWithMask(cmdBaseWorker.cbActionMask)
        if(sortedActions.length>0)
        {
        ...
        else if(isSelf && cmdBaseWorker.cbOperateCode == WIK_NULL && cmdBaseWorker.wOperateUser == cmdBaseWorker.wProvideUser)
        {
            if(cmdBaseWorker.cbListenState[cmdBaseWorker.wOperateUser] == 2)
                    playNode.Auto_card()
        }
    },
    Auto_card:function(){
        playNode.AutoCard = cc.sequence(
            cc.delayTime(0.5),
        ...
        playNode.node.runAction(playNode.AutoCard)
        /*cc.delayTime(5)
        cmdBaseWorker.wCurrentUser = INVALID_WORD
        console.log("进入自动出牌")
        var OutCard = getObjWithStructName('CMD_C_OutCard')
    ...
    showListenCard:function(handGroupNode,cardData,currentMajiang,currentMajiangTipsNode,count)
    {
        var majiangNode = new cc.Node()
        
        var bg = new cc.Sprite('#mf_currentMjBg.png')
        bg.setScale(majiangFactory.scale_upDown)
        majiangNode.addChild(bg)

        var mj = majiangFactory.getOne(1, 0, 0, true, true)
        mj.setScale(majiangFactory.scale_upDown * 1)
        majiangNode.addChild(mj)
        ...
        currentMajiangTipsNode.addChild(majiangNode)

        width = currentMajiangTipsNode._children.length
        ...
        if(count >= 9)
            origin_X = 8 * (-mj.width - 40) /2
        else
            origin_X = (count - 1) * (-mj.width - 40) /2

        currentMajiangTipsNode.y = handGroupNode.height/2 - currentMajiang.height * 2 + 10
        
        mj.getChildByTag(101).setSpriteFrame('mf_' + cardData+ '.png') 

        currentMajiangTipsNode.x = handGroupNode.width /2

        majiangNode.x = origin_X + (width -1 ) * (mj.width + 40)

        majiangNode.y = height * (mj.height + 20)


    },
    majiangTouchOne:function(user)
    {   //user.wChairID  服务器椅子
        /*var handMajiangs = playNode.handMajiangs4D[0][0]
        handMajiangs.push(playNode.handMajiangs4D[0][1])*/

        

        var handMajiangs = playNode.handMajiangs4D[0][0].slice(0,playNode.handMajiangs4D[0][0].length)
        handMajiangs.push(playNode.handMajiangs4D[0][1])
        //var parent = user.userNodeInsetChair.currentRoundNode.listenMajiangNode

        var parent = topUINode.listenerTop

        parent.removeAllChildren()

        var isListenOutCardDatas = cmdBaseWorker.cbOperateListenCardData[user.wChairID]
        var currentShowNode = new cc.Node()
        currentShowNode.scale = 0.7
        var maxHeight = handMajiangs[0].y
        for(var i= 0 ;i<handMajiangs.length;i++)
        {
            if(maxHeight <handMajiangs[i].y)
                maxHeight = handMajiangs[i].y
        }

        for(var i =0 ;i<handMajiangs.length;i++)
        {
            console.log("handMajiangs : " + handMajiangs[i].cardData)
            console.log("handMajiangs.y : " + handMajiangs[i].y)
            if(handMajiangs[i].y == maxHeight)
            {
                var majiang = handMajiangs[i]
                for(var j = 0 ;j<isListenOutCardDatas.length;j++)
                {
                    if(majiang.cardData == isListenOutCardDatas[j] && isListenOutCardDatas[j] != 0 )
                    {
                        var showCards = cmdBaseWorker.showListenCardData[j]
                        var count = 0;
                        while(showCards[count] != 0)
                            count++
                        ...
                        for(var k = 0;k<count;k++)
                        {
                            if(showCards[k]!=0)
                            {
                               playNode.showListenCard(parent,showCards[k],majiang,currentShowNode,count)
                            }
                        }
                        //user.userNodeInsetChair.currentRoundNode.listenMajiangNode.removeAllChildren()
                        parent.addChild(currentShowNode)

                        console.log("isListenOutCardDatas:" + isListenOutCardDatas)
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            var User = tableData.getUserWithUserId(selfdwUserID)
            //console.log(66666,cmdBaseWorker.cisShowGangData)
            if(cmdBaseWorker.cbListenState[User.wChairID] == 1 && isAllowOut)
            {

                playNode.majiangTouchOne(User)
                isAllowOut = false
                
                var operateHandMajiangs4D = playNode.handMajiangs4D[0][0]
                var isListenOutCardDatas = cmdBaseWorker.cbOperateListenCardData[User.wChairID]

                for(var i = 0 ;i<isListenOutCardDatas.length;i++)
                {
                    if(majiang.cardData == isListenOutCardDatas[i] && isListenOutCardDatas[i] != 0)
                    {
                        isAllowOut = true
                        break
                    }
                }
                console.log("听牌的isAllowOut : "  + isAllowOut)
                console.log("operateHandMajiangs4D:" + operateHandMajiangs4D.length)
                ...
                if(isAllowOut )
                {
                   ...
                   // var parent = User.userNodeInsetChair.currentRoundNode.listenMajiangNode
                    parent._children[0].setVisible(false)
                    for(var i = 0 ;i<operateHandMajiangs4D.length;i++)
                    {
                        operateHandMajiangs4D[i].color = cc.color(255,255,255)
                    }
                    playNode.handMajiangs4D[0][1].color = cc.color(255,255,255)
                    console.log("麻将恢复正常")
            ...
            if(isAllowOut && cmdBaseWorker.cbListenState[tableData.getUserWithUserId(selfdwUserID).wChairID] != 2)
            {
                if(cmdBaseWorker.cbListenState[tableData.getUserWithUserId(selfdwUserID).wChairID] == 1)
                    cmdBaseWorker.cbListenState[tableData.getUserWithUserId(selfdwUserID).wChairID] = 2
    ...
    addlistenerToListenTs:function(spr,listenNode)
    {
        cc.eventManager.addListener({
            ...
            onTouchBegan:function(touch,event)
            {
                ...
                if(!cc.rectContainsPoint(rect,locationInNode))
                    return false
                listenNode._children[0].setVisible(true)
                return true
            },
            onTouchEnded:function(touch,event)
            {
                listenNode._children[0].setVisible(false)
                console.log("点击听牌提示")
             //   return true
            }
        },spr)
    ...
    {   
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            if(self.dwUserID == user.dwUserID && cmdBaseWorker.cbListenState[self.wChairID] != 0)
            {
                playNode.showGangCard(user)
            }
        }
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        var control = {}
        playNode.scoreTTF._visible = false
        playNode.ScoreSprTTF._visible = false
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
           ...
           /* var resultTTF = cc.LabelTTF.create('', "Helvetica", 16)
            ...
            var resultTTF = cc.LabelTTF.create('', "Helvetica", 12)
            resultTTF.setFontFillColor( cc.color(244, 230, 159) )
            var chrStr = ''
            if(cmdBaseWorker.dwChiHuKind[wChairID] == WIK_CHI_HU)
            {
                for (var i = 0; i < scoreKind.length; i++) 
                {
                    if (cmdBaseWorker.m_lGameScore2Name[i] != 0 )
                    {
                        if(i < 5)
                        {
                            chrStr += scoreKind[i] + cmdBaseWorker.m_lGameScore2Name[i] + ' ';
                        }
                        else
                            chrStr += scoreKind[i] + ' ';
                    }
            ...
            //var self = tableData.getUserWithUserId(selfdwUserID)
            /*var weaveScoreTTF = cc.LabelTTF.create('',"Helvetica",16)
            weaveScoreTTF.setFontFillColor( cc.color(244, 230, 159) )
            var chrStr = ''
            if(showChairid == wChairID)
            {
                for(var i = 0;i<GAME_PLAYER;i++)
                {
                    var weaveScore = cmdBaseWorker.lGameWeaveScore[i]
                }
                chrStr += ''
            }
            weaveScoreTTF.setString(chrStr)
            weaveScoreTTF.anchorX = 0
            weaveScoreTTF.x = 720
            weaveScoreTTF.y = 0
            gendBar.addChild(weaveScoreTTF)*/
    ...
    //             if(!hasLightProvideMj && wChairID== cmdBaseWorker.wWinner && majiang.cardData == cmdBaseWorker.cbProvideCardData)
    //             {
    //                 majiang.color = cc.color(188, 255, 188)
    //                 hasLightProvideMj = true 
    //             }
    ...
    //         if(wChairID==cmdBaseWorker.wExitUser)
    //             control['winflag'+wChairID].setSpriteFrame('gendFlag0.png') 
    //         else if(wChairID==cmdBaseWorker.wWinner && cmdBaseWorker.endType == 2)
    //             control['winflag'+wChairID].setSpriteFrame('gendFlag1.png') 
    //         else if(wChairID==cmdBaseWorker.wProvideUser && cmdBaseWorker.endType == 3)
    //             control['winflag'+wChairID].setSpriteFrame('gendFlag2.png')
    //         else
    ...
    showLaizi:function()
    {
        for(var i=0;i<TURNOVER_COUNT_MAGIC;i++)
        {
            ...
            var fanpai = new cc.Sprite('#caiShenBz.png')
            fanpai.x = mj.x + 40
            fanpai.y = mj.y

            playNode.laiziNode.addChild(bg)
            playNode.laiziNode.addChild(mj)
            playNode.laiziNode.addChild(fanpai)
        }
    },
    hideLaizi:function()
    {
