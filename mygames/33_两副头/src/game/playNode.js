...
{   
    ...
    initCurrentRoundNode:function(currentRoundNode)
    {
        ...
        //显示明牌
        currentRoundNode.showHandCard = new cc.Node()
        currentRoundNode.addChild(currentRoundNode.showHandCard) 
        ...
        //显示分数
        currentRoundNode.special = cc.LabelTTF.create('', "Helvetica", 30)
        currentRoundNode.special.setFontFillColor( cc.color(244, 230, 159, 255) )
        currentRoundNode.addChild(currentRoundNode.special)  
        currentRoundNode.special.anchorY = 0
    ...
    {           
        ...
        currentRoundNode.special.setString('')
        currentRoundNode.showHandCard.removeAllChildren()
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        currentRoundNode.showHandCard.setPosition( cc.p(sign==-1?45:-270,0))
        currentRoundNode.showHandCard.setAnchorPoint(cc.p((1+sign)*0.5,0.5))

        currentRoundNode.notOutSpr.setPosition( cc.p(-45 * sign, 0) )
        currentRoundNode.notOutSpr.setAnchorPoint(cc.p((1 + sign)*0.5, 0.5))

        currentRoundNode.special.setPosition(cc.p(-100*sign,20))
    ...
    onCMD_StatusFree:function() 
    {
        ...
        if(!cmdBaseWorker.isChoose)
        {
            playNode.chooserule.setVisible(tableData.createrUserID == selfdwUserID )
        }
        playNode.showrule()
    },
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            if(cmdBaseWorker.cbCallRecord[i]==3)
                playNode.mingcard(i)
        }
        var bombTimes = 0
        for(var i=0;i<GAME_PLAYER;i++)
    ...
    {
        // if(cmdBaseWorker.cbCallRecord[cmdBaseWorker.wCallUser]==3)
        //     tableNode.setBankerIcon(cmdBaseWorker.wCallUser, 1)
        ...
        if(cmdBaseWorker.cbCallData!=0&&biggestScore!=3)
        {
            playNode.showbiandata(cmdBaseWorker.cbCallData)
            // playNode.showrule()
        }
            
         if(biggestScore==3)
        {
            playNode.dubao.setString('边报:独报')
            playNode.turnoverc.setVisible(false)
            for(var i=0;i<GAME_PLAYER;i++)
            {                
                var user = tableData.getUserWithChairId(i)
                var hand = user.userNodeInsetChair.currentRoundNode.showHandCard
    ...
    mingcard:function(i)
    {
        ...
        hand.removeAllChildren();
        var co=0;
        var handCardDatastemp =[]

        for(var l=0;l<cmdBaseWorker.cbHandCardCount[i];l++)
        {
            var ming =cmdBaseWorker.cbHandCardDataMing[l]
            if(0<ming&&ming<80)
                handCardDatastemp[co++]=ming
        }
        var handCardDatas=cardFactory.sortCardDatasWithScore(handCardDatastemp)
        for(var j=0;j<handCardDatas.length;j++)
            {
                ...
                card.x = (j>14?(j-15):j)*0.5*30 
                card.y = j>14?-95:-65//pos.y
                card.scale =0.35

                hand.addChild(card)
            }                  
    },
    showbiandata:function(data)
    {
       
        if(data!=0)
            playNode.turnoverc.setVisible(true)
        var card1 = playNode.turnoverc.getChildByTag(100)
        cardFactory.updateIdxOfCardSpr(card1, data)  
        cardFactory.decorateCard(card1)
    }, 
    showrule:function()
    {
        playNode.quantao.setVisible(true)
        playNode.quantao.setString(cmdBaseWorker.isQuantao?'全讨':'非全讨')
        playNode.pingju.setVisible(true)
        playNode.pingju.setString(cmdBaseWorker.isPingju?'有平局':'无平局')
    },
    ///选择一张牌当朋友
    chooseCard:function()
    {
        // showTips({str:'您的选牌不合理'}) 
        //todo不能选的牌要变暗
        var selectCardDatas = []
        var cardSprs = playNode.handCards4D[0]
        if(selectCardDatas.length==0)
        {
    ...
    {   
        ...
        {
            var user = tableData.getUserWithChairId(i)
            var special = user.userNodeInsetChair.currentRoundNode.special
                
            if(i==cmdBaseWorker.wOutCardUser)
            {   
                if(cmdBaseWorker.wSpecialScore!=0)          
                    special.setString('+'+cmdBaseWorker.wSpecialScore)
                else
                    special.setString('')
            }else
                special.setString(cmdBaseWorker.wSpecialScore==0?'':-cmdBaseWorker.wSpecialScore/3)
        }
    ...
    onCMD_OutCard:function() 
    {
        ...
        //如果有人出了朋友牌而且不是本来的庄家那么
        if(cmdBaseWorker.cbCallRecord[cmdBaseWorker.wOutCardUser]!=1&&biggestScore==1)
            tableNode.setBankerIcon(cmdBaseWorker.wOutCardUser, cmdBaseWorker.showfriend[cmdBaseWorker.wOutCardUser])
        for(var i=0;i<GAME_PLAYER;i++)
        {
            if(cmdBaseWorker.wOutCardUser==i&&cmdBaseWorker.cbCallRecord[i]==3)
                playNode.mingcard(i)
        }
        // playNode.refreshCallRecord()
        playNode.turnDataNode.setVisible(true)
    ...
    onCMD_GameEnd:function() 
    {
        ...
        playNode.rulenode.setVisible(false)
...
// ]
        ...
        if(cmdBaseWorker.cbCallData>0&&cmdBaseWorker.cbCallData<80)
        {
            playNode.showbiandata(cmdBaseWorker.cbCallData)
            playNode.showrule()
        }
        ...
        if(cmdBaseWorker.cbCallData==90)
            playNode.dubao.setString('边报:独报')
    ...
    {   
        ...
        {             
            ...
            //如果出两张牌 但出牌玩家手牌不是2不能出一张小王一张大王
            if(selectCardDatas.length==2)
            {
                if(cmdBaseWorker.cbHandCardCount[cmdBaseWorker.wCurrentUser]!=2&&selectCardDatas[0]==78&&selectCardDatas[1]==79)
                    cardsTypeArrray=1;
            }
    ...
    _outCardCall1:function(cardsType)
    {
        ...
        if(cardsType)
        {
            ...
            else
            {
                ...
                if(cardsType.typeScores>=14&&cardsType2.typeScores>=14)
                    c=1
                isAllowOut = c == 0
            }
        }
    ...
    popOutCardNode:function(isCan)
    {
        ...
        if(cmdBaseWorker.outCardType.typeLevel == 11)
    ...
    {      
        var call = function(cbParam1,data)
        {
            var CallCard = getObjWithStructName('CMD_C_Call') 
            CallCard.cbCallScore = cbParam1
            CallCard.cbCallData = data
            // CallCard.isPingju=playNode.quantao.getString()=="有平局"?true:false
            // CallCard.isQuantao=playNode.quantao.getString()=="全讨"?true:false
            // CallCard.isPingju=
            // CallCard.isQuantao=playNode.ruleButton1.isVisible()?
            ...
            if(cbParam1==1)
            {
                ...
                {
                        playNode.handCards4D[0][i].color =cc.color(255,255,255) 
                }
            }
            ...
            if(cbParam1==2)
            {
                ...
                {
                    var self = tableData.getUserWithUserId(selfdwUserID)
                    if(!cmdBaseWorker.canChooseCard(cmdBaseWorker.cbHandCardData[self.wChairID],playNode.handCards4D[0][i].cardData))
                        playNode.handCards4D[0][i].color =cc.color(100,100,100) 
                }
            }
        }
        playNode.rule1=function()
        {
            playNode.ruleButton1.isVisible()
            playNode.ruleButton2.setVisible(true)
            playNode.ruleButton1.setVisible(false)
        }
        playNode.rule2=function()
        {
            playNode.ruleButton1.setVisible(true)
            playNode.ruleButton2.setVisible(false)
        }
        playNode.rule3=function()
        {
            playNode.ruleButton4.setVisible(true)
            playNode.ruleButton3.setVisible(false)
        }
        playNode.rule4=function()
        {
            playNode.ruleButton3.setVisible(true)
            playNode.ruleButton4.setVisible(false)
        }
        playNode.wuping0=function()
        {
            playNode.wupingju0.setVisible(false)
            playNode.wupingju1.setVisible(true)
            playNode.pingju0.setVisible(false)
            playNode.pingju1.setVisible(true)
        }
        playNode.ping1=function()
        {
            playNode.wupingju1.setVisible(false)
            playNode.wupingju0.setVisible(true)
            playNode.pingju1.setVisible(false)
            playNode.pingju0.setVisible(true)
        }
        playNode.quan1=function()
        {
            playNode.quantao1.setVisible(false)
            playNode.quantao0.setVisible(true)
            playNode.feiquantao1.setVisible(false)
            playNode.feiquantao0.setVisible(true)
        }
        playNode.feiquan=function()
        {
            playNode.quantao0.setVisible(false)
            playNode.quantao1.setVisible(true)
            playNode.feiquantao0.setVisible(false)
            playNode.feiquantao1.setVisible(true)
        }
        playNode.callrule=function()
        {
            // playNode.quantao.setVisible(true)
            // playNode.quantao.setString(playNode.quantao0.isVisible()?'全讨':'非全讨')
            // playNode.quantao.getString()=="全讨"
            // playNode.pingju.setVisible(true)
            // playNode.pingju.setString(playNode.pingju0.isVisible()?'有平局':'无平局')
            // playNode.chooserule.setVisible(false)

            var Rule = getObjWithStructName('CMD_C_Rule')
            Rule.isPingju=playNode.pingju0.isVisible()?true:false
            Rule.isQuantao=playNode.quantao0.isVisible()?true:false
            socket.sendMessage(MDM_GF_GAME, SUB_C_RULE, Rule)
            playNode.hideCallNode()
            playNode.chooserule.setVisible(false)
       ...
       /////////////////////////////////
    _initChooseCall:function()
    {
        playNode.callScore1 = function()
        {             
            var selectCardDatas = []
            var cardSprs = playNode.handCards4D[0]
            var cardSprData =[]
            for(var i=0;i<cardSprs.length;i++)
            {
                cardSprData[i]=cardSprs[i].cardData
                var cardSpr = cardSprs[i]
                if(cardSpr.y != cardSpr.originY)
                    selectCardDatas[selectCardDatas.length] = cardSpr.cardData
            }
            ...
            if(selectCardDatas.length==1&&cmdBaseWorker.canChooseCard(cardSprData,selectCardDatas[0]))
                hasInTipsArray =true
            ...
            {
                var CallCard = getObjWithStructName('CMD_C_Call') 
                CallCard.cbCallScore = 1
                CallCard.cbCallData = selectCardDatas[0]
                CallCard.isPingju=playNode.ruleButton1.isVisible()?true:false
                CallCard.isQuantao=playNode.ruleButton3.isVisible()?true:false
                socket.sendMessage(MDM_GF_GAME, SUB_C_CALL, CallCard)
                playNode.hideCallNode()
                playNode.rulenode.setVisible(false)
    ...
    popCallNode:function(isCan, cbCallScore) // 0 当前0分
    {
        ...
        if(cbCallScore==0)
        {
            // playNode.rulenode.setVisible(true)

            playNode.callButton1.setVisible(false)
            playNode.callButton0.setVisible(false)
            playNode.callButton2.setVisible(true)
            playNode.callButton3.setVisible(true)
            playNode.callButton2.setEnabled(true)
            playNode.callButton3.setEnabled(true)
        }
        else if(cbCallScore==2)
        {
            playNode.callButton1.setVisible(true)
            playNode.callButton0.setVisible(false)
            playNode.callButton1.setEnabled(true)
            playNode.callButton2.setVisible(false)
            playNode.callButton3.setVisible(false)
        }
        //当第一个玩家选择后其他玩家会出现独报和不报按钮
        else if(cbCallScore!=0)
        {
            playNode.callButton0.setVisible(true)
            playNode.callButton0.setEnabled(true)
            playNode.callButton1.setVisible(false)
            playNode.callButton2.setVisible(false)
            playNode.callButton3.setEnabled(true)
            playNode.callButton3.setVisible(true)
    ...
    refreshCallRecord:function()
    {
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            var user = tableData.getUserWithChairId(i)
            var tipsSpr = user.userNodeInsetChair.currentRoundNode.tipsSpr
            // 'callSpr_'+cbCallScore+'.png'    
            tipsSpr.setSpriteFrame('callSpr_'+(cmdBaseWorker.cbCallRecord[i]==1?2:cmdBaseWorker.cbCallRecord[i])+'.png') 
        }
    },
    ...
    initTurnoverbian:function()
    {
        var node = playNode['turnoverc']
        var card = cardFactory.getOne(0, 1, 0)
        card.scaleX = node.width/card.width
        card.scaleY = node.height/card.height
        card.x = node.width/2
    ...
    resetPlayNode:function()
    {
        ...
        playNode.dubao.setString('边报:')
        playNode.turnoverc.setVisible(false)
        playNode.difenTTF.setString( 1 )
        cmdBaseWorker.cbCallData=null
        // playNode.quantao.setVisible(false)
        // playNode.pingju.setVisible(false)
