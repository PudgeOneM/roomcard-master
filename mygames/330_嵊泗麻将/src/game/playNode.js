...
{   
    ...
    onReStart:function()
    {
        ...
        playNode.iscontrol_1 = []
        playNode.iscontrol_2 = []
       // playNode.controlusr = null
      //  playNode.outCard = null
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        playNode.isPlaying = true
        //playNode.controlusr = cmdBaseWorker.wCurrentUser
        tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
        playNode.scoreTTF.setString(cmdBaseWorker.cbLeftCardCount)
        playNode.showLaizi()
        playNode.ischi = false 
        if(cmdBaseWorker.ischi[cmdBaseWorker.wCurrentUser] && cmdBaseWorker.wCurrentUser != INVALID_CHAIR)
        {
           playNode.iscontrol_1[cmdBaseWorker.wCurrentUser] =  cmdBaseWorker.notallowout_1
           if(cmdBaseWorker.notallowout_2 != INVALID_BYTE)
           {
                playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] =  cmdBaseWorker.notallowout_2
                playNode.ischi = true
           }
           else
                playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] =  null
        }
        // else
        // {
        //     for(var i=0;i<4;i++)
        //     {
        //         playNode.iscontrol_1[i] = null
        //         playNode.iscontrol_2[i] = null
        //     }
        // }
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            user.userNodeInsetChair.currentRoundNode.upTTF.setVisible(false)
        }
        ...
        {    
            var user = tableData.getUserWithChairId(cmdBaseWorker.wCurrentUser).dwUserID
            var selfChairId = tableData.getUserWithUserId(user).wChairID
            var direction = tableData.getShowChairIdWithServerChairId(selfChairId)
            if(playNode.handMajiangs4D[direction])
            {
            var mj = playNode.handMajiangs4D[direction]
            for(var i=0;i<mj[0].length;i++)
            {
                    if(mj[0][i].cardData == playNode.iscontrol_1[cmdBaseWorker.wCurrentUser])
                    {
                        var majiang = mj[0][i]
                        majiang.color = cc.color(122,122,122)

                    }
                    else if(mj[0][i].cardData == playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] )
                    {
                        var majiang = mj[0][i]
                        majiang.color = cc.color(122,122,122)
                    }
                }
                var mj0 =  playNode.handMajiangs4D[direction][1]
                if(mj0)
                {
                    if(mj0.cardData == playNode.iscontrol_1[cmdBaseWorker.wCurrentUser])
                    {
                        var majiang = mj0
                        majiang.color = cc.color(122,122,122)
                    }
                    else if(playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] == mj0.cardData)
                    {
                        var majiang = mj0
                        majiang.color = cc.color(122,122,122)
                    }
    ...
    onCMD_GameStart:function() 
    {
        ...
        scores[scores.length] = magicCardData[0]
        majiangFactory.initCardData2ScoreMap( idxs, scores )
       // playNode.controlusr = cmdBaseWorker.wBankerUser
        var self = tableData.getUserWithUserId(selfdwUserID)
        for(var i=0;i<GAME_PLAYER;i++)
        {
            var user = tableData.getUserWithTableIdAndChairId(self.wTableID, i)
        ...
        {
            tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)
            var siceNum1 = cmdBaseWorker.touzi_1//getRandNum(Math.max(cmdBaseWorker.cbSiceCount-6, 1), Math.min(cmdBaseWorker.cbSiceCount-1, 6))
            var siceNum2 = cmdBaseWorker.touzi_2//cmdBaseWorker.cbSiceCount - siceNum1
    ...
    onCMD_OutCard:function() 
    {
        ...
        var user = tableData.getUserWithChairId(cmdBaseWorker.wOutCardUser).dwUserID
        var selfChairId = tableData.getUserWithUserId(user).wChairID
        var direction = tableData.getShowChairIdWithServerChairId(selfChairId)
        var mj = playNode.handMajiangs4D[direction]
        for(var i=0;i<mj[0].length;i++)
    ...
    onCMD_SendCard:function() 
    {
        ...
        playNode.iscontrol_1[cmdBaseWorker.wTakeCardUser] = null
        playNode.iscontrol_2[cmdBaseWorker.wTakeCardUser] = null
     ...
     //   playNode.controlusr = cmdBaseWorker.wCurrentUser
        ...
        if(sortedActions.length>0)
         ...
         if(cmdBaseWorker.isbahua && isSelf)
       {
                playNode.istianhubahua = true
                var bahua = getObjWithStructName('CMD_C_BaHua')
                bahua.user = cmdBaseWorker.wTakeCardUser
                socket.sendMessage(MDM_GF_GAME,SUB_C_BAHUA,bahua)
       }
       ////////////////////
    },
    onCMD_OperateResult:function() 
    {
       ...
       // playNode.controlusr = cmdBaseWorker.wCurrentUser
        playNode.iscontrol_1[cmdBaseWorker.wCurrentUser] = null
        playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] = null
        playNode.timer.switchTimer(cmdBaseWorker.wCurrentUser==INVALID_WORD?[]:[tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wCurrentUser)])
        var tmp = false//用来判断有没有吃
        if(cmdBaseWorker.cbOperateCode == WIK_RIGHT)
        {
            tmp = true
            playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] = cmdBaseWorker.cbProvideCardData - 3
            playNode.ischi = true //用来判断是不是中吃，中吃只有吃的哪张不能打，不是中吃为true
        }
        else if(cmdBaseWorker.cbOperateCode == WIK_CENTER)
        {
            tmp = true
            playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] = null
            playNode.ischi = false
        }
        else if(cmdBaseWorker.cbOperateCode == WIK_LEFT)
        {
            tmp = true
            playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] = cmdBaseWorker.cbProvideCardData + 3
            playNode.ischi = true
        ...
        if(tmp)
            playNode.iscontrol_1[cmdBaseWorker.wCurrentUser] = cmdBaseWorker.cbProvideCardData 
        ...
        ///吃动作后给不能打出的牌颜色加深，方便玩家一眼就认出这张牌不能打///////////////
        if(tmp)
        {
            var user = tableData.getUserWithChairId(cmdBaseWorker.wOperateUser).dwUserID
            var selfChairId = tableData.getUserWithUserId(user).wChairID
            var direction = tableData.getShowChairIdWithServerChairId(selfChairId)
            var mj = playNode.handMajiangs4D[direction]
            for(var i=0;i<mj[0].length;i++)
        ...
        //end///////////////////////////
        //判断碰吃杠后是否还有动作，有的话执行动作，没有的话将吃动作后不能打出的牌上传服务器记录，以便断线重连使用
        var sortedActions = majiangLogic.getSortedActionsWithMask(cmdBaseWorker.cbActionMask)
        if(sortedActions.length>0)
        {
            if(cmdBaseWorker.cbActionMask == WIK_CHI_HU && cmdBaseWorker.cbOperateCode == WIK_GANG)
            {
        ...
        {
            var record = getObjWithStructName('CMD_C_AllowOut') 
            if(tmp && playNode.ischi)
            {
                record.notallowout_1 = playNode.iscontrol_1[cmdBaseWorker.wCurrentUser]
                record.notallowout_2 = playNode.iscontrol_2[cmdBaseWorker.wCurrentUser]
            }
            else if(tmp && !playNode.ischi)
            {
                record.notallowout_1 = playNode.iscontrol_1[cmdBaseWorker.wCurrentUser]
                record.notallowout_2 = INVALID_BYTE
            }
            if(tmp)
            {
                record.ischi = true
                record.usr = cmdBaseWorker.wCurrentUser
                socket.sendMessage(MDM_GF_GAME, SUB_C_ALLOWOUT, record)
            }
        }
        ////////////////END////////////////////////////////////////////////////
    },
    ...
    onCMD_GameEnd:function() 
    {
        ...
        playNode.hideActionBtns()
        playNode.istianhubahua = cmdBaseWorker.isbahua
        cocos.setInterval(playNode.updateOnFree, 1000, playNode.node)
        ...
        //alert(cmdBaseWorker.cbProvideCardData)
        ...
        function onPlayAnimationOfGameEnd()
        {
            var a = cc.sequence( 
                ...
                {   
                    ...
                    playNode.popGameEnd2(continueCall, szNickName_gameEnd) 
                }) 
            )           
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            var a = true
            var b = false
            if(playNode.iscontrol_1[cmdBaseWorker.wCurrentUser] == majiang.cardData)
                a = false
            if(playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] == majiang.cardData && playNode.iscontrol_2[cmdBaseWorker.wCurrentUser]!= INVALID_BYTE)
                a = false
            console.log("666666",playNode.iscontrol_1[cmdBaseWorker.wCurrentUser])
            if(cmdBaseWorker.handcount!= null)
            {
                console.log(cmdBaseWorker.handcount[cmdBaseWorker.wCurrentUser],playNode.ischi)
                console.log(a,b)
                if(cmdBaseWorker.handcount[cmdBaseWorker.wCurrentUser] == 2 )
                {
                    b = true   
                    if(cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][1]!= cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][0])
                    {
                        if(cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][0] == playNode.iscontrol_1[cmdBaseWorker.wCurrentUser] && playNode.iscontrol_1[cmdBaseWorker.wCurrentUser] == majiang.cardData && cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][1] != playNode.iscontrol_2[cmdBaseWorker.wCurrentUser])   
                        {
                            b = false
                        }
                        else if(cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][1] == playNode.iscontrol_1[cmdBaseWorker.wCurrentUser] && playNode.iscontrol_1[cmdBaseWorker.wCurrentUser] == majiang.cardData && cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][0] != playNode.iscontrol_2[cmdBaseWorker.wCurrentUser])
                            b= false
                        else if(cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][1] == playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] && playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] == majiang.cardData&& cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][0] != playNode.iscontrol_1[cmdBaseWorker.wCurrentUser])
                            b= false
                        else if(cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][0] == playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] && playNode.iscontrol_2[cmdBaseWorker.wCurrentUser] == majiang.cardData&& cmdBaseWorker.handcarddate[cmdBaseWorker.wCurrentUser][1] != playNode.iscontrol_1[cmdBaseWorker.wCurrentUser])
                            b= false
                    }
                }
            }
            if(isAllowOut && (a || b))
            {
                console.log(cmdBaseWorker.wCurrentUser,majiang.cardData)
        ...
        onTouchBegan: function (touch, event) {
            ...
            onTouchEnded: function (touch, event) {
                ...
                if(currentMajiang)
                {
                    ...
                    if((isTouchFromPop && touchedMjNum==1) || ismoved)
                    {
    ...
    popGameEnd2:function(continueCall, szNickName_gameEnd)
    {
        ...
        for(var i=0;i<GAME_PLAYER;i++)
        {
            ...
            if((cmdBaseWorker.endType==2||cmdBaseWorker.endType==3) && cmdBaseWorker.dwChiHuKind[i] == WIK_CHI_HU)
                cbWinFlag[i] = 7
        ...
        if(cmdBaseWorker.endType != 1 || cmdBaseWorker.endType != 0)
        {
            ...
            {
                var chrStr = "花型："
                if(cmdBaseWorker.havehua[i][0] == true)
                {
                    control['huaTTF'+i].setVisible(true)
                    chrStr += '春'+' '
                }
                if(cmdBaseWorker.havehua[i][1] == true)
                {
                    control['huaTTF'+i].setVisible(true)
                    chrStr += '夏'+' '
                }
                if(cmdBaseWorker.havehua[i][2] == true)
                {
                    control['huaTTF'+i].setVisible(true)
                    chrStr += '秋'+' '
                }
                if(cmdBaseWorker.havehua[i][3] == true)
                {
                    control['huaTTF'+i].setVisible(true)
                    chrStr += '冬'+' '
                }
                if(cmdBaseWorker.havehua[i][4] == true)
                {
                    control['huaTTF'+i].setVisible(true)
                    chrStr += '梅'+' '
                }
                if(cmdBaseWorker.havehua[i][5] == true)
                {
                    control['huaTTF'+i].setVisible(true)
                    chrStr += '兰'+' '
                }
                if(cmdBaseWorker.havehua[i][7] == true)
                {
                    control['huaTTF'+i].setVisible(true)
                    chrStr += '竹'+' '
                }
                if(cmdBaseWorker.havehua[i][6] == true)
                {
                    control['huaTTF'+i].setVisible(true)
                    chrStr += '菊'+' '
                }
                control['huaTTF'+i].setString(chrStr)
            }
        }
        ////////////END//////////////////////////////////////
        // control.resultTTF.setString( args.msg )
        if(cmdBaseWorker.endType == 1 || cmdBaseWorker.endType == 0)
        {
            if(cmdBaseWorker.endType == 1)
                ...
                control.endType.setVisible(false)
            for(var i =0;i<4;i++)
                control['resultTTF'+i].setVisible(false)
        }
        else
        {
            ...
            for(var j = 0;j<GAME_PLAYER;j++)
            {
                ...
                {
                    var chr_type = window[ map_mask2Name[i][0] ] 
                    if (cmdBaseWorker.dwChiHuRight[j] & chr_type)
                    {
                        chrStr += map_mask2Name[i][1] + ' ';
                    }
                }
                if(cmdBaseWorker.dwChiHuKind[j] != WIK_CHI_HU)
                    control['resultTTF'+j].setVisible(false)
                else
                    control['resultTTF'+j].setVisible(true)
                control['resultTTF'+j].setString(chrStr)
        ...
        for(var chairid=0;chairid<GAME_PLAYER;chairid++)
        {
            ...
            for(var i = 0;i<MAX_WEAVE;i++)
            {
                for(var j = 0;j<4;j++)
                {
                    ...
                    if (cardIdx>0) 
                    {
                        ...
                        {
                            cardIndex -= 2
                            aCard.x = (150+( cardIndex+1 )*(aCard.getContentSize().width*aCard.getScale() - 2)) - numbercon, aCard.getContentSize().height /2 *aCard.getScale() 
            ...
            for(var i = 0;i<MAX_COUNT;i++)
            {
                ...
                if (cardIdx>0 ) 
                {
                    ...
                    if (cardIdx == cmdBaseWorker.cbProvideCardData && cmdBaseWorker.cbHandCardCount[chairid] + cmdBaseWorker.cbWeaveCount[chairid]*3 == MAX_COUNT) 
                    {
                        cardIndex--
    ...
    playAnimationOfGameEnd:function(call)
    {
        ...
        {
            var spr = actionFactory.getSprWithAnimate('lj', true, 0.15)
        ...
        {
            if(playNode.istianhubahua)//天胡八花由于数据传输太快导致骰子没结束就结算，所以刚给判断，然后延迟结算界面出来
            {
                var seq = cc.sequence(cc.delayTime(3),cc.callFunc(function(){
                    playNode.playAnimationWithDirection('zim', tableData.getShowChairIdWithServerChairId(winner))
                    playNode.playGenderEffect('zimo0', tableData.getUserWithChairId(winner).cbGender)
                }),cc.delayTime(0.5),cc.callFunc(function(){
                    call?call():' '
                }))
                playNode.node.runAction(seq)
            }
            else
            {
                playNode.playAnimationWithDirection('zim', tableData.getShowChairIdWithServerChairId(winner))
        ...
        else if(cmdBaseWorker.endType == 3)
        {
            ...
            playNode.node.runAction(a)//显示结算界面
        }
    },
    /////other ui end////////
    ...
    playAnimationWithDirection:function(name, direction, call,isOpenEffect)
    {
        // if(!isOpenEffect)
        // {
        //     call?call():''
    ...
    {   
        ...
        function playDice(direction)
        {
            ...
            if(direction == 3)
                playNode.playDiceOneDirection(call, cmdBaseWorker.touzi_1, cmdBaseWorker.touzi_2, direction)
            else
                playNode.playDiceOneDirection(function()
                    {
                        playDice(direction+1)
                    }, cmdBaseWorker.touzi_1, cmdBaseWorker.touzi_2, direction)
        }
