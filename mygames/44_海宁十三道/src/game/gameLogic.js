...
gameLogic.getScore = function(type, sortedCardDatas, param1, param2)
{
    ...
    if(type==level_shunzi)//海宁十三道A2345是最小的顺子
    {
        if(cardLogic.getNum(sortWithScore[0]) == 1 && cardLogic.getNum(sortWithScore[1])==5)
            score = score_1*10 + cardLogic.getColor(sortWithScore[1])+score_2
        else
            score = score_0*10 + cardLogic.getColor(sortWithScore[0])+score_1
    }
    ...
    if(type==level_tonghuashun)//海宁十三道A2345是最小的顺子
    {      
        if(cardLogic.getNum(sortWithScore[0]) == 1 && cardLogic.getNum(sortWithScore[1])==5)
            score = score_1*10 + cardLogic.getColor(sortWithScore[1])+score_2
        else
