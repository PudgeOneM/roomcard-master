...
console.log(888888, isMinGang, clone(cmdBaseWorker.wProvideUser) )
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        // }
        if(cmdBaseWorker.dxgenpaiCount>0){
            tableData.getUserWithChairId(cmdBaseWorker.wBankerUser).userNodeInsetChair.currentRoundNode.upTTF.setString( '跟牌:'+cmdBaseWorker.dxgenpaiCount+'次')
        }
        
        if(cmdBaseWorker.wOutCardUser!=INVALID_WORD)
        {
            var d = tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wOutCardUser)
            console.log(333333,cmdBaseWorker.cbOutCardData)
    ...
    onCMD_GameStart:function() 
    {
        ...
        //     //user.userNodeInsetChair.currentRoundNode.upTTF.setString( '下跑:'+cmdBaseWorker.cbCallRecord[i][0]+' 加顶:' + cmdBaseWorker.cbCallRecord[i][1] )
        // }
        // tableData.getUserWithChairId(cmdBaseWorker.wBankerUser).userNodeInsetChair.currentRoundNode.upTTF.setString( '跟牌:0次')
        ...
        console.log("庄家位置",bankerShowChairid);
        function gameStart()
        {
            ...
            playNode.showLaizi()
            //bankerPlayDiceFindMagic()抓完牌后确定财神
        }

        function bankerPlayDice()//庄家掷筛子，确定从哪桌开始抓牌
        {
            playNode.playDiceOneDirection(takePlayDice, cmdBaseWorker.cbSiceCount[0], 
                cmdBaseWorker.cbSiceCount[1], bankerShowChairid)
        }

        /*function bankerPlayDiceFindMagic()//庄家掷筛子,确定财神
        {
            playNode.playDiceOneDirection(playNode.showLaizi, cmdBaseWorker.cbSiceCountMG[0], 
                cmdBaseWorker.cbSiceCountMG[1], bankerShowChairid)
            console.log(cmdBaseWorker.cbSiceCountMG[0],cmdBaseWorker.cbSiceCountMG[1])
        }*/

        function takePlayDice()//闲家掷筛子后，调用gameStart函数
        {
            tableNode.setBankerIcon(cmdBaseWorker.wBankerUser, true)//显示庄家的标签
            //playNode.playDiceOneDirection(gameStart, cmdBaseWorker.cbSiceCount[2], cmdBaseWorker.cbSiceCount[3], takerShowChairid)//闲家掷筛子
            playNode.playDiceOneDirection(gameStart, cmdBaseWorker.cbSiceCount[2], cmdBaseWorker.cbSiceCount[3], bankerShowChairid)
        }

        if(cmdBaseWorker.bIsRandBanker)//判断是否已经选好庄家
    ...
    onCMD_OutCard:function() 
    {
        ...
        if(cmdBaseWorker.cdwgenpaiCount>0){
           tableData.getUserWithChairId(cmdBaseWorker.wBankerUser).userNodeInsetChair.currentRoundNode.upTTF.setString( '跟牌:'+cmdBaseWorker.cdwgenpaiCount+'次') 
        }
        

        var majiangs = playNode.handMajiangs4D[outDir]
        if(outUser.dwUserID==selfdwUserID)//自己方位
        {
    ...
    {
        //console.log("之前摸的牌",cmdBaseWorker.rightCardDate)
        if(cmdBaseWorker.cbOutCardCount != 0)
            managerAudio.playEffect('gameRes/sound/sendcard.mp3')//发牌音乐
        //console.log("抓牌玩家",cmdBaseWorker.wTakeCardUser,"自己的ID",selfdwUserID)
        var isSelf = tableData.getUserWithChairId(cmdBaseWorker.wTakeCardUser).dwUserID == selfdwUserID

        playNode.scoreTTF.setString(cmdBaseWorker.cbLeftCardCount)
        playNode.timer.switchTimer(cmdBaseWorker.wCurrentUser==INVALID_WORD?[]:[tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wCurrentUser)])

        var takeDir =  tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wTakeCardUser)//抓牌玩家位子
        ///////
        //console.log('财神',cmdBaseWorker.cbMagicCardData[0])
        
        for(var i=0;i<cmdBaseWorker.cbSendCardCount;i++)
        {   
            if(i==1){
                console.log("财神的位置",cmdBaseWorker.cbSiceCount[2], cmdBaseWorker.cbSiceCount[3],"tt的值",cmdBaseWorker.cbGangmagic)
                var item3 = cmdBaseWorker.sendCardArray[i]
                var outUser = tableData.getUserWithChairId(cmdBaseWorker.wTakeCardUser)
                var outDir = tableData.getShowChairIdWithServerChairId(cmdBaseWorker.wTakeCardUser)
                
                if(cmdBaseWorker.cbSiceCount[2]+cmdBaseWorker.cbSiceCount[3]==cmdBaseWorker.cbGangmagic){

                    console.log("扔的牌(财神)",item3.cbCardData,"不能扔")
                }else{
                      
                    //牌堆
                    var heapMajiangsDir3 = tableData.getShowChairIdWithServerChairId(item3.wHeapDir)  
                    majiangFactory.deleteHeapMajiangs(playNode.heapMajiangs4D[heapMajiangsDir3], [item3.wHeapPos])
                    //手牌
                    if(item3.cbCardData == SEND_DISCARD_CARD_DATA)//丢弃牌
                        continue
                    //var cardData2 = isSelf?item3.cbCardData:0
                    //playNode.setCurrentDiscardMj(outCardData, outDir)
                    majiangFactory.addDiscardMajiangs(playNode.discardMajiangs4D[outDir], outDir,item3.cbCardData, outUser.userNodeInsetChair.currentRoundNode.discardMajiangsNode)//丢到桌面上
                    //console.log("掷的筛子数",cmdBaseWorker.cbSiceCountMG[0],cmdBaseWorker.cbSiceCountMG[1],"扔的牌(上面的)",item3.cbCardData,"财神：",cmdBaseWorker.cbMagicCardData[0])
                }
            }else{//下面这张
                ...
                //if(cmdBaseWorker.cbSendCardCount==2){
                    //item.cbCardData = cmdBaseWorker.cbSendCardCount
                    
                //}

                var cardData = isSelf?item.cbCardData:0//自己可见，其他位置不可见
                ...
                //console.log("摸的牌",cardData)
                //cmdBaseWorker.rightCardDate = cmdBaseWorker.cbProvideCardData//记录上次摸得牌
    ...
    onCMD_OperateResult:function() 
    {
        ...
        //console.log("执行动作玩家：",operateUser.wChairID);
        //
        //杠的时候掷筛子
        if(cmdBaseWorker.cbOperateCode == WIK_GANG){
            playNode.playDiceOneDirection(function(){return true}, cmdBaseWorker.cbSiceCountMG[0], cmdBaseWorker.cbSiceCountMG[1], operateUser.wChairID)//杠后要掷筛子抓牌
            //console.log("杠,","执行动作玩家：",operateUser.wChairID,cmdBaseWorker.cbSiceCountMG[0],cmdBaseWorker.cbSiceCountMG[1])
        }
    ...
    onCMD_GameEnd:function() 
    {
        ...
        playNode.playAnimationOfGameEnd(onPlayAnimationOfGameEnd)//结束动画
        function onPlayAnimationOfGameEnd()
        {
            //结束动画包括重新设置时间、隐藏当前麻将、在各个头像上显示得分或扣分
    ...
    _getHandMajiangsGroupNode:function()
    {
        ...
        var touchEndCall = function(direction, majiang)
        {
            ...
            //轮到自己的时候才能出牌
            //console.log(cmdBaseWorker.wCurrentUser,tableData.getUserWithUserId(selfdwUserID).wChairID)
            if(isAllowOut)
            {
    ...
    //     control.gendTitle.setSpriteFrame('gendTitle'+cmdBaseWorker.endType + '.png')//结束方式
    ...
    //             control['winflag'+wChairID].setSpriteFrame('gendFlag0.png') //强退
    //         else if(wChairID==cmdBaseWorker.wWinner&&cmdBaseWorker.endType!=1){
    //             control['winflag'+wChairID].setSpriteFrame('gendFlag1.png') //流局的时候不显示胡字
    //             console.log("胡")
    //         }
    //         //else if(wChairID==cmdBaseWorker.wProvideUser)
    //             //control['winflag'+wChairID].setSpriteFrame('gendFlag2.png') //不能点炮
    //         else
        ...
        popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var showChairid=0;showChairid<4;showChairid++)
        {
            ...
            for (var i = 0; i < map_mask2Name.length; i++) 
            {
                ...
                if(i == map_mask2Name.length-1&&wChairID == cmdBaseWorker.wBankerUser&&cmdBaseWorker.cdwgenpaiCount>0)
                {
                    chrStr += '跟牌:'+cmdBaseWorker.cdwgenpaiCount+'次'+ ' ' //+ ' ' + '连庄';  
                }
                if(i == map_mask2Name.length-1&&wChairID == cmdBaseWorker.wBankerUser&&cmdBaseWorker.cbPrebank){
                    chrStr +='老庄';  
                }
            }
            resultTTF.setString(chrStr)
