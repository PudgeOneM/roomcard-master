...
var handMahJong = {
	...
	setHuaCardData:function(cbHuaData, cbBuHuaData)
	{
		...
		if (handMahJong.control[0].visible)
		{
			var cbCardDataNew = [];
			for (var i = 1; i < handMahJong.cbCardCount; i++) {
				cbCardDataNew[i-1] = handMahJong.cbCardData[i]; 
			};
			mainScene.sortHandCard(cbCardDataNew);
			for (var i = 1; i < handMahJong.cbCardCount; i++) {
				handMahJong.cbCardData[i] = cbCardDataNew[i-1]; 
			};
		}
		else
		{
...
var buHua = 
{
	...
	{
		var cbFeng = (GAME_PLAYER+wChaird-mainScene._wBankerUser)%GAME_PLAYER;
		var nZhengHua = 0;
		var nYeHua = 0;
		for (var i = 0; i < MAX_HUA_CARD; i++)
		{
