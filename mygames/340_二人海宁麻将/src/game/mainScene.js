...
var mainScene = {
  ...
  inittableCCB:function()
  {
      ...
      tableNode.callBackQF = function()
      {
          tableNode.gameSetNode.visible = false;
          var psResult = getObjWithStructName('CMD_C_FANBEI_RESUTL');
          psResult.bFanBei = false;
          socket.sendMessage(MDM_GF_GAME, SUB_C_FANBEI_RESULT, psResult); 
      }
      tableNode.callBackSJF = function()
      {
          tableNode.gameSetNode.visible = false;
          var psResult = getObjWithStructName('CMD_C_FANBEI_RESUTL');
          psResult.bFanBei = true;
          socket.sendMessage(MDM_GF_GAME, SUB_C_FANBEI_RESULT, psResult); 
      }
  ...
  onEventScenePlay:function(data)
  {
      ...
      //设置
      mainScene.onSubGameSetResult(data.bFanBei);
  },
  //游戏开始
	onSubGameStart:function(data)
	{
      ...
      mainScene._wMeChaird       = gameLogic.getMeChaird();
  		...
  		mainScene._cbLeftCardCount = 109;
      mainScene._cbCardData      = [];
      if(mainScene._wMeChaird != INVALID_CHAIR)
      {
  ...
  onSubOutCard:function(data)
  {
      ...
      if (data.wOutCardUser == data.wPlayerPC && data.cbOutCardData == 0x37)
      {
          if (outUser.cbGender)
  ...
  onSubGameSetResult:function(bResult)
  {
      if (bResult)
      {
          tableNode.fanbeiText.setString("随机翻");
      }
      else
      {
          tableNode.fanbeiText.setString("全翻");
      }
  ...
  onListerMenuShare:function()
  {
      ...
      //   desc: tableNode.fanbeiText.getString(),
