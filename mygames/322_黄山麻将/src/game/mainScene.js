...
var mainScene = {
  ...
  inittableCCB:function()
  {
      ...
      tableNode.callBackGSL = function()
      {
          tableNode.gameSetNode.visible = false;
          var psResult = getObjWithStructName('CMD_C_GAMESET_RESUTL');
          psResult.bGameSet = true;
          socket.sendMessage(MDM_GF_GAME, SUB_C_GAMESET_RESULT, psResult); 
      }
      tableNode.callBackGSR = function()
      {
          tableNode.gameSetNode.visible = false;
          var psResult = getObjWithStructName('CMD_C_GAMESET_RESUTL');
          psResult.bGameSet = false;
          socket.sendMessage(MDM_GF_GAME, SUB_C_GAMESET_RESULT, psResult); 
      }
  ...
  testCode:function()
  {
      ...
      for (var i = 0; i < GAME_PLAYER; i++) {
          mainScene._szNickName[i] = '黄山麻将'+i;
         // for (var j = 0; j < 17; j++) {
  ...
  onEventScenePlay:function(data)
  {
      ...
      //设置
      mainScene.onSubGameSetResult(data.bGameSet);
  },
  //游戏开始
	onSubGameStart:function(data)
	{
      ...
      mainScene._wMeChaird       = gameLogic.getMeChaird();
  		...
  		mainScene._cbLeftCardCount = 62;
      //mainScene._cbLeftCardCount = mainScene._cbLeftCardCount-(data.cbSice1+data.cbSice2+2)*2;
      mainScene._cbCardData      = [];
      mainScene._cbCardData      = data.cbCardData;
  ...
  onSubOutCard:function(data)
  {
      ...
      var cbOutIndex = gameLogic.switchToCardIndex(mainScene._cbOutCardData);
      if (cbOutIndex == mainScene._cbMagicIndex) 
      {
          mainScene._cbPiaoFeiCount[mainScene._wOutCardUser]++;
          if (mainScene._cbPiaoFeiCount[mainScene._wOutCardUser] > 1)
          {
              if (outUser.cbGender)
                managerAudio.playEffect('gameRes/sound/man/zaipiao.mp3');
              else
                managerAudio.playEffect('gameRes/sound/woman/zaipiao.mp3');
          }
          else
          {
              if (outUser.cbGender)
                managerAudio.playEffect('gameRes/sound/man/piao.mp3');
              else
                managerAudio.playEffect('gameRes/sound/woman/piao.mp3');
          }
      }
      else
      {
          if (outUser.cbGender)
            managerAudio.playEffect('gameRes/sound/man/'+mainScene._cbOutCardData+'.mp3');
          else
  ...
  onsubGameEnd:function(data)
  {
      ...
      {
        if (data.dwChiHuRight & chr_type[i])
        {
          if (user.cbGender)
            managerAudio.playEffect('gameRes/sound/man/'+sz_type[i]+'.mp3');
          else
            managerAudio.playEffect('gameRes/sound/man/'+sz_type[i]+'.mp3');
        }
      }
  ...
  onSubGameGM:function(data)
  {
      ...
      mainScene.sortHandCard(data.cbCardData);
  ...
  //游戏设置
  onSubGameSetResult:function(bResult)
  {
      if (bResult)
      {
          tableNode.gameSetText.setString("8分封顶");
      }
      else
      {
          tableNode.gameSetText.setString("无上限");
      }
  ...
  onListerMenuShare:function()
  {
      ...
      //   desc: tableNode.gameSetText.getString(),
