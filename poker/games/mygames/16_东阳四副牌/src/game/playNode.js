...
{   
    ...
    {   
        ...
        cardFactory.handCountOneRowMin = Math.ceil(cardFactory.handCountOneRowMax*0.4) 
        cardFactory.handGroupNodeHeight = 115/640*node.height//200
        cardFactory.outCountOneRow_upDown = Math.ceil(cardFactory.handCountOneRowMax/3) 
        cardFactory.outCountOneRow_rightLeft = Math.ceil(cardFactory.handCountOneRowMax/6) 
        cardFactory.intervalXRightAndDownOut = 25
        cardFactory.selectCardOffsetY = 12
        cardFactory.handIntervalYScale = 0.45
        cardFactory.outIntervalXScale = 0.32
        cardFactory.outIntervalYScale = 0.45

        cardFactory.handGroupNodeWidth = playNode.node.width - 5
    ...
    {   
        setTimeout(function()
        {
            if(cardSpr.where == 0)
            {
                var cardData = cardSpr.cardData 
                var handCardDatas = cmdBaseWorker.cbHandCardData[tableData.getUserWithUserId(selfdwUserID).wChairID]
               
                if(cardSpr.idxInHandCards==0 || 
                    cardLogic.getNum(cardData)!=cardLogic.getNum(handCardDatas[cardSpr.idxInHandCards-1]) )
                {
                    var count = 0
                    for(var i=0;i<handCardDatas.length;i++)
                    {
                        if(cardLogic.getNum(handCardDatas[i]) == cardLogic.getNum(cardData))
                            count++
                    }
                    var countText = new ccui.TextAtlas()
                    countText.setProperty(count, resp.nums1, 21, 30, "0")
                    countText.x = cardSpr.width * 0.12
                    countText.y = cardSpr.height* 0.1
                    cardSpr.addChild( countText  )
                }
            }
        })    
    },
    adaptUi:function()
    {
        ...
        //play
        //playNode.turnDataNode.y = -1000
        playNode.turnDataNode.y = cardFactory.handGroupNodeHeight + cardFactory.selectCardOffsetY + 20
        playNode.uiOperate.y = cardFactory.handGroupNodeHeight + cardFactory.selectCardOffsetY + 35

        //topui
        topUINode.voiceNode.y = cardFactory.handGroupNodeHeight + cardFactory.selectCardOffsetY + 50
        topUINode.faceNode.y = topUINode.voiceNode.y + 95
    ...
    updateCurrentRoundNode:function(currentRoundNode, userId)
    {
        ...
        currentRoundNode.upTTF.setPositionY(81)
    ...
    onCMD_StatusPlay:function() 
    {
        ...
        for(var wChairID=0;wChairID<GAME_PLAYER;wChairID++)
        {
            ...
            if(outCards.length>=4 && outCards[0].cardData<78)
               playNode.decorateOutCardsNode({id:4, length:outCards.length}, playNode.outCards4D, direction, outCardsNode)
    ...
    onCMD_OutCard:function() 
    {
        ...
        playNode.refreshWinOrder(cmdBaseWorker.wWinOrder)
        playNode.getOffLineUserOutCardDatas(cmdBaseWorker.wCurrentUser,cmdBaseWorker.wOutCardUser)
    },
    ...
    onCMD_OffLineOutCard:function()
    {
        //出牌时进入托管状态
        //出牌前进入托管状态
        var isUserOffLine = cmdBaseWorker.cbGameState[cmdBaseWorker.wCurrentUser] == US_OFFLINE_TRUSTEE
        var isOffLineOut = cmdBaseWorker.wOutCardUser == INVALID_WORD || cmdBaseWorker.wCurrentUser == cmdBaseWorker.wOffLineUser
        if(isUserOffLine)
        {
            var offLineAction = cc.sequence( 
               // cc.delayTime(5), //出牌等待时间
                cc.callFunc(function()
                {   
                    if(!isOffLineOut)
                        return
                    ...
                    if(cmdBaseWorker.wReplaceUser == self.wChairID )
                    {
                        console.log("出牌阶段掉线")
                        playNode.getOffLineUserOutCardDatas(cmdBaseWorker.wCurrentUser,cmdBaseWorker.wOutCardUser)
                    }
            ...
            playNode.node.runAction(offLineAction)
        }
        else
        {
    ...
    _getHandCardsGroupNode:function()
    {
        ...
        var touchEndCall = function(endCardSpr)
        {
            ...
            else
            {
                ...
                //东阳特殊autofill
                var num = cardLogic.getNum(endCardSpr.cardData) 
                var wangCount = 0
                for(var i=cardSprs.length-1;i>=0;i--) //cardSprs打牌在后面
                {   
                    var cardSpr = cardSprs[i]
                    if(cardSpr.cardData>=78)
                        wangCount++
                    else
                        break
                }
                ...
                {
                    var cardSpr = cardSprs[i]
                    var num2 = cardLogic.getNum(cardSpr.cardData)
                    if(num == num2 || (wangCount>=4&&num>13&&num2>13) )
                        cardSpr.y = cardSpr.originY + cardFactory.selectCardOffsetY
                    else
                        cardSpr.y = cardSpr.originY
...
// ]
    ...
    popGameEnd:function(continueCall, userData_gameEnd)
    {
        ...
        for(var wChairID=0;wChairID<GAME_PLAYER;wChairID++)
        {
            ...
            resultTTF.setString(('分:'+cmdBaseWorker.lPlayerData[wChairID][0]
                +' 赏:'+cmdBaseWorker.lPlayerData[wChairID][1]))
            resultTTF.anchorX = 0
            resultTTF.x = 102
            resultTTF.y = 25
            ...
            else
            {
                for(var j=0;j<handCardDatas.length;j++)
                {
                    ...
                    var card = cardFactory.getOne(cardData, 0, 0, true)
                    ...
                    if(card.idxInHandCards==0 || 
                        cardLogic.getNum(cardData)!=cardLogic.getNum(handCardDatas[card.idxInHandCards-1]) )
                    {
                        var count = 0
                        for(var i=0;i<handCardDatas.length;i++)
                        {
                        ...
                        var countText = new ccui.TextAtlas()
                        countText.setProperty(count, resp.nums1, 21, 30, "0")
                        countText.x = card.width * 0.12
                        countText.y = card.height* 0.1
    ...
    //         //     if(wChairID == cmdBaseWorker.wBankerUser)
    //         //     {
    //         //         if(cmdBaseWorker.cbHandCardCount[wChairID]!=0 && cmdBaseWorker.cbOutCardTimes[wChairID]==1)
    //         //             cbWinFlag[wChairID] = 7
    //         //     }
    //         //     else if(nongMinNotOut)
    //         //     {
    //         //         cbWinFlag[wChairID] = 7
    //         //     }
    //         // }
    ...
    //     for(var wChairID=0;wChairID<GAME_PLAYER;wChairID++)
    //     {
    //         control['resultNode'+wChairID].setVisible(true)
    ...
    //         control['banker'+wChairID].setVisible(cmdBaseWorker.wBankerUser == wChairID)
    ...
    outCard:function(cardType)
    {
        ...
        if(outCards.length>=4 && outCards[0].cardData<78)
           playNode.decorateOutCardsNode({id:4, length:outCards.length}, playNode.outCards4D, operateDir, outCardsNode)
    ...
    updateTipsArray:function(operateHandCardData,operateUser)
    {
        var isFirstOut = !cmdBaseWorker.outCardType.cardDatas
        var handCardDatas = operateHandCardData[operateUser]
        handCardDatas = cardLogic.sortWithNum( clone(handCardDatas) )
        var tipsArray
        if(isFirstOut)
        {
        ...
        {
            var cardsType = cmdBaseWorker.outCardType
            tipsArray = gameLogic.getTipsArray(cardsType, handCardDatas, [])
        }

        //普通牌，炸弹不可拆分，比如8张炸弹必须同时出，不可分成两组四张炸弹，也不可分成3张+5张；
        //过滤  
        var wangCount = 0
        for(var i=0;i<handCardDatas.length;i++)
        {
            if(handCardDatas[i]>=78)
                wangCount++
        }
        playNode.tipsArray = []
        for(var i=0;i<tipsArray.length;i++)
        {
            var tips = tipsArray[i]
            var num1 = cardLogic.getNum(tips[0])
            var sameCardCount = 0
            for(var ii=0;ii<handCardDatas.length;ii++)
            {   
                var num2 = cardLogic.getNum(handCardDatas[ii])
                if(num1<=13)
                {
                    if(num1==num2)
                        sameCardCount++
            ...
            if(sameCardCount==tips.length)
                playNode.tipsArray[playNode.tipsArray.length] = tips
        }
    },
    popOutCardNode:function(isCan)
    {
        ...
        playNode.updateTipsArray(cmdBaseWorker.cbHandCardData,tableData.getUserWithUserId(selfdwUserID).wChairID)
    ...
    getChooseTypeItem:function(listViewWidth, params)
    {
        ...
        var cardTypeSpr = new cc.Sprite("#" + 'ddz_typeIcon' + params.id + '.png')
    ...
    playCardTypeEffect:function(cardsType, isMan)
    {
        ...
        if(cardsType.id == 101)
            ...
            name = playNode.getSoundName(cardsType, true)
    ...
    refreshPlayerData:function(playerData)
    {
        ...
        {
            var user = tableData.getUserWithChairId(wChairID)
            if(!user && !cmdBaseWorker.cbGameState[wChairID] == US_OFFLINE_TRUSTEE) continue
            ...
            user.userNodeInsetChair.currentRoundNode.upTTF.setString('分:'+score+' 赏:'+shangCount)
            //user.userNodeInsetChair.currentRoundNode.upTTF.setString('')
        }
    },
    getSoundName:function(cardsType, isFirstOut)
    {
        var name = ''
        switch(cardsType.id)
        {
            case 1:
            {
                if(cardsType.cardDatas.length>1)
                {
                    if(isFirstOut)
                        name = name + 'shunzi'
                    else
                    ...
                    name = name + 'danzhang' + cardLogic.getNum(cardsType.cardDatas[0])
                break
            }
            case 2:
            {
                if(cardsType.cardDatas.length>2)
                {
                    if(isFirstOut)
                        name = name + 'liandui'
                    else
                    ...
                    name = name + 'duizi' + cardLogic.getNum(cardsType.cardDatas[0])
                break
            }
            case 3:
            {
                if(cardsType.cardDatas.length==3)
                    name = name + 'sange' + cardLogic.getNum(cardsType.cardDatas[0])
                else if(cardsType.cardDatas.length==4)
                {
                    if(isFirstOut)
                        name = name + 'sandaiyi'
                    else
                ...
                else if(cardsType.cardDatas.length==5)
                {
                    if(isFirstOut)
                        name = name + 'sandaier'
                    else
    ...
    decorateOutCardsNode:function(cardsType, outCards4D, direction, outCardsNode)
    {
        var outCards = outCards4D[direction]

        if(cardsType.id == 4 )
        {
            var lineNum = cardsType.length
            var outCountOneRow = direction%2==0?cardFactory.outCountOneRow_upDown:cardFactory.outCountOneRow_rightLeft
            var centerX = 0
            var centerY = 0
            if(lineNum<=outCountOneRow)
            {
                centerX = (outCards[0].x + outCards[outCards.length-1].x)/2
                centerY = outCards[0].y
            }
            else
            {
                centerX = (outCards[0].x + outCards[outCountOneRow-1].x)/2
                centerY = (outCards[0].y + outCards[outCards.length-1].y)/2
            }

            var outCardSpr = new cc.Sprite('#outDecorate1.png')
            outCardSpr.x = centerX
            outCardSpr.y = centerY
            outCardsNode.addChild(outCardSpr, 1000)  

            var linebg2 = new cc.Sprite('#outDecorate2.png')
            linebg2.x = centerX + 15
            linebg2.y = centerY
            outCardsNode.addChild(linebg2, 1000 )  
            var starNum = new ccui.TextAtlas()
            starNum.setProperty(lineNum, resp.nums5, 18, 23, "0")
            starNum.x = centerX - 15
            starNum.y = centerY
            outCardsNode.addChild( starNum, 1000 )
        }
    },
    getOffLineUserOutCardDatas:function(wCurrentUser,wOutCardUser)
    {
        var userisOffLine = cmdBaseWorker.cbGameState[wCurrentUser] == US_OFFLINE_TRUSTEE
        var self = tableData.getUserWithUserId(selfdwUserID)

        if(!userisOffLine || self.wChairID != cmdBaseWorker.wReplaceUser)
            return

        //获取可以出的牌
        //替代玩家
        console.log("66666666" ,wCurrentUser,cmdBaseWorker.wReplaceUser)
        playNode.updateTipsArray(cmdBaseWorker.cbHandCardDataOffLine,wCurrentUser)

        var offLineAction = cc.sequence( 
            cc.callFunc(function()
            {   
                if(playNode.tipsArray.length != 0 && cmdBaseWorker.wReplaceUser == self.wChairID)
                {
                    console.log("playNode.tipsArray" + playNode.tipsArray)
                    var isContinuousOut = wCurrentUser == wOutCardUser || wOutCardUser == INVALID_WORD
                    var cardsTypeArrray = gameLogic.getCardsTypesWithCardDatas( playNode.getMinCardsTypesWithCardDatas(isContinuousOut), cmdBaseWorker.cbMagicCardData ) 
    ...
    getMinCardsTypesWithCardDatas:function(isContinuousOut)
    {
        if(isContinuousOut)
        {
            var cardData = playNode.tipsArray[0][0]
            var minNum = gameLogic.num2Scores[cardLogic.getNum(cardData)]
            for(var i = 1;i < playNode.tipsArray.length;++i)
            {
                cardData = playNode.tipsArray[i][0]
                if(playNode.tipsArray[i].length > 3 || cardData >= 78)
                    continue
                
                var num = gameLogic.num2Scores[cardLogic.getNum(cardData)]
                if(num < minNum)
                {
                    minNum = num
                    playNode.tipsArray[0] = playNode.tipsArray[i]
                }
            }
        }


        return playNode.tipsArray[0]
    },
}
