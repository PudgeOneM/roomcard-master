...
{   
    ...
    preLoadRes:
    [
    'components/sharePop/res/sharePop.plist', 
    'components/sharePop/res/sharePop.png'
    ],
    ...
    {   
        ...
        var node  = cc.BuilderReader.load('components/sharePop/res/sharePop.ccbi', control)
        control.linkLabel.setString(wxData.data.share.title +  '\n' + wxData.data.share.desc)    
        sharePop._showArrow()
        ...
        cc.loader.loadImg(url, {isCrossOrigin : false}, function(err,img){
                ...
                control.headIcon.setSpriteFrame(frame)
        })
        ...
        var listener = cc.EventListener.create({
            ...
            swallowTouches: true,
            onTouchBegan: function (touch, event) {
                if(control.linkNode.isVisible()) return false
                var target = event.getCurrentTarget()
                var locationInNode = target.convertToNodeSpace(touch.getLocation())
                var s = target.getContentSize();
                var rect = cc.rect(0, 0, s.width, s.height)
            ...
            onTouchEnded: function (touch, event) {
                control.linkNode.setVisible(true)
                sharePop._showArrow()
                control.ewmNode.setVisible(false)
                control.linkBtn.setSpriteFrame('sp_shareBtn1_2.png') 
                control.ewmBtn.setSpriteFrame('sp_shareBtn2_0.png') 
            }
        })
        cc.eventManager.addListener(listener, control.linkBtn)
        ...
        var listener = cc.EventListener.create({
            ...
            onTouchEnded: function (touch, event) {
                control.linkNode.setVisible(false)
                sharePop._hideArrow()
                control.ewmNode.setVisible(true)
                control.linkBtn.setSpriteFrame('sp_shareBtn1_0.png') 
                control.ewmBtn.setSpriteFrame('sp_shareBtn2_2.png') 
                ...
                function showEwmImg(url)
                {
                    cc.loader.loadImg(url, {isCrossOrigin : false}, function(err,img){
                            ...
                            var frame = new cc.SpriteFrame(texture2d, cc.rect(0, 0, texture2d.getContentSize().width, texture2d.getContentSize().height))
                            control.ewmImg.setSpriteFrame(frame)
                            control.ewmImg.setScale(225/texture2d.getContentSize().width)
                ...
                if(f)
                {
                    control.ewmImg.setSpriteFrame(f)
                    control.ewmImg.setScale(225/f.getTexture().getContentSize().width)
                }
                else
                {
        ...
        cc.eventManager.addListener(listener, control.ewmBtn)
