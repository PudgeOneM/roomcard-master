...
function httpRequest(type, url, callback, body, args)
{
    var xmlhttp
...
function loadJs(url, onLoad, isDependLocalStorage, md5)
{
    if( isDependLocalStorage && localStorage[url] && hex_md5(localStorage[url]) == md5)
    {   
        onLoad?onLoad(localStorage[url]):''
    }
    else
    {
        httpRequest("GET",url+'?md5='+md5,function(ret, data){
            if(ret)
            {   
                if(isDependLocalStorage)
                {
                  try
                  {
...
function loadJSArray(params)
{ 
    progressBar.increase(1/(params.length+1) * 100)
    for(var i=0;i<params.length;i++)
    {   
        (function(i)
        {
            var oldcall = params[i].callback
            params[i].callback = function(data)
            { 
                oldcall?oldcall(data):''
                ...
                window.eval(data)
                if(params[i+1])
                  loadJs(params[i+1].url, params[i+1].callback, params[i+1].isDependLocalStorage, params[i+1].md5)
            }
        })(i)
    }
    loadJs(params[0].url, params[0].callback, params[0].isDependLocalStorage, params[0].md5)
}

/////////////////////////loadjs end////////////////////

function appendRefreshtime(href)
{
    if(href.search(/refreshtime\=/)<0)
        if(href.search(/\?/)<0)
          return href + '?refreshtime=' + new Date().getTime()
        else
          return href + '&refreshtime=' + new Date().getTime()
    else
        return href.replace(/refreshtime\=[0-9]+/, 'refreshtime=' + new Date().getTime())
}
