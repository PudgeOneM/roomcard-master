...
{   
    ...
    {   
        var resp = userSettingPop.resp
        var node  = cc.BuilderReader.load(resp + 'userSettingPop.ccbi', userSettingPop)
        ...
        var listener = cc.EventListener.create({
            ...
            onTouchEnded: function (touch, event) {
                ...
                userSettingPop._updateData.call(userSettingPop)
            }
        })
        cc.eventManager.addListener(listener, userSettingPop.closeIcon)
        ...
        for(var i=0; i<2; i++)
        {
            (function(i)
            {
                var listener = cc.EventListener.create({
                    ...
                    onTouchEnded: function (touch, event) {
                        userSettingPop.styleChoose.x = userSettingPop['styleSpr'+i].x
                    }
                })
                cc.eventManager.addListener(listener, userSettingPop['styleSpr'+i])
            })(i)
        }
        ...
        for(var i=0; i<userSettingPop.itemShowState.length; i++)
        {
            ...
            {
                userSettingPop['switchOn' + '_' + i].color = cc.color( 100, 100, 100)
                userSettingPop['switchOff' + '_' + i].color = cc.color( 100, 100, 100)
                continue  
            }
            ...
            (function(i)
            {
                var listener = cc.EventListener.create({
                    ...
                    onTouchEnded: function (touch, event) {
                        userSettingPop['switchOn' + '_' + i].setVisible(!userSettingPop['switchOn' + '_' + i].isVisible())
                        userSettingPop['switchOff' + '_' + i].setVisible(!userSettingPop['switchOff' + '_' + i].isVisible())
                    }
                })
                cc.eventManager.addListener(listener, userSettingPop['switchNode' + '_' + i])
            })(i)
        }

        userSettingPop._refreshUi.call(userSettingPop)
