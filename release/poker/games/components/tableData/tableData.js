...
{   
    ...
    {   
        ...
        tableData.wOriginalPlayFees = null
        ...
        case MDM_GR_MANAGE:  //6
            {
                ...
                case SUB_GR_ACCESS_TABLE_BY_KEY_RESULT:
                {
                    ...
                    //服务端还没加这个字段
                    body.wOriginalPlayFees = body.wTotalPlayFees
                    //1.19-1.28 3D麻将打5折
                    var t = new Date().getTime()
                    if(gameKind == '25D4P14' && t>1516291200000 && t<1517068800000)
                    {
                        body.wOriginalPlayFees = body.wTotalPlayFees*2
                    }
                    ...
                    {  
                        ...
                        tableData.wOriginalPlayFees = body.wOriginalPlayFees
           ...
           case MDM_GF_FRAME: //100
                {   
                    ...
                    {   
                        ...
                        if(body.wItemIndex<1000)
                        {
                            var event = new cc.EventCustom("userFace")
                            event.setUserData([user, body.wItemIndex])
                            cc.eventManager.dispatchEvent(event)
                        }
                        else
                        {
                            var event = new cc.EventCustom("userTalk")
                            event.setUserData([user, body.wItemIndex-1000])
                            cc.eventManager.dispatchEvent(event) 
                        }
        ...
        case MDM_GR_USER:       //用户信息  3
            {   
                ...
                {   
                    ...
                    if(chair)
                        chair.userNode.userScore.setString(user.lScoreInGame)
    ...
    _getOldShowChairIdWithNew:function(oldShowChairId, newShowChairId)
    {
        ...
        for(var i=0;i<SHOW_CHAIR;i++)
        {
            var o = (oldShowChairId + i)%SHOW_CHAIR
            var n = (newShowChairId + i)%SHOW_CHAIR
    ...
    {
        var serverChairId = tableData.showChairId2ServerChairIdMap[showChairId]
        if(serverChairId<SERVER_CHAIR)
        return serverChairId
    },
